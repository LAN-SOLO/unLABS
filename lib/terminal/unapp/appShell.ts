...
const STANDARD_MODULES: Record<
  string,
  (deviceId: string, ctx: CommandContext, app: AppRegistryEntry) => string[]
> = {
...
export function renderApp(
  ...
  if (app.device_id) {
    ...
    if (customModules && customModules[targetModule]) {
      return [...header, ...customModules[targetModule](ctx, app), "", ...renderModuleMenu(app)];
    }
  }
  ...
  if (sysModules && sysModules[targetModule]) {
    return [...header, ...sysModules[targetModule](ctx, app), "", ...renderModuleMenu(app)];
  }
