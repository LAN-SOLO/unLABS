...
const uec001: Record<string, ModuleRenderer> = {
  ...
  "cascade-monitor": (ctx) => {
    ...
    const risk =
      stability > 80 ? "LOW" : stability > 50 ? "MODERATE" : stability > 25 ? "HIGH" : "CRITICAL";
...
const p3d001: Record<string, ModuleRenderer> = {
  ...
  "quality-presets": () => [
    ...
    "  ─────────────────────────",
    "  [1] Draft      (0.3mm, fast)",
    "  [2] Standard   (0.2mm, balanced)",
    "  [3] Fine       (0.1mm, slow)",
    "  [4] Ultra-Fine (0.05mm, very slow)",
  ],
  "model-viewer": () => [
    "  MODEL VIEWER",
    "  ─────────────────────────",
    "  No model loaded.",
    "  Use print-queue to start a job.",
  ],
...
const hms001: Record<string, ModuleRenderer> = {
  ...
  "oscillator-bank": (ctx) => {
    ...
    return [
      "  OSCILLATOR BANK",
      "  ─────────────────────────",
      `  Active OSCs:  ${state?.oscillatorCount ?? 0}`,
      `  Tier:         T${state?.currentTier ?? 1}`,
    ];
  },
  "waveform-designer": () => [
    "  WAVEFORM DESIGNER",
    ...
    "  Use patch-editor for basic editing.",
  ],
  "preset-library": () => [
    "  PRESET LIBRARY",
    "  ─────────────────────────",
...
const exd001: Record<string, ModuleRenderer> = {
  ...
  "high-speed-mode": (ctx) => {
    ...
    return [
      "  HIGH-SPEED MODE",
      "  ─────────────────────────",
      `  Speed:     ${state?.speed ?? 0} m/s`,
      `  GPS Signal: ${bar(state?.gpsSignal ?? 0, 100)} ${state?.gpsSignal ?? 0}%`,
    ];
  },
};
...
const aic001: Record<string, ModuleRenderer> = {
  ...
  "inference-monitor": (ctx) => {
    ...
    return [
      "  INFERENCE MONITOR",
      "  ─────────────────────────",
      `  Anomalies:    ${state?.anomalyCount ?? 0}`,
      `  Uptime:       ${state?.uptime ?? 0}s`,
    ];
  },
  "learning-analytics": () => [
    "  LEARNING ANALYTICS",
...
const tmp001: Record<string, ModuleRenderer> = {
  "sensor-readings": (ctx) => {
    ...
    return [
      "  SENSOR READINGS",
      "  ─────────────────────────",
      `  Current:  ${state?.temperature ?? 0}°C`,
      `  Max:      ${state?.maxTemp ?? 0}°C`,
      `  Min:      ${state?.minTemp ?? 0}°C`,
    ];
  },
  "alert-thresholds": () => [
    "  ALERT THRESHOLDS",
  ...
  "heatmap-display": () => [
    "  HEATMAP DISPLAY",
    "  ─────────────────────────",
    "  Heatmap data requires panel view.",
  ],
  "history-charts": () => [
    "  HISTORY CHARTS",
    "  ─────────────────────────",
    "  Chart data requires panel view.",
  ],
};
...
const mem001: Record<string, ModuleRenderer> = {
  ...
  "allocation-map": () => [
    "  ALLOCATION MAP",
    "  ─────────────────────────",
    "  Map view requires panel display.",
  ],
  "cache-stats": () => [
    "  CACHE STATS",
    "  ─────────────────────────",
...
const net001: Record<string, ModuleRenderer> = {
  "traffic-analytics": (ctx) => {
    ...
    return [
      "  TRAFFIC ANALYTICS",
      "  ─────────────────────────",
      `  Bandwidth:    ${state?.bandwidth ?? 0} Mbps`,
      `  Latency:      ${state?.latencyMs ?? 0} ms`,
      `  Packet Loss:  ${state?.packetLoss ?? 0}%`,
    ];
  },
  "latency-monitor": (ctx) => {
    const state = ctx.data.netDevice?.getState();
...
const clk001: Record<string, ModuleRenderer> = {
  "timer-management": (ctx) => {
    ...
    return [
      "  TIMER MANAGEMENT",
      "  ─────────────────────────",
      `  Mode:       ${state?.displayMode ?? "local"}`,
      `  Stopwatch:  ${state?.stopwatchRunning ? "RUNNING" : "STOPPED"} (${(state?.stopwatchTime ?? 0).toFixed(1)}s)`,
    ];
  },
  ...
  "countdown-display": (ctx) => {
    ...
    return [
      "  COUNTDOWN",
      "  ─────────────────────────",
      `  Running: ${state?.countdownRunning ? "YES" : "NO"}`,
      `  Time:    ${(state?.countdownTime ?? 0).toFixed(1)}s`,
    ];
  },
};
...
const dim001: Record<string, ModuleRenderer> = {
  "stability-meter": (ctx) => {
    ...
    return [
      "  STABILITY METER",
      "  ─────────────────────────",
      `  Stability:  ${bar(state?.stability ?? 0, 100)} ${state?.stability ?? 0}%`,
      `  Rift:       ${state?.riftActivity ?? 0}%`,
    ];
  },
  "rift-detection": (ctx) => {
    const state = ctx.data.dimDevice?.getState();
    return [
      "  RIFT DETECTION",
      "  ─────────────────────────",
      `  Activity:   ${state?.riftActivity ?? 0}%`,
      `  Fluctuation: ${state?.fluctuation ?? 0}%`,
    ];
  },
...
const dgn001: Record<string, ModuleRenderer> = {
  "test-profiles": (ctx) => {
    ...
    return [
      "  TEST PROFILES",
      "  ─────────────────────────",
      `  Category:   ${state?.category ?? "SYSTEMS"}`,
      `  Scan Depth: ${state?.scanDepth ?? 75}%`,
      `  Health:     ${bar(state?.healthPercent ?? 0, 100)} ${state?.healthPercent ?? 0}%`,
    ];
  },
  "health-reports": (ctx) => {
    const state = ctx.data.dgnDevice?.getState();
  ...
  "component-viewer": () => [
    "  COMPONENT VIEWER",
    "  ─────────────────────────",
    "  36 device components registered.",
  ],
};
...
const vnt001: Record<string, ModuleRenderer> = {
  "airflow-control": (ctx) => {
    ...
    return [
      "  AIRFLOW CONTROL",
      "  ─────────────────────────",
      `  CPU Fan: ${state?.cpuFan?.rpm ?? 0} RPM (${state?.cpuFan?.mode ?? "AUTO"})`,
      `  GPU Fan: ${state?.gpuFan?.rpm ?? 0} RPM (${state?.gpuFan?.mode ?? "AUTO"})`,
    ];
  },
  "filter-status": (ctx) => {
    const state = ctx.data.vntDevice?.getState();
    return [
      "  FILTER STATUS",
      "  ─────────────────────────",
      `  Health:     ${bar(state?.filterHealth ?? 0, 100)} ${state?.filterHealth ?? 0}%`,
      `  Air Quality: ${state?.airQuality ?? 0}%`,
    ];
  },
  "environment-readings": (ctx) => {
    ...
    return [
      "  ENVIRONMENT",
      "  ─────────────────────────",
      `  CPU Temp:   ${state?.cpuTemp ?? 0}°C`,
      `  GPU Temp:   ${state?.gpuTemp ?? 0}°C`,
...
const mfr001: Record<string, ModuleRenderer> = {
  "plasma-containment": (ctx) => {
    ...
    return [
      ...
      "  ─────────────────────────",
      `  Stability:  ${bar(state?.stability ?? 0, 100)} ${state?.stability ?? 0}%`,
      `  Plasma Temp: ${state?.plasmaTemp ?? 0}°K`,
    ];
  },
  "ignition-control": () => [
    "  IGNITION CONTROL",
  ...
  "fusion-metrics": (ctx) => {
    ...
    return [
      "  FUSION METRICS",
      "  ─────────────────────────",
      `  Output:     ${state?.powerOutput ?? 0}W`,
      `  Efficiency: ${bar(state?.efficiency ?? 0, 100)} ${state?.efficiency ?? 0}%`,
      `  Ring Speed: ${state?.ringSpeed ?? 0} RPM`,
    ];
  },
  "thermal-link": () => [
    "  THERMAL LINK",
...
const sca001: Record<string, ModuleRenderer> = {
  "job-manager": (ctx) => {
    ...
    return [
      "  JOB MANAGER",
      "  ─────────────────────────",
      `  Queue:      ${state?.jobQueue ?? 0} jobs`,
      `  Active:     ${state?.activeNodes ?? 0} nodes`,
    ];
  },
  "core-allocator": (ctx) => {
    const state = ctx.data.scaDevice?.getState();
    return [
      "  CORE ALLOCATOR",
      "  ─────────────────────────",
      `  Utilization: ${bar(state?.utilization ?? 0, 100)} ${state?.utilization ?? 0}%`,
      `  FLOPS:       ${state?.flops ?? 0} TFLOPS`,
    ];
  },
  "benchmark-suite": () => [
    "  BENCHMARK SUITE",
  ...
  "cluster-monitor": (ctx) => {
    ...
    return [
      "  CLUSTER MONITOR",
      "  ─────────────────────────",
      `  Nodes:      ${state?.activeNodes ?? 0}`,
      `  Memory:     ${bar(state?.memoryUsage ?? 0, 100)} ${state?.memoryUsage ?? 0}%`,
      `  Bandwidth:  ${state?.interconnectBandwidth ?? 0} GB/s`,
    ];
  },
};
...
const qan001: Record<string, ModuleRenderer> = {
  "analysis-queue": (ctx) => {
    ...
    return [
      "  ANALYSIS QUEUE",
      "  ─────────────────────────",
      `  Mode:        ${state?.mode ?? "SCAN"}`,
      `  Coherence:   ${bar(state?.coherence ?? 0, 100)} ${state?.coherence ?? 0}%`,
    ];
  },
  "state-viewer": () => [
    "  STATE VIEWER",
    "  ─────────────────────────",
    "  Quantum state visualization requires panel.",
  ],
  "measurement-history": () => [
    "  MEASUREMENT HISTORY",
    "  ─────────────────────────",
...
const qsm001: Record<string, ModuleRenderer> = {
  "coherence-tracker": (ctx) => {
    ...
    return [
      ...
      "  ─────────────────────────",
      `  Coherence:  ${bar(state?.coherence ?? 0, 100)} ${state?.coherence ?? 0}%`,
      `  Qubits:     ${state?.qubits ?? 0}`,
      `  Error Rate: ${state?.errorRate ?? 0}%`,
    ];
  },
  "decoherence-alerts": () => [
    "  DECOHERENCE ALERTS",
...
const emc001: Record<string, ModuleRenderer> = {
  "field-integrity": (ctx) => {
    ...
    return [
      "  FIELD INTEGRITY",
      "  ─────────────────────────",
      `  Field:      ${bar(state?.fieldStrength ?? 0, 100)} ${state?.fieldStrength ?? 0}%`,
  ...
  "sample-manager": (ctx) => {
    ...
    return [
      ...
      "  ─────────────────────────",
      `  Units:     ${state?.units ?? 0}`,
      `  Temp:      ${state?.temperature ?? 0}°K`,
    ];
  },
  "safety-interlock": () => [
    "  SAFETY INTERLOCK",
    "  ─────────────────────────",
    "  All interlocks: ENGAGED",
    "  Override: LOCKED",
  ],
};
...
const tlp001: Record<string, ModuleRenderer> = {
  "coordinate-manager": (ctx) => {
    ...
    return [
      "  COORDINATE MANAGER",
      "  ─────────────────────────",
      `  Last Dest:  ${state?.lastDestination ?? "None"}`,
      `  Charge:     ${bar(state?.chargeLevel ?? 0, 100)} ${state?.chargeLevel ?? 0}%`,
    ];
  },
  "teleport-history": () => [
    "  TELEPORT HISTORY",
...
const and001: Record<string, ModuleRenderer> = {
  "scanner-interface": (ctx) => {
    ...
    return [
      ...
      "  ─────────────────────────",
      `  Signal:     ${bar(state?.signalStrength ?? 0, 100)} ${state?.signalStrength ?? 0}%`,
      `  Anomalies:  ${state?.anomaliesFound ?? 0}`,
      `  Mode:       ${state?.displayMode ?? "waveform"}`,
    ];
  },
  "detection-map": () => [
    "  DETECTION MAP",
    "  ─────────────────────────",
    "  Map requires panel display.",
  ],
  "alert-config": () => [
    "  ALERT CONFIG",
    "  ─────────────────────────",
  ...
  "deep-scan": () => [
    ...
    "  ─────────────────────────",
    "  Deep scan: IDLE",
    "  Use undev AND-001 test to initiate.",
  ],
};
...
const rmg001: Record<string, ModuleRenderer> = {
  "pull-radius": (ctx) => {
    ...
    return [
      "  PULL RADIUS",
      "  ─────────────────────────",
      `  Strength:  ${bar(state?.strength ?? 0, 100)} ${state?.strength ?? 0}%`,
      `  Field:     ${state?.fieldActive ? "ACTIVE" : "INACTIVE"}`,
    ];
  },
  "collection-stats": () => [
    "  COLLECTION STATS",
...
const qcp001: Record<string, ModuleRenderer> = {
  "bearing-readings": (ctx) => {
    ...
    return [
      ...
      `  Mode:       ${state?.displayMode ?? "compass"}`,
    ];
  },
  "anomaly-proximity": (ctx) => {
    ...
    return [
      "  ANOMALY PROXIMITY",
      "  ─────────────────────────",
      `  Nearest: ${state?.anomalyDistance ?? 0}m at ${state?.anomalyDirection ?? 0}°`,
    ];
  },
  "navigation-mode": () => [
    "  NAVIGATION MODE",
...
const bat001: Record<string, ModuleRenderer> = {
  "charge-manager": (ctx) => {
    ...
    return [
      "  CHARGE MANAGER",
      "  ─────────────────────────",
      `  Charge:     ${bar(state?.chargePercent ?? 0, 100)} ${state?.chargePercent ?? 0}%`,
      `  Charging:   ${state?.isCharging ? "YES" : "NO"}`,
      `  Auto-Regen: ${state?.autoRegen ? "ON" : "OFF"}`,
    ];
  },
  "cell-health": (ctx) => {
    const state = ctx.data.batDevice?.getState();
...
const int001: Record<string, ModuleRenderer> = {
  "algorithm-selector": (ctx) => {
    ...
    return [
      "  ALGORITHM SELECTOR",
      "  ─────────────────────────",
      `  Accuracy:   ${bar(state?.interpolationAccuracy ?? 0, 100)} ${state?.interpolationAccuracy ?? 0}%`,
      `  Streams:    ${state?.inputStreams ?? 0}`,
    ];
  },
  "data-pipeline": () => [
    "  DATA PIPELINE",
  ...
  visualization: () => [
    "  VISUALIZATION",
    "  ─────────────────────────",
    "  Visualization requires panel view.",
  ],
};
...
const atk001: Record<string, ModuleRenderer> = {
  "tank-levels": () => [
    "  TANK LEVELS",
    "  ─────────────────────────",
    "  Primary:   " + bar(45, 100) + " 45%",
    "  Secondary: " + bar(20, 100) + " 20%",
  ],
  "transfer-controls": () => [
    "  TRANSFER CONTROLS",
    "  ─────────────────────────",
...
const spk001: Record<string, ModuleRenderer> = {
  "channel-routing": (ctx) => {
    ...
    return [
      "  CHANNEL ROUTING",
      "  ─────────────────────────",
      `  Volume:  ${bar(state?.volume ?? 0, 100)} ${state?.volume ?? 0}%`,
  ...
  "volume-control": (ctx) => {
    ...
    return [
      "  VOLUME CONTROL",
      "  ─────────────────────────",
      `  Level: ${bar(state?.volume ?? 0, 100)} ${state?.volume ?? 0}%`,
    ];
  },
  "frequency-response": () => [
    "  FREQUENCY RESPONSE",
    "  ─────────────────────────",
    "  Response curve requires panel view.",
  ],
};
...
const msc001: Record<string, ModuleRenderer> = {
  "scan-interface": (ctx) => {
    ...
    return [
      "  SCAN INTERFACE",
      "  ─────────────────────────",
      `  Scan Line:  ${state?.scanLine ?? 0}`,
      `  Detected:   ${state?.detectedMaterials ?? 0} materials`,
    ];
  },
  "material-database": () => [
    "  MATERIAL DATABASE",
...
const sysLab: Record<string, ModuleRenderer> = {
  ...
  "resource-summary": () => [
    "  RESOURCE SUMMARY",
    "  ─────────────────────────",
    '  Use "res list" for detailed resource data.',
  ],
  "activity-feed": () => [
    "  ACTIVITY FEED",
    "  ─────────────────────────",
  ...
  "quick-actions": () => [
    "  QUICK ACTIONS",
    "  ─────────────────────────",
    "  [1] Run diagnostics (undev DGN-001 test)",
    "  [2] Check power (unapp run PWR-001)",
    "  [3] View inventory (crystal list)",
  ],
};
...
const sysPwr: Record<string, ModuleRenderer> = {
  "grid-overview": (ctx) => {
    ...
    return [
      "  POWER GRID OVERVIEW",
      "  ─────────────────────────",
      `  Primary:    UEC-001 (${uec?.isPowered ? "ONLINE" : "OFFLINE"})`,
      `  Output:     ${uec?.energyOutput?.toFixed(1) ?? "0"} E/s`,
    ];
  },
  "load-planner": () => [
    "  LOAD PLANNER",
...
const sysRes: Record<string, ModuleRenderer> = {
  "tree-browser": () => [
    "  TECH TREE BROWSER",
    "  ─────────────────────────",
    '  Use "research" command for full tree.',
  ],
  "active-research": () => [
    "  ACTIVE RESEARCH",
    "  ─────────────────────────",
  ...
  "unlock-preview": () => [
    "  UNLOCK PREVIEW",
    "  ─────────────────────────",
    '  Use "research" to see available unlocks.',
  ],
};
...
const sysInv: Record<string, ModuleRenderer> = {
  "crystal-browser": () => [
    "  CRYSTAL BROWSER",
    "  ─────────────────────────",
    '  Use "crystal list" for crystal data.',
  ],
  "slice-organizer": () => [
    "  SLICE ORGANIZER",
    "  ─────────────────────────",
    '  Use "inv" for inventory.',
  ],
  "collection-stats": () => [
    "  COLLECTION STATS",
    "  ─────────────────────────",
...
const utlLog: Record<string, ModuleRenderer> = {
  "system-logs": () => [
    "  SYSTEM LOGS",
    "  ─────────────────────────",
    '  Use "history" for command history.',
  ],
  "device-logs": () => [
    "  DEVICE LOGS",
    "  ─────────────────────────",
...
const utlCfg: Record<string, ModuleRenderer> = {
  "config-browser": () => [
    "  CONFIG BROWSER",
    "  ─────────────────────────",
    '  Use "unsyspref" for system preferences.',
  ],
  validation: () => ["  VALIDATION", "  ─────────────────────────", "  All configs: VALID"],
  "backup-restore": () => [
    "  BACKUP/RESTORE",
    "  ─────────────────────────",
    "  Use UTL-BAK for backups.",
  ],
};
