...
export function renderStatusModule(
  ...
  if (state.fieldStability !== undefined)
    lines.push(`  Stability:    ${bar(state.fieldStability, 100)} ${state.fieldStability}%`);
  ...
  if (state.utilization !== undefined)
    lines.push(`  Utilization:  ${bar(state.utilization, 100)} ${state.utilization}%`);
  if (state.coherence !== undefined)
    lines.push(`  Coherence:    ${bar(state.coherence, 100)} ${state.coherence}%`);
...
export function renderDiagnosticsModule(
  ...
  const dev = getDeviceActions(deviceId, ctx);
  if (!dev) {
    return ["  DIAGNOSTICS", "  ─────────────────────────", `  Device ${deviceId} unavailable.`];
  }
  const state = dev.getState();
...
export function renderFirmwareModule(
  deviceId: string,
  ctx: CommandContext,
  app: AppRegistryEntry,
): string[] {
  ...
  // Check FirmwareManager for available updates
  ...
  if (fwActions) {
    ...
    if (check.available && check.update) {
      ...
      lines.push("", "  Firmware is up to date.");
    }
  } else {
