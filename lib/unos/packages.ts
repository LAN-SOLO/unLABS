...
const DEFAULT_PACKAGES: Package[] = [
  ...
  },
  {
    ...
    version: "4.1.0",
    description: "Crystal data cache toolkit",
    depends: ["unos-core"],
    provides: ["cdc-cli"],
    size: 1024,
    installedSize: 4096,
  ...
  },
  {
    ...
    version: "2.0.0",
    description: "Solana blockchain interface",
    depends: ["unos-core", "unnet-tools"],
    provides: ["unchain"],
    size: 2048,
    installedSize: 8192,
    installed: true,
  ...
  },
  {
    name: "screw-buttons",
    version: "1.0.0",
    description: "Screw button multiplayer system",
    depends: ["unos-core", "unnet-tools"],
    provides: ["screwbuttons"],
    size: 512,
    installedSize: 2048,
    installed: true,
    repository: "stable",
  ...
  },
  {
    name: "crystal-miner",
    version: "0.9.0",
    description: "Automated crystal mining daemon",
    depends: ["crystal-toolkit"],
    provides: ["unminer"],
    size: 1536,
    installedSize: 6144,
    installed: false,
  ...
  },
  {
    ...
    version: "0.5.0",
    description: "Quantum entanglement tools",
    depends: ["quantum-libs"],
    provides: ["qentangle"],
    size: 4096,
    installedSize: 16384,
    installed: false,
...
export class PackageManager {
  ...
  install(name: string): { success: boolean; message: string } {
    ...
    for (const dep of pkg.depends) {
      const depPkg = Array.from(this.packages.values()).find(
        (p) => p.name === dep || p.provides.includes(dep),
      );
      if (!depPkg?.installed) {
  ...
  remove(name: string): { success: boolean; message: string } {
    ...
    const dependents = Array.from(this.packages.values()).filter(
      (p) => p.installed && p.depends.includes(name),
    );
    if (dependents.length > 0) {
      return {
        success: false,
        message: `E: Cannot remove ${name}: required by ${dependents.map((d) => d.name).join(", ")}`,
      };
    }
