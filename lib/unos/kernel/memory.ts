...
export class MemoryManager {
  ...
  private updateDerived() {
    ...
    this.stats.freeKB = Math.max(
      0,
      this.stats.totalKB - used - this.stats.buffersKB - this.stats.cachedKB,
    );
  ...
  tick() {
    ...
    this.stats.cachedKB = Math.max(
      1_000_000,
      Math.min(6_000_000, this.stats.cachedKB + cacheDelta),
    );
  ...
  private oomCheck() {
    ...
    if (largest) {
      this.dmesg.write(
        "CRIT",
        "kernel",
        `Out of memory: Kill process ${largest.pid} (${largest.name}) score ${Math.floor(largest.rss / 1024)} or sacrifice child`,
      );
