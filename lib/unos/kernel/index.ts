...
export class Kernel {
  ...
  setSysctlValue(key: string, value: string): { success: boolean; message: string } {
    ...
    if (!entry)
      return {
        success: false,
        message: `sysctl: cannot stat /unproc/sys/${key.replace(/\./g, "/")}: No such file or directory`,
      };
    if (!entry.writable)
      return { success: false, message: `sysctl: permission denied on key '${key}'` };
    entry.value = value;
