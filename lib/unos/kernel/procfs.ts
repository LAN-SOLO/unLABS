...
export class ProcFS {
  ...
  generate(path: string): string | null {
    ...
    if (p === "/meminfo") {
      ...
      const pad = (label: string, val: number) =>
        `${label}:${" ".repeat(Math.max(1, 16 - label.length))}${String(Math.floor(val)).padStart(10)} kB`;
      return [
    ...
    if (p === "/modules") {
      ...
      return (
        modules
          ...
          .map(
            (m) =>
              `${m.name} ${m.size * 1024} ${m.refCount} ${m.dependencies.length > 0 ? m.dependencies.join(",") : "-"} Live 0x${(0xffffffffa0000000 + Math.floor(Math.random() * 0x10000000)).toString(16)}`,
          )
          .join("\n") + "\n"
      );
    }
