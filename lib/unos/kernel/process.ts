...
export class ProcessManager {
  ...
  spawnDaemons(): void {
    for (const daemon of DAEMON_PROCESSES) {
      this.spawn(daemon.name, daemon.cmdline, daemon.name === "init" ? 0 : INIT_PID, {
        ...
        nice: daemon.nice,
        state:
          daemon.name.startsWith("kworker") ||
          daemon.name.startsWith("ksoftirq") ||
          daemon.name === "rcu_sched" ||
          daemon.name === "kcompactd0"
            ? "sleeping"
            : "sleeping",
      });
    }
  ...
  kill(pid: number, signal: number): { success: boolean; message: string } {
    ...
    if (pid === INIT_PID && signal !== 0)
      return { success: false, message: `kill: (${pid}) - Operation not permitted` };
  ...
  setNice(pid: number, nice: number): { success: boolean; message: string } {
    ...
    return {
      success: true,
      message: `${pid} (process ID) old priority ${oldNice}, new priority ${nice}`,
    };
  }
  ...
  toJSON(): KernelProcess[] {
    ...
    return Array.from(this.processes.values()).filter(
      (p) => DAEMON_PROCESSES.some((d) => d.name === p.name) || p.pid === INIT_PID,
    );
  }
