...
export class ModuleManager {
  ...
  load(name: string): { success: boolean; message: string } {
    ...
    for (const dep of mod.dependencies) {
      ...
      if (!depMod || !depMod.loaded) {
        return {
          success: false,
          message: `modprobe: FATAL: Module ${name} depends on ${dep} which is not loaded`,
        };
      }
    }
  ...
  unload(name: string): { success: boolean; message: string } {
    ...
    if (!mod.loaded)
      return { success: false, message: `rmmod: ERROR: Module ${name} is not currently loaded` };
    if (mod.refCount > 0)
      return {
        success: false,
        message: `rmmod: ERROR: Module ${name} is in use (refcount: ${mod.refCount})`,
      };
