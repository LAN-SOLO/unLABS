...
const DEFAULT_CONNECTIONS: Connection[] = [
  ...
  },
  {
    ...
    state: "LISTEN",
    pid: 1205,
    process: "crystal-engine",
  },
  {
    proto: "tcp",
    localAddr: "10.0.0.1",
    localPort: 38742,
    remoteAddr: "149.91.3.17",
    remotePort: 8899,
    state: "ESTABLISHED",
    pid: 1201,
    process: "unchain",
...
export class NetworkManager {
  ...
  ifdown(name: string): { success: boolean; message: string } {
    ...
    if (!iface) return { success: false, message: `Interface ${name} not found` };
    if (iface.name === "unlo")
      return { success: false, message: `Cannot bring down loopback interface` };
    iface.state = "down";
    ...
    for (let i = 0; i < count; i++) {
      if (!isKnown) {
        ...
        results.push({
          success: true,
          host: ip,
          rtt: +(0.01 + Math.random() * 0.05).toFixed(3),
          ttl: 64,
          seq: i + 1,
        });
      } else if (isLocal) {
        results.push({
          success: true,
          host: ip,
          rtt: +(0.5 + Math.random() * 5).toFixed(3),
          ttl: 64,
          seq: i + 1,
        });
      } else {
        results.push({
          success: true,
          host: ip,
          rtt: +(15 + Math.random() * 85).toFixed(3),
          ttl: 52,
          seq: i + 1,
        });
      }
  ...
  static fromJSON(
    data:
      | NetworkInterface[]
      | { interfaces: NetworkInterface[]; routes?: Route[]; connections?: Connection[] },
  ): NetworkManager {
