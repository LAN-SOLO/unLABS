...
export class VirtualFS {
  ...
  private addDir(
    parent: VNode,
    name: string,
    opts: Parameters<VirtualFS["mknode"]>[2] = {},
  ): VNode {
  ...
  private _walkNode(
    currentPath: string,
    node: VNode,
    callback: (nodePath: string, node: VNode) => void,
  ): void {
  ...
  getNodeType(path: string): string | null {
    ...
    if (!node) return null;
    if (node.type === "device")
      return node.deviceType === "block" ? "block device" : "character device";
    ...
    if (node.name.endsWith(".conf") || node.name.endsWith(".cfg"))
      return "ASCII text (configuration)";
    ...
    if (
      node.content?.startsWith("<binary>") ||
      node.content?.startsWith("<kernel") ||
      node.content?.startsWith("<firmware")
    )
      return "binary data";
    return "regular file";
  }
  ...
  private createUnhome(root: VNode) {
    ...
    // /unhome/operator
    const opHome = this.addDir(home, "operator", {
      owner: "operator",
      group: "operator",
      permissions: 0o750,
    });
  ...
  private createUnsrv(root: VNode) {
    ...
    this.addFile(srvReports, "stability.log", {
      content:
        "[2026-01-28 00:00:00] Crystal stability check: ALL NOMINAL\n[2026-01-28 00:05:00] Volatility tier: 2 (TPS: 1847)\n",
    });
  ...
  private createUnusr(root: VNode) {
    ...
    this.addFile(usrBin, "crystal-cli", {
      permissions: 0o755,
    ...
    this.addFile(usrDoc, "README", {
      content: `_unOS - Unstable Laboratories Operating System\nVersion ${UNOS_VERSION}\n`,
    });
  ...
  private createUnvar(root: VNode) {
    ...
    this.addFile(varLogUnos, "system.log", {
      content:
        "[2026-01-28 00:00:00] _unOS kernel initialized\n[2026-01-28 00:00:01] All subsystems online\n",
    });
    ...
    this.addFile(varLogUnos, "screwbutton.log", {
      content:
        "[2026-01-28 00:00:03] ScrewButton subsystem initialized\n[2026-01-28 00:00:03] 4 buttons registered: SB-01, SB-02, SB-03, SB-04\n",
    });
    this.addFile(varLogUnos, "network-sync.log", {
      content:
        "[2026-01-28 00:00:04] Network sync daemon started\n[2026-01-28 00:00:05] Awaiting node-sync activation...\n",
    });
    this.addFile(varLog, "auth.log", {
      permissions: 0o640,
      content: "[2026-01-28 00:00:00] operator logged in from terminal\n",
    });
    // Jade's margin notes — lore clues for resonance protocols
    const jadeLog = this.addDir(varLog, "jade");
    this.addFile(jadeLog, "day47.txt", {
      content: [
        "DAY 47 — margin notes (pencil, smudged)",
        "",
        "the devices talk to each other when you are not looking.",
        "not in any protocol I recognize. in harmonics.",
        "",
        "when three sing the same note, the lab remembers what it forgot.",
        "",
        "I set the synthesizer, the recorder, and the speaker to the same",
        "frequency. 37 Hz. the number was arbitrary. the result was not.",
        "",
        "something woke up. briefly. the oscilloscope stuttered.",
        "I do not think it was an error.",
        "",
        "— jade",
      ].join("\n"),
    });
    this.addFile(jadeLog, "day112.txt", {
      content: [
        "DAY 112 — margin notes (ink, careful)",
        "",
        "the quantum devices are paired. I do not mean entangled in the",
        "physics sense. I mean they *remember* each other.",
        "",
        "if you power on QSM and QUA at the same time and bridge them,",
        "something transfers. not data. something older than data.",
        "",
        "the anomalies noticed. they always notice the quantum events.",
        "",
        "— jade",
      ].join("\n"),
    });

    // Fridge engineering logs — lore clues for resonance protocols
    const fridgeLog = this.addDir(varLog, "fridge");
    this.addFile(fridgeLog, "calibration_003.dat", {
      content: [
        "ENG LOG: CALIBRATION REPORT #003",
        "DATE: 2025-11-14",
        "OPERATOR: fridge",
        "",
        "Oscilloscope cross-reference against anomaly detector output.",
        "AND-001 sensitivity at maximum (100).",
        "DIM-001 mode: scan.",
        "",
        "FINDING: secondary carrier at f\u2080\u00b1\u03b4",
        "  origin: external",
        "  instrument error: unlikely (3 independent confirmations)",
        "",
        "NOTE: carrier persists only when all three devices are active",
        "simultaneously. Temporal window appears narrow (~20s).",
        "",
        "RECOMMENDATION: further investigation. this signal predates the lab.",
      ].join("\n"),
    });
  ...
  private createUnboot(root: VNode) {
    ...
    this.addFile(boot, "vmlinuz-6.1.0-_unSC", {
      permissions: 0o644,
      content: "<binary: kernel image>",
    });
    this.addFile(boot, "initramfs-6.1.0-_unSC.img", {
      permissions: 0o644,
      content: "<binary: initramfs>",
    });
  ...
  ls(path?: string, flags: { long?: boolean; all?: boolean } = {}): string[] {
    ...
    if (flags.long) {
      ...
      for (const name of dynamicNames) {
        result.push(
          `dr-xr-xr-x root       root            0 ${new Date().toLocaleDateString("en-US", { month: "short", day: "2-digit", hour: "2-digit", minute: "2-digit" })} ${name}/`,
        );
      }
      return result;
    }
  ...
  touch(path: string): string | null {
    ...
    const file = this.mknode(fileName, "file", {
  ...
  rm(path: string, recursive: boolean = false): string | null {
    ...
    if (target.type === "dir" && !recursive)
      return `rm: cannot remove '${path}': Is a directory (use -r)`;
  ...
  ln(target: string, linkName: string, symbolic: boolean = true): string | null {
    ...
    const parent = this.resolve(parentPath);
    if (!parent || parent.type !== "dir")
      return `ln: cannot create '${linkName}': No such file or directory`;
