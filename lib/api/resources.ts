...
export async function getPlayerContainers(playerId: string): Promise<ResourceContainerRow[]> {
  const result = await fromAny("resource_containers").select("*").eq("player_id", playerId);
...
export async function upsertPlayerContainer(
  ...
  const result = await fromAny("resource_containers").upsert(
    {
...
export async function getContainerDefinitions(): Promise<ContainerDefinitionRow[]> {
  ...
  const result = await fromAny("container_definitions").select("*").order("tier");
...
export async function upsertBootstrapState(
  ...
  const result = await fromAny("bootstrap_state").upsert(
...
export async function getProductionRecipes(): Promise<ProductionRecipeRow[]> {
  ...
  const result = await fromAny("production_recipes").select("*").order("tier");
...
export async function getAbstractumSources(playerId: string): Promise<AbstractumSourceRow[]> {
  const result = await fromAny("abstractum_sources").select("*").eq("player_id", playerId);
...
export async function upsertAbstractumSource(
  ...
  const result = await fromAny("abstractum_sources").upsert(
    {
      source_id: sourceId,
      player_id: playerId,
      ...data,
...
export async function recordContainerUpgrade(
  ...
  const result = await fromAny("container_upgrades").insert({
    player_id: playerId,
    container_id: containerId,
