...
export function AvailableCombinationCard({
  combo,
  currentDeviceId,
  onLink,
  linking,
}: AvailableCombinationCardProps) {
  const canLink = combo.is_unlocked;
  const statusSymbol = canLink ? "○" : "●";
  const statusColor = canLink ? "text-amber-400" : "text-green-500/30";

  return (
    <div className={`space-y-0.5 py-1 font-mono text-[10px] ${canLink ? "" : "opacity-60"}`}>
      {/* Header */}
      ...
      <div className="flex items-center gap-1 pl-4">
        <span className="text-green-500/40">=</span>
        <span className={canLink ? "text-amber-400/80" : "text-green-500/40"}>
          {combo.combo_name}
        </span>
      </div>
      ...
      <div className="flex items-center gap-1 pl-4">
        ...
        <span className={combo.combined_power >= 0 ? "text-green-500/50" : "text-red-400/50"}>
          {combo.combined_power > 0 ? "+" : ""}
          {combo.combined_power.toFixed(0)} E/s
        </span>
      </div>
      ...
      <div className="pl-4">
        {canLink ? (
          <button
            ...
            disabled={linking}
            className="cursor-pointer text-cyan-400/60 hover:text-cyan-400 disabled:text-green-500/20"
          >
