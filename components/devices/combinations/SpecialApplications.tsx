...
function getSpecialApps(device: Device): { name: string; description: string; system: string }[] {
  ...
  if (caps.has("research_boost") || caps.has("simulation")) {
    apps.push({
      ...
      description: "Boosts tech tree research speed",
      system: "Research",
    });
  }
  if (caps.has("exploration") || caps.has("aerial_survey")) {
    apps.push({
      ...
      description: "Find anomalies and resource nodes",
      system: "Exploration",
    });
  }
  if (caps.has("resource_attraction") || caps.has("material_collection")) {
    apps.push({
      ...
      description: "Gather resources while idle",
      system: "Economy",
    });
  }
  if (caps.has("sound_synthesis") || caps.has("multi_oscillator")) {
    apps.push({
      name: "Resonance Experiments",
      description: "Crystal frequency manipulation",
      system: "Audio",
    });
  }
  if (caps.has("containment_field") || caps.has("exotic_storage")) {
    apps.push({
      ...
      description: "Store volatile materials safely",
      system: "Safety",
    });
  }
  if (caps.has("trait_identification") || caps.has("state_mapping")) {
    apps.push({
      ...
      description: "Identify crystal traits with high accuracy",
      system: "Quantum",
    });
  }
  if (caps.has("automation") || caps.has("resource_optimization")) {
    apps.push({
      name: "Lab Automation",
      description: "AI-managed task scheduling",
      system: "Management",
    });
  }
  if (caps.has("teleportation")) {
    apps.push({
      ...
      description: "Short-range teleportation of materials",
      system: "Transport",
    });
  }
  if (caps.has("fabrication") || caps.has("prototyping")) {
    apps.push({
      ...
      description: "Manufacture parts and prototypes",
      system: "Production",
    });
  }
  if (caps.has("cooling") || caps.has("reactor_efficiency_boost")) {
    apps.push({
      ...
      description: "Manage heat for reactor efficiency",
...
export function SpecialApplications({ device }: SpecialApplicationsProps) {
  ...
  if (apps.length === 0) {
    return (
      <div className="py-2 font-mono text-[10px] text-green-500/30">
        ...
        <div key={app.name} className="flex items-start gap-2 py-0.5">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[9px] text-violet-400/50">{app.system}</span>
            </div>
