...
export function DevicePowerChart({
  powerFull,
  powerIdle,
  powerStandby,
  powerCurrent,
  ...
  return (
    <div className="font-mono text-[10px] whitespace-pre">
      <div className="mb-0.5 text-green-500/60">
        24H POWER {isGenerator ? "OUTPUT" : "DRAW"} (E/s)
      </div>
      {/* Y-axis label + chart */}
      <div className="flex">
        <div className="flex w-[36px] flex-col justify-between pr-1 text-right text-green-500/40">
          <span>{maxLabel}</span>
          <span>{midLabel}</span>
          <span>0</span>
        </div>
      ...
      <div className="flex">
        ...
        <div
          className="flex justify-between text-green-500/40"
          style={{ width: `${CHART_WIDTH}ch` }}
        >
          <span>0h</span>
          <span>12h</span>
          <span>NOW</span>
      ...
      {/* Legend */}
      <div className="mt-1 flex gap-4 text-green-500/40">
        <span>
          FULL:{" "}
