    ...
    <div className="flex flex-col gap-1">
      ...
      <div className="font-mono text-[10px] whitespace-pre">
        ...
        <div className="flex">
          ...
          <span className="text-glow-green text-green-400">{device.name}</span>
        ...
        <div className="flex">
          ...
          <span className="ml-4 text-green-500/30">│</span>
          <span className="ml-2 text-green-500/60">{device.version}</span>
          <span className="ml-4 text-green-500/30">│</span>
          <span className="ml-2 text-cyan-400">{device.tech_tree}</span>
          <span className="flex-1" />
          <span className="text-green-500/30">║</span>
        </div>
        <div className="text-green-500/30">╠{"═".repeat(76)}╣</div>
      ...
      {/* Tab bar */}
      <div className="flex items-center gap-0 font-mono text-[10px] whitespace-pre">
        <span className="text-green-500/30">║ </span>
        {TABS.map((tab, i) => (
          <span key={tab.key}>
        ...
        <span className="flex-1" />
        <button onClick={onBack} className="cursor-pointer text-green-500/50 hover:text-green-400">
          [B] BACK
        </button>
        <span className="text-green-500/30"> ║</span>
      </div>
      <div className="font-mono text-[10px] whitespace-pre text-green-500/30">
        {"╠" + "═".repeat(76) + "╣"}
      </div>
      ...
      <div className="min-h-[200px]">
        {activeTab === "info" && (
          <DeviceInfoSection device={device} state={state} playerState={playerState ?? null} />
        )}
      ...
      <div className="flex justify-between font-mono text-[9px] text-green-500/30">
