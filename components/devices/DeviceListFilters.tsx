...
import type {
  DeviceListFilters as FilterState,
  SortField,
  SortDirection,
  DeviceListSort,
  ...
  return (
    <div className="flex items-center gap-1">
      ...
      <select
        ...
        className="cursor-pointer border border-green-500/20 bg-transparent px-1 py-0 font-mono text-[10px] text-green-400 outline-none focus:border-green-500/50"
      >
        <option value="" className="bg-[#141618]">
          ALL
        </option>
        {options.map((o) => (
          <option key={o.value} value={o.value} className="bg-[#141618]">
            {o.label}
          </option>
        ))}
      </select>
    ...
    <div className="flex flex-wrap items-center gap-3 font-mono text-[10px]">
      ...
      <div className="flex items-center gap-1">
        ...
        <input
          ...
          placeholder="..."
          className="w-[100px] border border-green-500/20 bg-transparent px-1 py-0 font-mono text-[10px] text-green-400 placeholder-green-500/20 caret-green-400 outline-none focus:border-green-500/50"
          spellCheck={false}
          autoComplete="off"
        />
      </div>
      ...
      <div className="flex items-center gap-1">
        ...
        <select
          value={filters.tier ?? ""}
          onChange={(e) =>
            updateFilter("tier", e.target.value ? Number(e.target.value) : undefined)
          }
          className="cursor-pointer border border-green-500/20 bg-transparent px-1 py-0 font-mono text-[10px] text-green-400 outline-none focus:border-green-500/50"
        >
          <option value="" className="bg-[#141618]">
          ...
          {[1, 2, 3, 4, 5].map((t) => (
            <option key={t} value={t} className="bg-[#141618]">
              T{t}
            </option>
          ))}
        </select>
      ...
      <button
        ...
        className="cursor-pointer border border-green-500/20 px-1 leading-[16px] text-green-400 hover:text-green-300"
      >
        {sort.direction === "asc" ? "▲" : "▼"}
      </button>
      ...
      {(filters.category || filters.state || filters.tier || filters.search) && (
        <>
          ...
          <button
            ...
            className="cursor-pointer text-amber-400/80 hover:text-amber-300"
          >
            [RESET]
          </button>
        </>
