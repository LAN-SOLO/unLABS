...
export function DeviceListItem({ device, selected, onClick }: DeviceListItemProps) {
  ...
  const healthColor =
    d.health >= 80 ? "text-green-400" : d.health >= 50 ? "text-amber-400" : "text-red-500";

  return (
    <div
      onClick={() => onClick(d.device_id)}
      className={`flex cursor-pointer font-mono text-[10px] leading-[18px] whitespace-pre transition-colors select-none hover:bg-green-500/5 ${selected ? "bg-green-500/10 text-green-300" : "text-green-400/80"} `}
    >
