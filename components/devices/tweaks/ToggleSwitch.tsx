...
export function ToggleSwitch({ tweak, onChange }: ToggleSwitchProps) {
  ...
  return (
    <button
      ...
      className="flex w-full cursor-pointer items-center gap-2 px-1 py-0.5 text-left font-mono text-[10px] hover:bg-green-500/5"
    >
      <span className={checked ? "text-green-400" : "text-green-500/30"}>
        {checked ? "[X]" : "[ ]"}
      </span>
      <span className={checked ? "text-green-400" : "text-green-500/60"}>{tweak.setting_name}</span>
