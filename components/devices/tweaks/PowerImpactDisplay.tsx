  ...
  return (
    <div className="font-mono text-[10px]">
      <div className="whitespace-pre text-green-500/30">
        {"┌─ "}
        <span className="text-green-500/60">POWER IMPACT</span>
        {" " + "─".repeat(48) + "┐"}
      </div>
      <div className="flex items-center gap-4 border-r border-l border-green-500/15 px-2 py-1">
        <div className="flex items-center gap-1">
          <span className="text-green-500/60">DEFAULT:</span>
          <span className="text-green-500/40">
            {defaultPower >= 0 ? "+" : ""}
            {defaultPower.toFixed(1)} E/s
          </span>
        </div>
        <div className="flex items-center gap-1">
      ...
      {isWorse && Math.abs(delta) > 10 && (
        <div className="mt-0.5 flex items-center gap-1 text-[9px] text-amber-400/80">
