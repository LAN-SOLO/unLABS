...
export function useTweakPanel({ deviceId, playerId }: UseTweakPanelProps) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const [defs, playerSettings] = await Promise.all([
          ...
          getPlayerTweakSettings(playerId, deviceId).catch(
    ...
    try {
      const raw =
        typeof window !== "undefined" ? localStorage.getItem(PRESETS_KEY_PREFIX + deviceId) : null;
  ...
  const setValue = useCallback((settingId: string, value: string | number | boolean) => {
    setTweaks((prev) =>
      prev.map((t) => (t.setting_id === settingId ? { ...t, current_value: value } : t)),
    );
    setDirty(true);
  }, []);
  ...
  const reset = useCallback(async () => {
    try {
      ...
      setTweaks((prev) => prev.map((t) => ({ ...t, current_value: t.default_value })));
      setDirty(false);
    } catch (e) {
