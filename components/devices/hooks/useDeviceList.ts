...
export function useDeviceList({ playerId, initialFilter }: UseDeviceListProps = {}) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true);
        const [devs, devStates] = await Promise.all([getAllDevices(), getAllDeviceStates()]);
  ...
  const filtered = useMemo(() => {
    return listItems.filter((item) => {
      ...
      if (filters.search) {
        ...
        if (!item.device_id.toLowerCase().includes(q) && !item.name.toLowerCase().includes(q)) {
          return false;
        }
      }
      return true;
  ...
  const sorted = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      switch (sort.field) {
        case "device_id":
          return a.device_id.localeCompare(b.device_id) * dir;
        case "name":
          return a.name.localeCompare(b.name) * dir;
        case "tier":
          return (a.tier - b.tier) * dir;
        case "power":
          return (a.power_current - b.power_current) * dir;
        case "health":
          return (a.health - b.health) * dir;
        case "load":
          return 0; // load not on DeviceListItem; fallback
        case "category":
