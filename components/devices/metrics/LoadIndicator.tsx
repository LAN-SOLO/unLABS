...
export function LoadIndicator({ load, width = 20 }: LoadIndicatorProps) {
  ...
  const color = clamped <= 60 ? "text-cyan-400" : clamped <= 80 ? "text-amber-400" : "text-red-500";

  const dimColor =
    clamped <= 60 ? "text-cyan-500/20" : clamped <= 80 ? "text-amber-500/20" : "text-red-500/20";
  ...
  return (
    <span className="font-mono text-[10px] whitespace-pre">
      ...
      {warn && <span className="ml-1 animate-pulse text-red-500">HIGH</span>}
    </span>
  );
}
