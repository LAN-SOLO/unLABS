...
export function TemperatureGauge({ temperature, maxThreshold = 85 }: TemperatureGaugeProps) {
  if (temperature === null) {
    return (
      <span className="font-mono text-[10px] whitespace-pre text-green-500/30">TEMP: --.-°C</span>
    );
  }
  ...
  const color =
    temperature < maxThreshold * 0.6
      ? "text-cyan-400"
      : temperature < maxThreshold * 0.8
        ? "text-amber-400"
        : "text-red-500";
  ...
  return (
    <span className="font-mono text-[10px] whitespace-pre">
      ...
      {warn && <span className="ml-1 animate-pulse text-red-500">&#9888;</span>}
    </span>
  );
}
