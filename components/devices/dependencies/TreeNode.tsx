...
export function TreeNodeDisplay({
  ...
  return (
    <div className="inline-flex flex-col font-mono text-[10px] leading-[16px]">
      {/* Device name line */}
      <div className="flex items-center gap-1">
        <span className="text-green-500/60">T{tier}:</span>
        <span className={isTarget ? "text-glow-green text-green-400" : "text-green-400"}>
          {name}
        </span>
        {isTarget && <span className="text-amber-400">◀</span>}
        {isCrossTree && <span className="text-[9px] text-violet-400/60">[{techTree}]</span>}
      </div>
      {/* Status line */}
