...
export function SysprefApp({ userId, username, initialArea, onExit }: SysprefAppProps) {
  ...
  return (
    <div
      className="flex h-full flex-col bg-[var(--bg-void,#0A0A0A)] font-mono text-sm text-[var(--crt-green,#00FF41)]"
      style={rootStyle}
    >
      <SysprefHeader hasUnsavedChanges={hasUnsavedChanges} />

      {confirmQuit && (
        <div className="border-b border-[var(--state-error,#FF3300)] bg-[rgba(255,51,0,0.1)] px-3 py-1 text-center text-[var(--state-error,#FF3300)]">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main
          ...
          role="main"
          aria-label={`${currentArea} settings`}
        >
          {renderPanel()}
        </main>
      </div>
