...
export function SysprefSidebar({ currentArea, onAreaChange }: SysprefSidebarProps) {
  return (
    <nav
      className="w-[16ch] shrink-0 border-r border-current py-1"
      role="navigation"
      aria-label="Preference areas"
    >
      <ul role="menubar">
        {areas.map((area) => {
          ...
          return (
            <li
              ...
              className={`cursor-pointer px-1 py-0.5 select-none ${
                selected
                  ? "bg-[rgba(255,170,0,0.1)] text-[var(--neon-amber,#FFAA00)]"
            ...
            >
              ...
              <span className="mr-1 text-[var(--state-offline,#666)]">[{area.key}]</span>
              {area.label}
            </li>
          );
        })}
