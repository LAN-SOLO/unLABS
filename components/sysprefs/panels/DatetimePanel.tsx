...
export function DatetimePanel({
  ...
  useEffect(() => {
    Promise.all([getDatetimePrefsServer(userId), getSystemConfigServer()])
      .then(([p, sc]) => {
        setPrefs(p);
        setOriginal(p);
        setSysConfig(sc);
  ...
  const update = useCallback(
    <K extends keyof DbPlayerDatetimePrefs>(key: K, value: DbPlayerDatetimePrefs[K]) => {
  ...
  if (!prefs)
    return (
      <div className="p-2 text-[var(--state-error,#FF3300)]">
        Failed to load datetime preferences
      </div>
    );
  ...
  const formatTime = () => {
    try {
      ...
      return (
        ...
        (prefs.show_milliseconds ? `.${String(now.getMilliseconds()).padStart(3, "0")}` : "")
      );
    } catch {
    ...
    <div className="space-y-1 overflow-y-auto">
      <SectionBox title="CURRENT TIME">
        <div className="py-2 text-center">
          ...
          <div className="mt-1 text-xs text-[var(--state-offline,#666)]">{prefs.timezone}</div>
        </div>
      </SectionBox>
      ...
      <SectionBox title="FORMAT">
        ...
        <Toggle
          label="Show Seconds"
          value={prefs.show_seconds}
          onChange={(v) => update("show_seconds", v)}
        />
        <Toggle
          label="Show Milliseconds"
          value={prefs.show_milliseconds}
          onChange={(v) => update("show_milliseconds", v)}
        />
        <Dropdown
          label="Week Starts"
