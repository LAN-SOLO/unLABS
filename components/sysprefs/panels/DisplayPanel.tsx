...
export function DisplayPanel({
  ...
  useEffect(() => {
    Promise.all([getDisplayPrefsServer(userId), getThemesServer(), getFontsServer()])
      .then(([p, t, f]) => {
        setPrefs(p);
        setOriginal(p);
        setThemes(t);
  ...
  const update = useCallback(
    <K extends keyof DbPlayerDisplayPrefs>(key: K, value: DbPlayerDisplayPrefs[K]) => {
  ...
  return (
    <div className="space-y-1 overflow-y-auto">
      ...
      <SectionBox title="CRT EFFECTS">
        <Toggle
          label="Scanlines"
          value={prefs.effect_scanlines}
          onChange={(v) => update("effect_scanlines", v)}
        />
        <Toggle
          label="Screen Curve"
          value={prefs.effect_curvature}
          onChange={(v) => update("effect_curvature", v)}
        />
        <Toggle
          label="Text Flicker"
          value={prefs.effect_flicker}
          onChange={(v) => update("effect_flicker", v)}
        />
        <Slider
          label="Phosphor Glow"
          value={prefs.effect_glow_intensity}
          min={0}
          max={100}
          onChange={(v) => update("effect_glow_intensity", v)}
        />
        <Toggle
          label="Glitch Effects"
          value={prefs.effect_glitch}
          onChange={(v) => update("effect_glitch", v)}
        />
        <Toggle
          label="Matrix Rain"
          value={prefs.effect_matrix_rain}
          onChange={(v) => update("effect_matrix_rain", v)}
        />
      </SectionBox>
      ...
      <SectionBox title="TYPOGRAPHY">
        ...
        <Slider
          label="Line Spacing"
          value={Math.round(Number(prefs.line_spacing) * 100)}
          min={100}
          max={250}
          showPercent={false}
          suffix="%"
          onChange={(v) => update("line_spacing", v / 100)}
        />
        <Slider
          label="Letter Spacing"
          value={Math.round(Number(prefs.letter_spacing) * 1000)}
          min={0}
          max={200}
          showPercent={false}
          suffix=""
          onChange={(v) => update("letter_spacing", v / 1000)}
        />
      </SectionBox>
      ...
      <SectionBox title="TERMINAL">
        <Slider
          label="Columns"
          value={prefs.terminal_columns}
          min={40}
          max={200}
          showPercent={false}
          onChange={(v) => update("terminal_columns", v)}
        />
        <Slider
          label="Rows"
          value={prefs.terminal_rows}
          min={15}
          max={60}
          showPercent={false}
          onChange={(v) => update("terminal_rows", v)}
        />
        <Dropdown
          label="Cursor Style"
          options={cursorOptions}
          value={prefs.cursor_style}
          onChange={(v) => update("cursor_style", v)}
        />
        <Dropdown
          label="Prompt Style"
          options={promptOptions}
          value={prefs.prompt_style}
          onChange={(v) => update("prompt_style", v)}
        />
        <Toggle
          label="Cursor Blink"
          value={prefs.cursor_blink}
          onChange={(v) => update("cursor_blink", v)}
        />
      </SectionBox>
      ...
      <SectionBox title="ACCESSIBILITY">
        <Toggle
          label="Plain Mode"
          value={prefs.plain_mode}
          onChange={(v) => update("plain_mode", v)}
          description="Disable all CRT effects"
        />
        <Toggle
          label="High Contrast"
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <input
        ...
        className="w-[9ch] border border-current bg-transparent px-1 font-mono text-inherit"
        maxLength={7}
      />
    </div>
  );
