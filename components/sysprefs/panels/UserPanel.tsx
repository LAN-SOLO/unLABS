    ...
    <div className="space-y-1 overflow-y-auto">
      ...
      {secPolicy && (
        <SectionBox title="SECURITY POLICY">
          ...
          <Row
            label="Special Char"
            value={secPolicy.require_special_char ? "Required" : "Not required"}
          />
          <Row
            label="Uppercase"
            value={secPolicy.require_uppercase ? "Required" : "Not required"}
          />
          <Row label="Number" value={secPolicy.require_number ? "Required" : "Not required"} />
          <Row
            label="Expiry"
            value={
              secPolicy.password_expiry_days ? `${secPolicy.password_expiry_days} days` : "Never"
            }
          />
      ...
      <SectionBox title="PASSWORD">
        <div className="text-[var(--state-offline,#666)]">
          Use the terminal command <span className="text-current">passwd</span> to change your
          password.
        </div>
      </SectionBox>
    </div>
