  ...
  return (
    <div className="flex flex-col">
      {suggestions.length > 1 && (
        <div className="flex flex-wrap gap-x-3 gap-y-0 px-0 pb-0.5 font-mono text-[9px] text-green-500/60">
          {suggestions.map((s, i) => {
      ...
      <div className="flex cursor-text items-center gap-2" onClick={handleContainerClick}>
        <span className="font-mono text-[10px] whitespace-nowrap text-green-400 select-none">
          {prompt}
        </span>
        <input
          ref={inputRef}
          ...
          disabled={disabled}
          className="flex-1 border-none bg-transparent font-mono text-[10px] text-green-400 placeholder-green-500/30 caret-green-400 outline-none disabled:opacity-50"
          ...
          spellCheck={false}
        />
        <span className="animate-pulse font-mono text-[10px] text-green-500">█</span>
      </div>
    </div>
  );
}
