...
export function MidnightCommander({
  ...
  useEffect(() => {
    try {
      localStorage.setItem(
        "unlabs_unmc_state",
        JSON.stringify({ left: leftPath, right: rightPath }),
      );
    } catch {
      /* ignore */
    }
      ...
      if (dialog) {
        if (dialog.type === "view") {
          if (e.key === "Escape" || e.key === "q" || e.key === "F10") {
            ...
            setDialog({
              ...dialog,
              scroll: Math.min(dialog.scroll + 1, Math.max(0, dialog.lines.length - visibleRows)),
            });
            ...
            setDialog({
              ...dialog,
              scroll: Math.min(
                dialog.scroll + visibleRows,
                Math.max(0, dialog.lines.length - visibleRows),
              ),
            });
        ...
        if (dialog.type === "edit") {
          ...
          if (e.key === "PageUp") {
            ...
            setDialog({
              ...dialog,
              cursorRow: newRow,
              cursorCol: Math.min(cursorCol, lines[newRow].length),
              scroll: newScroll,
            });
            return;
          }
          ...
          if (e.key === "Backspace") {
            if (cursorCol > 0) {
              const newLines = [...lines];
              newLines[cursorRow] =
                newLines[cursorRow].slice(0, cursorCol - 1) + newLines[cursorRow].slice(cursorCol);
          ...
          if (e.key === "Delete") {
            if (cursorCol < lines[cursorRow].length) {
              const newLines = [...lines];
              newLines[cursorRow] =
                newLines[cursorRow].slice(0, cursorCol) + newLines[cursorRow].slice(cursorCol + 1);
          ...
          if (e.key === "Tab") {
            const newLines = [...lines];
            newLines[cursorRow] =
              newLines[cursorRow].slice(0, cursorCol) + "  " + newLines[cursorRow].slice(cursorCol);
          ...
          if (e.key.length === 1 && !e.ctrlKey && !e.metaKey && !e.altKey) {
            const newLines = [...lines];
            newLines[cursorRow] =
              newLines[cursorRow].slice(0, cursorCol) +
              e.key +
              newLines[cursorRow].slice(cursorCol);
            setDialog({ ...dialog, lines: newLines, cursorCol: cursorCol + 1, modified: true });
            return;
          }
          return;
        ...
        if (dialog.type === "mkdir") {
          if (e.key === "Escape") {
            ...
            if (dialog.value.trim()) {
              ...
              const newPath =
                base === "/" ? `/${dialog.value.trim()}` : `${base}/${dialog.value.trim()}`;
  ...
  const renderPane = (
    ...
    for (let i = 0; i < visibleRows; i++) {
      ...
      rows.push({
        text: line.slice(0, PANE_WIDTH - 1),
        highlighted: realIdx === cursor && isActive,
      });
    }
    return { rows, path };
  };
  ...
  const renderLines = () => {
    ...
    if (dialog?.type === "edit") {
      ...
      for (let i = 0; i < editRows; i++) {
        ...
        if (isCursorLine) {
          ...
          const cursorChar =
            dialog.cursorCol < lineContent.length ? lineContent[dialog.cursorCol] : " ";
          ...
          const padW =
            contentW - displayBefore.length - (displayCursorChar ? 1 : 0) - displayAfter.length;
    ...
    lines.push(<div key="header">{`┌${leftTitlePad}┬${rightTitlePad}┐`}</div>);
    ...
    lines.push(
      <div key="colsep">{`├${"─".repeat(PANE_WIDTH - 1)}┼${"─".repeat(PANE_WIDTH - 1)}┤`}</div>,
    );
        ...
        <div key={`row-${i}`}>
          ...
          <span
            className={
              lr?.highlighted
                ? ""
                : lr?.text.trim() && leftPane.rows[i]?.text.includes("<DIR>")
                  ? "opacity-100"
                  : "opacity-70"
            }
            style={lr?.highlighted ? { backgroundColor: "currentColor" } : undefined}
          >
          ...
          <span
            className={
              rr?.highlighted
                ? ""
                : rr?.text.trim() && rightPane.rows[i]?.text.includes("<DIR>")
                  ? "opacity-100"
                  : "opacity-70"
            }
            style={rr?.highlighted ? { backgroundColor: "currentColor" } : undefined}
          >
    ...
    // Bottom separator
    lines.push(
      <div key="bottomsep">{`├${"─".repeat(PANE_WIDTH - 1)}┴${"─".repeat(PANE_WIDTH - 1)}┤`}</div>,
    );

    // Function key bar
    const fnBar = " F3 View F4 Edit F5 Copy F6 Move F7 Mkdir F8 Del F10 Quit";
    lines.push(<div key="fnbar">{`│${pad(fnBar, TOTAL_WIDTH - 2)}│`}</div>);
    lines.push(<div key="bottom">{`└${"─".repeat(TOTAL_WIDTH - 2)}┘`}</div>);
  ...
  return (
    <div
      ...
      className="min-h-0 flex-1 cursor-default overflow-hidden font-mono text-[10px] leading-tight outline-none select-none"
      style={{
        fontFamily:
          'ui-monospace, SFMono-Regular, "SF Mono", Menlo, Consolas, "Liberation Mono", monospace',
      }}
    >
      <pre className="whitespace-pre">{renderLines()}</pre>
    </div>
  );
}
