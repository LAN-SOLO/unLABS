  ...
  for (let y = 0; y < ph; y++) {
    for (let x = 0; x < pw; x++) {
      ...
      if (params.complexity >= 3)
        v += Math.sin(Math.sqrt(sx * sx * sFreq * sFreq + sy * sy * sFreq * sFreq * 0.6) + t * 0.5);
      if (params.complexity >= 4)
        v += Math.sin(sx * sFreq * 1.5 - sy * sFreq * 0.5 + t * 1.2) * 0.5;
      if (params.complexity >= 5)
        v += Math.cos(sx * sFreq * 0.7 + sy * sFreq * 1.3 + t * 0.9) * 0.3;
...
export function renderMatrix(
  ...
  if (!state || !state.inited || state.lastW !== numCols || state.lastH !== numRows) {
    const columns = new Array(numCols)
      .fill(0)
      .map(() =>
        Math.random() > 1 - params.density / 100
