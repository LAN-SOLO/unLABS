...
export function ScreensaverEditor({ onFullScreen }: ScreensaverEditorProps) {
  ...
  const setPatternParam = useCallback(
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="border-t border-current pt-1">
        <div className="mb-1 text-center text-xs opacity-60">PATTERN SETTINGS</div>
        <PatternControls pattern={pattern} params={params} setParam={setPatternParam} />
      </div>

      {/* Appearance settings */}
      <div className="border-t border-current pt-1">
        <div className="mb-1 text-center text-xs opacity-60">APPEARANCE</div>
        <RadioGroup
          label="Color Source"
          options={[
        ...
        {config.colorSource === "custom" && (
          <div className="flex items-center gap-2 px-1 py-0.5">
            <span className="inline-block w-[18ch] text-right whitespace-nowrap">
              Custom Color:
            </span>
            <label className="relative cursor-pointer" title="Pick color">
              ...
              <input
                ...
                onChange={(e) =>
                  updateConfig((c) => ({ ...c, customColor: e.target.value.toUpperCase() }))
                }
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (v.match(/^#[0-9A-Fa-f]{0,6}$/))
                  updateConfig((c) => ({ ...c, customColor: v.toUpperCase() }));
              }}
      ...
      <div className="flex justify-center pt-1">
        <button
          className="cursor-pointer border border-current px-4 py-1 font-mono hover:bg-[rgba(255,170,0,0.1)]"
      ...
      return (
        <>
          <Slider
            ...
            min={3}
            max={20}
            onChange={(v) => setParam("ringCount", v)}
            showPercent={false}
          />
          <Slider
            ...
            min={1}
            max={100}
            onChange={(v) => setParam("rotationSpeed", v)}
          />
          <Slider
            label="Line Width"
            value={p.lineWidth}
          ...
          <Slider
            ...
            min={10}
            max={100}
            onChange={(v) => setParam("centerDistance", v)}
          />
        </>
      );
    case "lissajous":
      return (
        <>
          ...
          <Slider
            ...
            min={1}
            max={100}
            onChange={(v) => setParam("phaseSpeed", v)}
          />
          <Slider
            label="Trail Length"
            value={p.trailLength}
            min={10}
            max={500}
            onChange={(v) => setParam("trailLength", v)}
            showPercent={false}
          />
        </>
      );
      ...
      return (
        <>
          <Slider
            ...
            min={20}
            max={100}
            onChange={(v) => setParam("outerRadius", v)}
          />
          <Slider
            label="Inner Radius"
            value={p.innerRadius}
            min={5}
            max={60}
            onChange={(v) => setParam("innerRadius", v)}
          />
          <Slider
            label="Pen Offset"
            value={p.penOffset}
            min={5}
            max={80}
            onChange={(v) => setParam("penOffset", v)}
          />
          <Slider
            label="Speed"
            value={p.speed}
          ...
          <Slider
            ...
            min={0}
            max={100}
            onChange={(v) => setParam("fadeRate", v)}
          />
        </>
      );
    case "matrix":
      return (
        <>
          ...
          <Slider
            ...
            min={8}
            max={24}
            onChange={(v) => setParam("fontSize", v)}
            showPercent={false}
            suffix="px"
          />
          <Dropdown
      ...
      return (
        <>
          <Slider
            ...
            min={50}
            max={500}
            onChange={(v) => setParam("starCount", v)}
            showPercent={false}
          />
          <Slider
            label="Speed"
