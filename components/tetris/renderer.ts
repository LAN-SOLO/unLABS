...
export function calculateLayout(
  ...
  const cellSize =
    totalNeededW > canvasW
      ? Math.floor(
          (canvasW -
            (sideW + BOARD_PADDING * 2 + 10) * playerCount -
            (playerCount === 2 ? 40 : 0)) /
            (COLS * playerCount),
        )
      : cellFromH;
  ...
  const actualTotalW =
    (boardW + sideW + BOARD_PADDING * 2 + 10) * playerCount + (playerCount === 2 ? 40 : 0);
  const startX = Math.floor((canvasW - actualTotalW) / 2);

  const layouts: BoardLayout[] = [];
  for (let p = 0; p < playerCount; p++) {
    const playerOffset =
      p * (boardW + sideW + BOARD_PADDING * 2 + 10 + (playerCount === 2 ? 40 : 0));
...
function drawGameOverOverlay(
  ...
  if (game.mode === "2p") {
    ctx.fillText(
      `P1: ${p1.score.toLocaleString()}  |  P2: ${game.players[1].score.toLocaleString()}`,
      w / 2,
      h / 2,
    );
  } else {
    ctx.fillText(
      `Score: ${p1.score.toLocaleString()}  Level: ${p1.level}  Lines: ${p1.lines}`,
      w / 2,
      h / 2,
    );
  }
