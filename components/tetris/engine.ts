...
import {
  ...
  DECAY_SPEED_FACTOR,
  DECAY_STABLE_RATIO,
  VOLATILITY_CONTAGION_THRESHOLD,
  VOLATILITY_CONTAGION_SPEEDUP,
  AGED_THRESHOLDS,
  getWallKicks,
  type PieceType,
  type RotationState,
...
function pullFromBag(state: PlayerState): {
  piece: PieceType;
  bag: PieceType[];
...
function lockPiece(
  ...
  piece: Piece,
  time: number,
): { board: CellValue[][]; cellAges: number[][] } {
...
function clearLines(
  ...
  if (fullRows.length === 0)
    return { board: newBoard, cellAges: newAges, linesCleared: 0, avgAgeRatio: 0 };
  ...
  const emptyBoardRows = Array.from(
    { length: fullRows.length },
    () => Array(COLS).fill(null) as CellValue[],
  );
  const emptyAgeRows = Array.from(
    { length: fullRows.length },
...
function applyGravity(
  state: PlayerState,
  time: number,
): { state: PlayerState; events: GameEvent[]; playerIdx: number } {
...
function softDrop(state: PlayerState): {
  state: PlayerState;
  events: GameEvent[];
  playerIdx: number;
} {
...
function hardDrop(
  ...
  // Lock immediately
  return lockAndSpawn({ ...state, currentPiece: piece, score }, playerIdx, time, events);
}
...
function lockAndSpawn(
  ...
  const {
    ...
    linesCleared,
    avgAgeRatio,
  } = clearLines(board, cellAges, time, state.level);
  ...
  return { state: newState, events, garbageSent } as { state: PlayerState; events: GameEvent[] } & {
    garbageSent?: number;
  };
}
  ...
  if (holdType) {
    ...
    if (!isValidPosition(state.board, newPiece)) {
      return {
        ...
        events: [...events, { type: "gameOver", player: playerIdx }],
      };
    }
...
export function updateGame(
  ...
  for (let i = 0; i < newPlayers.length; i++) {
    ...
    const vIndex = computeVolatilityIndex(
      newPlayers[i].board,
      newPlayers[i].cellAges,
      time,
      newPlayers[i].level,
    );
    const lifespan = getDecayLifespan(newPlayers[i].level);
    const effectiveLifespan =
      vIndex > VOLATILITY_CONTAGION_THRESHOLD ? lifespan * VOLATILITY_CONTAGION_SPEEDUP : lifespan;
    let decayed = false;
    ...
    if (playerState.lockTimer !== null && playerState.currentPiece) {
      if (time - playerState.lockTimer >= LOCK_DELAY) {
        ...
        const result = { state: locked, events } as { state: PlayerState; events: GameEvent[] } & {
          garbageSent?: number;
        };
...
export function processAction(
  ...
  switch (action.type) {
    ...
    case "hardDrop": {
      ...
      const hdResult = result as { state: PlayerState; events: GameEvent[] } & {
        garbageSent?: number;
      };
