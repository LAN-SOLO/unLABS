...
export function EquipmentTile({
  ...
  return (
    <PanelFrame variant={variant} className={cn("p-2", className)}>
      {/* Header */}
      <div className="mb-2 flex items-center justify-between">
        <div>
          <div className="font-mono text-[10px] tracking-wider text-[var(--neon-amber)] uppercase">
            {title}
          </div>
          {subtitle && <div className="font-mono text-[8px] text-white/40">{subtitle}</div>}
        </div>
      ...
      {value !== undefined && (
        <div className="mb-2 flex items-baseline gap-1">
          <span className="text-glow-cyan font-mono text-lg text-[var(--neon-cyan)]">{value}</span>
          {unit && <span className="font-mono text-[10px] text-white/50">{unit}</span>}
        </div>
      )}
...
export function CrystalDataCache({
  ...
  const [localTestPhase, setLocalTestPhase] = useState<
    "memory" | "bus" | "cache" | "power" | "protocol" | "complete" | null
  >(null);
  const [localShutdownPhase, setLocalShutdownPhase] = useState<
    "saving" | "flushing" | "releasing" | "halted" | null
  >(null);
  ...
  const bootPhase =
    cdcManager?.bootPhase ??
    (localBootPhase > 0
      ? (["post", "memory", "cache", "bus", "sync", "ready"][localBootPhase - 1] as
          | "post"
          | "memory"
          | "cache"
          | "bus"
  ...
  const handlePowerToggle = useCallback(async () => {
    if (deviceState === "booting" || deviceState === "shutdown" || deviceState === "rebooting")
      return;
  ...
  const handleTest = async () => {
    ...
    if (cdcManager) {
      ...
      const phases: ("memory" | "bus" | "cache" | "power" | "protocol" | "complete")[] = [
        "memory",
        "bus",
        "cache",
        "power",
  ...
  const microBtnClass = (active: boolean, color: string) =>
    cn(
    ...
    >
      ...
      >
        <div className="p-1.5 px-2">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="font-mono text-[8px] font-bold tracking-wider text-[var(--neon-amber)]">
              CDC-001
            </span>
            <span
              className={cn(
            ...
            <button
              ...
              disabled={isTransitioning}
              className="flex h-3 w-3.5 shrink-0 items-center justify-center rounded-sm border font-mono text-[5px] transition-all"
              style={{
                background:
                  isPowered && deviceState !== "standby" ? "rgba(0,255,102,0.15)" : "#0a0a0f",
                color:
                  isPowered && deviceState !== "standby"
                    ? "var(--neon-green)"
            ...
            {isPowered && deviceState !== "standby" ? (
              <button
                onClick={
                  showFoldedInfo
                    ? () => {
                        setShowFoldedInfo(false);
                        handleToggleExpand();
                      }
                    : toggleFoldedInfo
                }
                className="flex h-3 w-3.5 shrink-0 items-center justify-center rounded-sm border border-white/10 bg-[#0a0a0f] font-mono text-[6px] text-white/40 transition-all hover:border-white/20 hover:text-white/60"
          ...
          >
            <div className="mt-1 grid grid-cols-3 gap-2 border-t border-white/5 pt-1">
      ...
      >
        ...
        <div className="absolute top-1.5 right-1.5 z-10 flex items-center gap-1">
          <button onClick={handleToggleExpand} className="group" title="Fold">
            <div
              className="flex h-2.5 w-2.5 items-center justify-center rounded-sm border transition-all"
              style={{
                background: "#0a0a0f",
                borderColor: "rgba(255,255,255,0.15)",
              }}
            >
              <span className="font-mono text-[5px] text-white/40 transition-colors group-hover:text-white/70">
                ▲
              </span>
            </div>
          </button>
        ...
        <div className="mb-1.5 flex items-center justify-between pr-12">
          <div className="flex items-center gap-1.5">
            <LED on={isLedOn} color={getLedColor()} size="sm" />
            <div>
              <div className="font-mono text-[9px] tracking-wider text-[var(--neon-amber)] uppercase">
                Crystal Data
              </div>
        ...
        <div className="mb-1.5 grid grid-cols-3 gap-1">
          <div className="relative overflow-hidden rounded border border-white/5 bg-black/40 p-1">
            <div className="font-mono text-[6px] text-white/40">Crystals</div>
            <div
              className={cn(
                "font-mono text-sm tabular-nums transition-all duration-300",
                (deviceState === "booting" && bootPhaseNum < 4) || deviceState === "standby"
                  ? "opacity-50"
                  : "",
              )}
              style={{
                ...
                textShadow:
                  displayValues.crystals !== "--" && displayValues.crystals !== "0"
            ...
            {deviceState === "testing" && testPhase === "cache" && (
              <div className="absolute inset-0 animate-pulse bg-[var(--neon-cyan)]/10" />
            )}
          </div>
          <div className="relative overflow-hidden rounded border border-white/5 bg-black/40 p-1">
            ...
            <div
              ...
              style={{
                color: "var(--neon-cyan)",
                textShadow:
                  displayValues.slices !== "--" && displayValues.slices !== "0"
          ...
          <div className="relative overflow-hidden rounded border border-white/5 bg-black/40 p-1">
            ...
            <div
              ...
              style={{
                ...
                textShadow:
                  displayValues.power !== "--" && displayValues.power !== "0.0"
            ...
            {deviceState === "testing" && testPhase === "power" && (
              <div className="absolute inset-0 animate-pulse bg-[var(--neon-amber)]/10" />
            )}
          </div>
        </div>
        ...
        <div className="mt-1 flex items-center gap-1 border-t border-white/5 pt-1">
          <span
            className={cn(
              "flex-1 truncate font-mono text-[5px] transition-colors",
              deviceState === "testing"
                ? "text-[var(--neon-cyan)]"
                : deviceState === "rebooting" ||
                    deviceState === "booting" ||
          ...
          <button
            ...
            disabled={deviceState !== "online"}
            className={cn(
              "flex h-3 w-3.5 items-center justify-center rounded-sm border font-mono text-[5px] transition-all",
              deviceState === "testing"
                ? "border-[var(--neon-cyan)]/50 bg-[var(--neon-cyan)]/20 text-[var(--neon-cyan)]"
                : testResult === "pass"
                  ? "border-[var(--neon-green)]/50 bg-[var(--neon-green)]/20 text-[var(--neon-green)]"
                  : testResult === "fail"
                    ? "border-[var(--neon-red)]/50 bg-[var(--neon-red)]/20 text-[var(--neon-red)]"
                    : "border-white/10 bg-[#0a0a0f] text-white/40 hover:border-white/20 hover:text-white/60 disabled:opacity-30",
            )}
            title="Test"
          >
            {deviceState === "testing"
          ...
          <button
            ...
            className={cn(
              "flex h-3 w-3.5 items-center justify-center rounded-sm border font-mono text-[5px] transition-all",
              deviceState === "rebooting" || deviceState === "booting"
                ? "border-[var(--neon-amber)]/50 bg-[var(--neon-amber)]/20 text-[var(--neon-amber)]"
                : "border-white/10 bg-[#0a0a0f] text-white/40 hover:border-white/20 hover:text-white/60 disabled:opacity-30",
            )}
            title="Reboot"
          >
          ...
          <div className="ml-0.5 flex gap-0.5">
            <div
              className={cn(
                ...
                deviceState === "online" && crystalCount > 0
                  ? "bg-[var(--neon-green)]"
                  : "bg-white/20",
              )}
            />
            <div
              className={cn(
                "h-1 w-1 rounded-full transition-colors",
                deviceState === "online" && sliceCount > 0
                  ? "bg-[var(--neon-cyan)]"
                  : "bg-white/20",
              )}
            />
            <div
              className={cn(
                "h-1 w-1 rounded-full transition-colors",
                deviceState === "online" && totalPower > 0
                  ? "bg-[var(--neon-amber)]"
                  : "bg-white/20",
              )}
            />
...
export function EnergyCore({
  ...
  const [localBootPhase, setLocalBootPhase] = useState(0);
  const [localTestPhase, setLocalTestPhase] = useState<
    "voltage" | "frequency" | "stability" | "output" | "sync" | "complete" | null
  >(null);
  const [localShutdownPhase, setLocalShutdownPhase] = useState<
    "draining" | "releasing" | "halted" | null
  >(null);
  const [localTestResult, setLocalTestResult] = useState<"pass" | "fail" | null>(null);
  const [localStatusMessage, setLocalStatusMessage] = useState("Initializing...");
  const [localIsPowered, setLocalIsPowered] = useState(true);
  ...
  const handleTest = async () => {
    ...
    if (uecManager) {
      ...
      const phases: ("voltage" | "frequency" | "stability" | "output" | "sync" | "complete")[] = [
        "voltage",
        "frequency",
        "stability",
        "output",
    ...
    >
        ...
        <div className="p-1.5 px-2">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              ...
              style={{
                background:
                  isPowered && deviceState !== "standby" ? "rgba(255,184,0,0.15)" : "#0a0a0f",
                color:
                  isPowered && deviceState !== "standby"
                    ? "var(--neon-amber)"
                    : "rgba(255,255,255,0.4)",
      ...
      >
        ...
        <div className="absolute top-1.5 right-1.5 z-10 flex items-center gap-1">
          ...
          >
            <div
              className="relative flex h-3 w-3 items-center justify-center transition-all"
              style={{
                ...
                background:
                  isPowered && deviceState !== "standby"
                    ...
                    : deviceState === "standby"
                      ? "linear-gradient(180deg, #4a3a0a 0%, #2a2a2a 100%)"
                      : "#1a1a1a",
                boxShadow:
                  isPowered && deviceState !== "standby"
                ...
                style={{
                  background:
                    isPowered && deviceState !== "standby"
                      ...
                      : deviceState === "standby"
                        ? "radial-gradient(circle, rgba(255,180,0,0.3) 0%, transparent 100%)"
                        : "#333",
                }}
              />
            </div>
        ...
        <div className="mb-1.5 flex items-center gap-2">
          ...
          <div
            className={cn(
              "relative overflow-hidden rounded border border-white/5 bg-black/40 px-2 py-1",
              deviceState === "testing" &&
                testPhase === "output" &&
                "ring-1 ring-[var(--neon-amber)]/50",
            )}
          >
            <div
              className={cn(
                "font-mono text-lg font-bold tabular-nums transition-all duration-300",
                (deviceState === "booting" && bootPhaseNum < 3) || deviceState === "standby"
                  ? "opacity-50"
                  : "",
              )}
              style={{
                ...
                textShadow:
                  displayValues.tier !== "--"
                    ? `0 0 8px ${volatilityTier >= 4 ? "var(--neon-red)" : "var(--neon-amber)"}`
                    : "none",
              }}
            >
              {displayValues.tier}
          ...
          <div className="flex-1">
            <div
              className={cn(
                "relative h-3 overflow-hidden rounded border border-white/5 bg-black/40",
                deviceState === "testing" &&
                  testPhase === "stability" &&
        ...
        <div className="mt-1 flex items-center gap-1 border-t border-white/5 pt-1">
          ...
          <div className="ml-0.5 flex gap-0.5">
            <div
              className={cn(
                "h-1 w-1 rounded-full transition-colors",
                deviceState === "online" && volatilityTier >= 1
                  ? "bg-[var(--neon-amber)]"
                  : "bg-white/20",
              )}
            />
            <div
              className={cn(
                "h-1 w-1 rounded-full transition-colors",
                deviceState === "online" && volatilityTier >= 3
                  ? "bg-[var(--neon-orange)]"
                  : "bg-white/20",
              )}
            />
            <div
              className={cn(
                "h-1 w-1 rounded-full transition-colors",
                deviceState === "online" && volatilityTier >= 5
                  ? "bg-[var(--neon-red)]"
                  : "bg-white/20",
              )}
            />
...
export function BatteryPack({
  ...
  const bootPhase = batManager
    ? bootPhaseFromContext === "init"
      ? 1
      : bootPhaseFromContext === "cells"
        ? 2
        : bootPhaseFromContext === "calibrate"
          ? 3
          : bootPhaseFromContext === "handshake"
            ? 4
            : bootPhaseFromContext === "ready"
              ? 6
              : 0
    : localBootPhase;
  ...
  const handleTest = async () => {
    ...
    if (batManager) {
      ...
      const phases: NonNullable<BatteryTestPhase>[] = [
        "cells",
        "voltage",
        "current",
        "capacity",
  ...
  const isLedOn =
    deviceState !== "standby" &&
    deviceState !== "shutdown" &&
    !(deviceState === "rebooting" && bootPhase === 0);
  ...
  const milBtnClass = (active: boolean, color: string) =>
    cn(
    ...
    >
      ...
      >
        <div className="p-1.5 px-2">
          ...
          <div className="flex items-center gap-1.5">
            <LED on={isLedOn} color={getLedColor()} size="sm" />
            <span className="font-mono text-[8px] font-bold tracking-wider text-[var(--neon-lime,#bfff00)]">
              BAT-001
            </span>
            <span
              className={cn(
            ...
            <button
              onClick={handlePowerToggle}
              disabled={isTransitioning}
              className="flex h-3 w-3.5 shrink-0 items-center justify-center border border-dashed font-mono text-[5px] transition-all"
              style={{
                background:
                  isPowered && deviceState !== "standby" ? "rgba(191,255,0,0.1)" : "#0a0a0f",
                color:
                  isPowered && deviceState !== "standby"
            ...
            {isPowered && deviceState !== "standby" ? (
              <button
                ...
                className="flex h-3 w-3.5 shrink-0 items-center justify-center border border-dashed border-[var(--neon-lime,#bfff00)]/30 bg-[#0a0a0f] font-mono text-[6px] text-[var(--neon-lime,#bfff00)]/60 transition-all hover:border-[var(--neon-lime,#bfff00)]/60"
                title={showFoldedInfo ? "Unfold" : "More info"}
              >
                {showFoldedInfo ? "▲" : "▼"}
              </button>
          ...
          >
            <div className="mt-1 grid grid-cols-3 gap-2 border-t border-white/5 pt-1">
              ...
              <div className="font-mono text-[6px]">
                ...
                <span className="text-[var(--neon-lime,#bfff00)]">
                  {batManager?.chargePercent ?? chargePercent}%
                </span>
              </div>
            </div>
            <div className="mt-0.5 grid grid-cols-3 gap-2">
      ...
      >
        ...
        <div className="absolute top-1.5 right-1.5 z-10 flex items-center gap-1">
          <button onClick={handleToggleExpand} className="group" title="Fold">
            <div
              className="flex h-2.5 w-2.5 items-center justify-center rounded-sm border border-dashed transition-all"
              style={{
                background: "#0a0a0f",
                borderColor: "rgba(191,255,0,0.2)",
              }}
            >
              <span className="font-mono text-[5px] text-[var(--neon-lime,#bfff00)]/40 transition-colors group-hover:text-[var(--neon-lime,#bfff00)]/70">
                ▲
              </span>
            </div>
          </button>
          ...
          >
            <div
              className="relative flex h-4 w-2.5 flex-col items-center justify-center gap-[1px] rounded-sm border transition-all"
              style={{
                background:
                  isPowered && deviceState !== "standby"
                    ? "linear-gradient(180deg, #1a3a1a 0%, #0a1f0a 100%)"
                    : deviceState === "standby"
                      ? "linear-gradient(180deg, #2a2a1a 0%, #1a1a0a 100%)"
                      : "#0a0a0a",
                borderColor:
                  isPowered && deviceState !== "standby"
                    ? "var(--neon-lime, #bfff00)"
            ...
            >
              <div
                className="absolute -top-[2px] h-[2px] w-1 rounded-t-sm"
                style={{
                  background:
                    isPowered && deviceState !== "standby"
                      ? "var(--neon-lime, #bfff00)"
              ...
              {[0, 1, 2].map((i) => (
                <div
                  key={i}
                  className="h-[3px] w-1.5 rounded-[1px] transition-all"
                  style={{
                    background:
                      isPowered && deviceState !== "standby"
                        ? i === 0
                        ...
                        : "#2a2a2a",
                    boxShadow:
                      isPowered && deviceState !== "standby" && i < 2
        ...
        <div className="mb-1 flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            <LED on={isLedOn} color={getLedColor()} size="sm" />
            <div>
              <div className="font-mono text-[8px] tracking-wider text-[var(--neon-lime,#bfff00)] uppercase">
          ...
          <div className="mr-8 font-mono text-[5px] text-white/20">BAT-001</div>
        </div>
        ...
        <div className="mb-1.5 flex items-center justify-between">
          ...
          <div
            className={cn(
              ...
              deviceState === "testing" &&
                testPhase === "cells" &&
                "rounded p-0.5 ring-1 ring-[var(--neon-cyan)]/50",
            )}
          >
          ...
          <div
            className={cn(
              ...
              deviceState === "testing" &&
                testPhase === "voltage" &&
                "rounded px-1 ring-1 ring-[var(--neon-cyan)]/50",
            )}
          >
            <div
              className={cn(
              ...
              style={{
                color: "var(--neon-green)",
                textShadow:
                  displayValues.available !== "--" && displayValues.available !== "0"
        ...
        <div className="mt-1 flex items-center gap-1 border-t border-white/5 pt-1">
          ...
          <button
            ...
            disabled={deviceState !== "online"}
            className={cn(
              "flex h-3 w-3.5 items-center justify-center border border-dashed font-mono text-[5px] transition-all",
              deviceState === "testing"
                ...
                : testResult === "pass"
                  ...
                  : testResult === "fail"
                    ? "border-[var(--neon-red)] bg-[var(--neon-red)]/10 text-[var(--neon-red)]"
                    : "border-[var(--neon-lime,#bfff00)]/30 bg-[#0a0a0f] text-[var(--neon-lime,#bfff00)]/60 hover:border-[var(--neon-lime,#bfff00)]/60 disabled:opacity-30",
            )}
            title="Test"
          >
            {deviceState === "testing"
          ...
          <button
            ...
            className={cn(
              "flex h-3 w-3.5 items-center justify-center border border-dashed font-mono text-[5px] transition-all",
              deviceState === "rebooting" || deviceState === "booting"
                ? "border-[var(--neon-amber)] bg-[var(--neon-amber)]/10 text-[var(--neon-amber)]"
                : "border-[var(--neon-lime,#bfff00)]/30 bg-[#0a0a0f] text-[var(--neon-lime,#bfff00)]/60 hover:border-[var(--neon-lime,#bfff00)]/60 disabled:opacity-30",
            )}
            title="Cycle"
          >
          ...
          <div className="ml-0.5 flex gap-0.5">
            <div
              className={cn(
                "h-1 w-1 rounded-full transition-colors",
                deviceState === "online" && available > 0
                  ? "bg-[var(--neon-green)]"
                  : "bg-white/20",
              )}
            />
...
export function TechTreeModule({ progress, category, className }: TechTreeModuleProps) {
  ...
  return (
    ...
    >
      <div className="space-y-2">
        ...
        {/* Experience progress */}
        {tier < maxTier && (
          <div className="h-1 overflow-hidden rounded bg-black/30">
            <div
        ...
        {/* Capability */}
        <div className="font-mono text-[9px] text-white/60">{capability}</div>
      </div>
    </EquipmentTile>
  );
}
...
export function HandmadeSynthesizer({
  ...
  const [localBootPhase, setLocalBootPhase] = useState(0);
  const [localTestPhase, setLocalTestPhase] = useState<
    "oscillator" | "waveform" | "filter" | "output" | "calibrate" | "complete" | null
  >(null);
  const [localTestResult, setLocalTestResult] = useState<"pass" | "fail" | null>(null);
  const [localStatusMessage, setLocalStatusMessage] = useState("Initializing...");
  ...
  const handleTest = async () => {
    ...
    if (hmsManager) {
      ...
      const phases: ("oscillator" | "waveform" | "filter" | "output" | "calibrate" | "complete")[] =
        ["oscillator", "waveform", "filter", "output", "calibrate", "complete"];
      const phaseMessages = {
  ...
  const isLedOn =
    deviceState !== "standby" &&
    deviceState !== "shutdown" &&
    !(deviceState === "rebooting" && bootPhaseNum === 0);
  ...
  const WaveformPowerButton = ({ small }: { small?: boolean }) => {
    ...
    const isTransBtn =
      deviceState === "booting" || deviceState === "shutdown" || deviceState === "rebooting";
    ...
    return (
      ...
      >
        ...
        <div
          className={cn(
            "absolute -bottom-0.5 left-1/2 h-1 w-1 -translate-x-1/2 rounded-full transition-all",
    ...
    >
      ...
      >
        <div className="p-1.5 px-2">
          <div className="flex items-center gap-1.5">
            <LED on={isLedOn} color={getLedColor()} size="sm" />
            <span className="font-mono text-[8px] font-bold tracking-wider text-[var(--neon-cyan)]">
              HMS-001
            </span>
            <span
              className={cn(
      ...
      >
        ...
        <div className="mb-1 flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            <LED on={isLedOn} color={getLedColor()} size="sm" />
            <div>
              <div className="font-mono text-[9px] tracking-wider text-[var(--neon-cyan)] uppercase">
          ...
          <div className="flex items-center gap-1.5">
              ...
              <div
                className={cn(
                  ...
                  deviceState === "testing"
                    ...
                    : testResult === "pass"
                      ? "border-[var(--neon-green)] bg-[var(--neon-green)] shadow-[0_0_6px_var(--neon-green)]"
                      : testResult === "fail"
                        ? "border-[var(--neon-red)] bg-[var(--neon-red)] shadow-[0_0_6px_var(--neon-red)]"
                        : "border-[var(--neon-cyan)]/30 bg-[#0a0a0f] group-hover:border-[var(--neon-cyan)]/60",
                )}
              />
              <span className="absolute -bottom-2 left-1/2 -translate-x-1/2 font-mono text-[4px] text-white/30">
                T
              </span>
            </button>
            ...
            >
              <div
                className={cn(
                  "h-2.5 w-2.5 rounded-full border transition-all",
                  deviceState === "rebooting" || deviceState === "booting"
                    ? "border-[var(--neon-amber)] bg-[var(--neon-amber)] shadow-[0_0_6px_var(--neon-amber)]"
                    : "border-[var(--neon-amber)]/30 bg-[#0a0a0f] group-hover:border-[var(--neon-amber)]/60",
                )}
              />
              <span className="absolute -bottom-2 left-1/2 -translate-x-1/2 font-mono text-[4px] text-white/30">
                R
            ...
            <div className="ml-0.5 font-mono text-[5px] text-white/20">HMS-001</div>
          </div>
        </div>
        ...
        <div
          className={cn(
            ...
            deviceState === "standby" || deviceState === "shutdown"
              ? "opacity-30"
              : deviceState === "booting" && bootPhaseNum < 3
                ? "opacity-50"
                : "opacity-100",
          )}
        >
          <div
            className={cn(
              "transition-all",
              deviceState === "testing" &&
                testPhase === "oscillator" &&
                "rounded-full ring-1 ring-[var(--neon-cyan)]/50",
            )}
          >
            <Knob
        ...
        <div className="mt-1.5 flex items-center justify-between border-t border-white/5 pt-1">
          <div className="flex items-center gap-1">
            <div
              className={cn(
                "rounded px-1 font-mono text-[5px]",
                deviceState === "online"
            ...
            <span
              className={cn(
                "truncate font-mono text-[6px] transition-colors",
                deviceState === "testing"
                  ? "text-[var(--neon-cyan)]"
                  : deviceState === "rebooting" || deviceState === "booting"
                    ? "text-[var(--neon-amber)]"
          ...
          <div className="flex gap-0.5">
            {[1, 2, 3, 4, 5].map((t) => (
              <div
                ...
                className={cn(
                  "h-1 w-1 rounded-full transition-colors",
                  deviceState !== "standby" && deviceState !== "shutdown" && tier >= t
                    ? "bg-[var(--neon-cyan)]"
                    : "bg-white/10",
                )}
              />
...
export function EchoRecorder({
  ...
  const [localBootPhase, setLocalBootPhase] = useState(0);
  const [localTestPhase, setLocalTestPhase] = useState<
    "antenna" | "decoder" | "buffer" | "sync" | "output" | "complete" | null
  >(null);
  const [localTestResult, setLocalTestResult] = useState<"pass" | "fail" | null>(null);
  const [localStatusMessage, setLocalStatusMessage] = useState("Initializing...");
  ...
  const handleTest = async () => {
    ...
    if (ecrManager) {
      ...
      const phases: ("antenna" | "decoder" | "buffer" | "sync" | "output" | "complete")[] = [
        "antenna",
        "decoder",
        "buffer",
        "sync",
  ...
  const SignalPowerButton = () => {
    ...
    return (
      <button
        ...
        disabled={!canToggle}
        className={cn(
          "relative h-5 w-5 rounded transition-all duration-300",
          canToggle ? "cursor-pointer hover:scale-110" : "cursor-not-allowed opacity-50",
        ...
        style={{
          ...
          boxShadow: isOn
            ? "0 0 6px rgba(255,170,0,0.4), inset 0 0 3px rgba(255,170,0,0.1)"
            : "none",
        }}
      >
  ...
  const nanoBtn = (
    ...
    >
      <div
        className={cn(
          "flex h-3 w-3 items-center justify-center rounded-sm border-2 transition-all",
    ...
    >
      ...
      >
        <div className="p-2">
          <div className="flex items-center gap-1.5">
            <LED on={isLedOn} color={getLedColor()} size="sm" />
            <span className="font-mono text-[8px] font-bold text-[var(--neon-amber)]">ECR-001</span>
            <span
              className={cn(
                "ml-1 font-mono text-[7px]",
            ...
            <button
              ...
              className={cn(
                "flex h-3 w-3 items-center justify-center rounded-sm transition-all",
                "border border-[#3a3a4a] bg-gradient-to-b from-[#2a2a3a] to-[#1a1a2a]",
                isPowered
                  ? "cursor-pointer hover:border-[var(--neon-amber)]/50"
          ...
          >
            <div className="mt-1.5 grid grid-cols-3 gap-x-3 gap-y-0.5 border-t border-white/5 pt-1">
              {[
      ...
      >
        <div className="p-2">
          ...
          <button
            ...
            className="absolute top-1 right-1 z-10 flex h-3 w-3 cursor-pointer items-center justify-center rounded-sm border border-[#3a3a4a] bg-gradient-to-b from-[#2a2a3a] to-[#1a1a2a] transition-all hover:border-[var(--neon-amber)]/50"
            title="Fold"
          >
          ...
          <div className="mb-1 flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              <LED on={isLedOn} color={getLedColor()} size="sm" />
              <div className="font-mono text-[9px] text-[var(--neon-amber)]">ECHO RECORDER</div>
            </div>
          ...
          <div
            className={cn(
              "mb-1.5 flex items-center justify-between font-mono text-[7px] text-white/40 transition-opacity",
              deviceState === "standby" || deviceState === "shutdown"
                ? "opacity-50"
                : deviceState === "booting" && bootPhaseNum < 4
                  ? "opacity-50"
                  : "opacity-100",
            )}
          >
          ...
          <div
            className={cn(
              ...
              deviceState === "standby" || deviceState === "shutdown"
                ? "opacity-30"
                : deviceState === "booting" && bootPhaseNum < 2
                  ? "opacity-50"
                  : "opacity-100",
            )}
          >
              ...
              <div
                className={cn(
                  ...
                  deviceState === "online" && (isRecording || tier > 0)
                    ? "animate-pulse bg-[var(--neon-amber)] shadow-[0_0_6px_var(--neon-amber)]"
                    : "bg-white/10",
                )}
              />
            </div>
          ...
          <div className="mt-1.5 flex items-center justify-between border-t border-white/5 pt-1">
            <span
              className={cn(
                "truncate font-mono text-[5px] transition-colors",
                deviceState === "testing"
                  ? "text-[var(--neon-cyan)]"
                  : deviceState === "rebooting" || deviceState === "booting"
                    ? "text-[var(--neon-amber)]"
...
export function Interpolator({
  ...
  const ToggleSwitch = ({
    ...
    >
      <div
        className={cn(
          "relative h-2 w-4 rounded-full transition-all",
      ...
      >
        ...
        >
          <div
            className={cn("h-0.5 w-0.5 rounded-full transition-all", ledColor || "bg-white/30")}
          />
        </div>
      </div>
    </button>
  ...
  const chevronBtn = (onClick: () => void, label: string, title: string) => (
    <button
      onClick={onClick}
      className="flex h-3 w-3 cursor-pointer items-center justify-center rounded-sm border border-[#3a3a4a] bg-gradient-to-b from-[#2a2a3a] to-[#1a1a2a] transition-all hover:border-[var(--neon-lime,#bfff00)]/50"
      title={title}
    >
    ...
    >
      ...
      >
        <div className="p-2">
          <div className="flex items-center gap-1.5">
            <LED on={isLedOn} color={getLedColor()} size="sm" />
            <span className="font-mono text-[8px] font-bold text-[var(--neon-lime,#bfff00)]">
              INT-001
            </span>
            <span
              className={cn(
            ...
            {isPowered && (
              <>
                <ToggleSwitch
                  ...
                  disabled={deviceState !== "online"}
                  title="Test"
                  isActive={!!(deviceState === "testing" || testResult)}
                  ledColor={testLedColor}
                />
                <ToggleSwitch
                  onClick={handleReboot}
                  disabled={
                  ...
                  title="Reset"
                  isActive={deviceState === "rebooting" || deviceState === "booting"}
                  ledColor={rebootLedColor}
                />
              </>
            )}
            <PrismPowerButton />
          ...
          >
            <div className="mt-1.5 grid grid-cols-3 gap-x-3 gap-y-0.5 border-t border-white/5 pt-1">
              {[
                ...
                <div key={label} className="flex justify-between">
                  <span className="font-mono text-[6px] text-white/30">{label}</span>
                  <span className="font-mono text-[6px] text-[var(--neon-lime,#bfff00)]">
                    {val}
                  </span>
                </div>
              ))}
      ...
      >
        <div className="p-2">
          ...
          <div
            className={cn(
              "mb-1 font-mono text-[7px] text-white/40 transition-opacity",
          ...
          <div
            className={cn(
              "relative flex h-6 items-center justify-center overflow-hidden rounded bg-black/30",
              deviceState === "testing" &&
                testPhase === "spectrum" &&
                "ring-1 ring-[var(--neon-cyan)]/50",
            )}
  ...
  const tools = [
    {
      ...
      color: "var(--neon-cyan)",
      testPhase: "probe" as const,
    },
    {
      name: "CLAMP",
      active: deviceState === "online",
      color: "var(--neon-green)",
      testPhase: "clamp" as const,
    },
    {
      name: "LASER",
      active: deviceState === "online" && selectedTool === "LASER",
      color: "var(--neon-red)",
      testPhase: "laser" as const,
    },
    {
      name: "DRILL",
      active: deviceState === "online",
      color: "var(--neon-amber)",
      testPhase: "drill" as const,
    },
  ];
      ...
      >
        ...
        <div className="flex items-center gap-1.5 px-2 py-1.5">
          ...
          <span
            className={cn(
              "font-mono text-[7px]",
              isPowered ? "text-[var(--neon-amber)]/70" : "text-white/30",
            )}
          >
            {stateLabel}
          </span>
          ...
          {isPowered && (
            <>
              {nanoBtn(
                ...
                "Test",
                "#3a3a4a",
                deviceState === "testing" ? "var(--neon-purple)" : undefined,
              )}
              {nanoBtn(
                reboot,
                deviceState === "booting" || deviceState === "rebooting",
                "Reboot",
                "#3a3a4a",
                deviceState === "rebooting" ? "var(--neon-amber)" : undefined,
              )}
            </>
          )}
          {/* Power rocker */}
          ...
          {isPowered ? (
            <button
              ...
              className="flex h-3 w-3 cursor-pointer items-center justify-center rounded-full border border-white/20 transition-colors hover:border-[var(--neon-amber)]/40"
              style={{ background: "radial-gradient(circle at 30% 30%, #2a2a3a 0%, #0a0a0f 70%)" }}
              title={showFoldedInfo ? "Hide info" : "Show info"}
            >
              <span className="font-mono text-[6px] text-[var(--neon-amber)]/60">
                {showFoldedInfo ? "▲" : "▼"}
              </span>
            </button>
          ) : (
        ...
        >
          <div className="flex flex-wrap gap-3 px-2 pb-1.5 font-mono text-[7px] text-[var(--neon-amber)]/60">
            {tools.map((t) => (
              <span key={t.name} className="flex items-center gap-0.5">
                <span
                  className="inline-block h-1 w-1 rounded-full"
                  style={{ backgroundColor: t.active ? t.color : "#333" }}
                />
                {t.name}
              </span>
            ))}
      ...
      >
        ...
        <div className="absolute top-1 right-1 z-10 flex gap-1">
          ...
          <button onClick={handleFoldToggle} className="group relative" title="Fold">
            <div
              className="flex h-3 w-3 items-center justify-center rounded-full border border-white/20 transition-all hover:border-[var(--neon-amber)]/40"
              style={{ background: "radial-gradient(circle at 30% 30%, #2a2a3a 0%, #0a0a0f 70%)" }}
            >
        ...
        <div
          className="pointer-events-none absolute z-0 font-mono text-[5px] text-white/20"
          style={logoPosition}
        >
          HNDX
        </div>
        ...
        <div className="mb-2 flex items-center justify-between">
          <div className="flex items-center gap-1">
            ...
            />
            <div className="font-mono text-[9px] text-[var(--neon-amber)]">BASIC TOOLKIT</div>
          </div>
        ...
        <div className="relative grid grid-cols-2 gap-1">
          {deviceState === "booting" && bootPhase && (
            <div className="absolute inset-0 z-10 flex items-center justify-center rounded bg-black/90">
                ...
                <div className="flex gap-0.5">
                  {["post", "tools", "interface", "ready"].map((phase, i) => (
                    <div
                      ...
                      style={{
                        backgroundColor:
                          ["post", "tools", "interface", "ready"].indexOf(bootPhase) >= i
                            ? "var(--neon-cyan)"
                            : "#333",
          ...
          {deviceState === "rebooting" && (
            <div className="absolute inset-0 z-10 flex items-center justify-center rounded bg-black/90">
              <span className="animate-pulse font-mono text-[6px] text-[var(--neon-amber)]">
                REBOOTING...
              </span>
            </div>
          )}
          ...
          {tools.map((tool) => {
            ...
            return (
              <button
                ...
                disabled={deviceState !== "online"}
                className={cn(
                  "flex items-center gap-1 rounded px-1 py-0.5 transition-all",
                  isSelected ? "border border-white/20 bg-white/10" : "bg-black/30",
...
export function MaterialScanner({
  ...
  useEffect(() => {
    if (deviceState === "online" && foundMaterials.length === 0) {
      setFoundMaterials(Array.from({ length: detectedMaterials }, (_, i) => 20 + i * 25));
    }
  ...
  const thinBtn = (
    ...
    <button onClick={onClick} disabled={disabled} className="group relative" title={title}>
      <div
        className="h-1.5 w-4 rounded-sm border transition-all"
        style={{
          background: "linear-gradient(180deg, #2a2a3a 0%, #1a1a2a 100%)",
          borderColor: isActive ? activeColor : "#3a3a4a",
          boxShadow: isActive
            ? `0 0 4px ${activeColor}, inset 0 0 2px ${activeColor}`
      ...
      <div
        className={cn(
          ...
          edgePos === "top" ? "top-0 rounded-t-sm" : "bottom-0 rounded-b-sm",
        )}
        style={{
    ...
    >
      ...
      >
        <div className="flex items-center gap-1.5 px-2 py-1.5">
          ...
          <span
            className={cn(
              "font-mono text-[7px]",
              isPowered ? "text-[var(--neon-cyan)]/70" : "text-white/30",
            )}
          >
            {stateLabel}
          </span>
          ...
          >
            <div
              className="h-[5px] w-[5px] rounded-full border transition-all"
              style={{
                background:
                  deviceState === "standby"
          ...
          {isPowered ? (
            ...
            >
              <div
                className="flex h-1.5 w-4 items-center justify-center rounded-sm border border-white/20"
                style={{ background: "linear-gradient(180deg, #2a2a3a 0%, #1a1a2a 100%)" }}
              >
                <span className="font-mono text-[5px] text-[var(--neon-cyan)]/60">
                  {showFoldedInfo ? "▲" : "▼"}
                </span>
              </div>
            </button>
        ...
        >
          <div className="flex flex-wrap gap-3 px-2 pb-1.5 font-mono text-[7px] text-[var(--neon-cyan)]/60">
      ...
      >
        ...
        <button
          onClick={handlePower}
          disabled={!manager || (deviceState !== "online" && deviceState !== "standby")}
          className="group absolute bottom-1 left-1 z-30"
          title={deviceState === "standby" ? "Power On" : "Power Off"}
        >
          <div
            className="h-[5px] w-[5px] rounded-full border transition-all"
        ...
        <div className="absolute top-1 right-1 z-10 flex flex-col gap-px">
          {/* Fold chevron */}
          <button onClick={handleFoldToggle} className="group relative" title="Fold">
            <div
              className="flex h-1.5 w-4 items-center justify-center rounded-sm border border-white/20"
        ...
        {/* Company logo */}
        <div
          className="pointer-events-none absolute z-0 font-mono text-[4px] text-white/15"
          style={logoPosition}
        >
          SCNR
        </div>
        ...
        <div className="relative mb-1 h-6 overflow-hidden rounded bg-black/40">
          {deviceState === "booting" && bootPhase && (
                ...
                <div className="flex gap-0.5">
                  {(["post", "sensors", "calibrate", "ready"] as const).map((phase, i) => (
                    <div
                      ...
                      style={{
                        backgroundColor:
                          (["post", "sensors", "calibrate", "ready"] as const).indexOf(
                            bootPhase!,
                          ) >= i
                            ? "var(--neon-cyan)"
                            : "#333",
                    ...
                    <div
                      ...
                      style={{
                        backgroundColor:
                          testPhase === phase
                            ? "var(--neon-purple)"
                            : (["emitter", "receiver", "calibrate", "sweep"] as const).indexOf(
                                  phase,
                                ) <
                                (["emitter", "receiver", "calibrate", "sweep"] as const).indexOf(
                                  testPhase as "emitter" | "receiver" | "calibrate" | "sweep",
                                )
                              ? "var(--neon-green)"
                              : "#333",
          ...
          {deviceState === "online" &&
            foundMaterials.map((pos, i) => (
              <div
                key={i}
                className="absolute h-1 w-1 rounded-full bg-[var(--neon-green)] transition-opacity"
                style={{
                  left: `${pos}%`,
                  top: "50%",
...
export function ResourceMagnet({
  ...
  const fieldActive =
    (rmg?.fieldActive ?? true) && (deviceState === "online" || deviceState === "testing");
  ...
  const getLedColor = () => {
    if (deviceState === "standby" || deviceState === "shutdown" || deviceState === "rebooting")
      return "red";
    if (deviceState === "booting") return "amber";
    if (deviceState === "testing") return "cyan";
    if (testResult === "pass") return "green";
  ...
  const isLedOn =
    deviceState !== "standby" && deviceState !== "shutdown" && !(deviceState === "rebooting");
  const stateLabel =
    deviceState === "online"
      ? "ONLINE"
      : deviceState === "testing"
  ...
  const chromeBtn = (
    ...
    >
      <div
        className={cn(
          "flex h-2 w-3 items-center justify-center rounded-[2px] transition-all group-active:scale-95",
        )}
        style={{
          background:
            style === "chrome"
      ...
      >
        ...
        />
        <div
          className={cn("z-10 h-1 w-1 rounded-full transition-all")}
          style={{
  ...
  const copperPowerBtn = (
    ...
    >
      <div
        className="flex items-center justify-center group-active:scale-95"
        style={{
          width: "3px",
          height: "5px",
      ...
      >
        <div className="flex items-center gap-1.5 px-2 py-1.5">
          ...
          <span
            className={cn(
              "font-mono text-[7px]",
              isPowered ? "text-[var(--neon-lime,#bfff00)]/70" : "text-white/30",
            )}
          >
            {stateLabel}
          </span>
          ...
          {isPowered ? (
            ...
            >
              <div
                className="flex h-2 w-3 items-center justify-center rounded-[2px]"
                style={{
                  background: "linear-gradient(180deg, #e8e8e8 0%, #a0a0a0 50%, #606060 100%)",
                  border: "0.5px solid #707070",
                }}
              >
                <span className="relative z-10 font-mono text-[6px] text-[#333]">
                  {showFoldedInfo ? "▲" : "▼"}
                </span>
              </div>
            </button>
        ...
        >
          <div className="flex flex-wrap gap-3 px-2 pb-1.5 font-mono text-[7px] text-[var(--neon-lime,#bfff00)]/60">
      ...
      >
        ...
        <div className="mb-1 flex items-center justify-between">
          ...
          <div className="flex items-center gap-0.5">
            ...
            <button onClick={handleFoldToggle} className="group relative" title="Fold">
              <div
                className="flex h-2 w-3 items-center justify-center rounded-[2px] group-active:scale-95"
                style={{
                  background: "linear-gradient(180deg, #e8e8e8 0%, #a0a0a0 50%, #606060 100%)",
                  boxShadow:
                    "inset 0 1px 0 rgba(255,255,255,0.6), inset 0 -1px 0 rgba(0,0,0,0.3), 0 1px 2px rgba(0,0,0,0.4)",
              ...
              >
                <span className="relative z-10 font-mono text-[6px] text-[#333]">▴</span>
              </div>
            </button>
            <div
              className="mr-0.5 px-0.5 font-mono text-[3px] text-white/40"
              style={{
            ...
            {chromeBtn(
              handleReboot,
              deviceState === "booting" || deviceState === "rebooting" || deviceState === "testing",
              "Reboot",
              "brass",
              deviceState === "rebooting" || deviceState === "booting"
                ? "var(--neon-amber)"
        ...
        <div
          className={cn(
            "relative flex h-8 items-center justify-center overflow-hidden rounded bg-black/40",
            deviceState === "testing" &&
              testPhase === "field" &&
              "ring-1 ring-[var(--neon-green)]/50",
          )}
        >
          {[3, 2, 1].map((ring) => (
            <div
          ...
          {deviceState === "testing" && testPhase === "calibrate" && (
            <div className="absolute inset-0 animate-pulse bg-[var(--neon-green)]/10" />
          )}

          {(deviceState === "standby" || deviceState === "shutdown") && (
            <div className="absolute inset-0 z-10 flex items-center justify-center rounded bg-black/80">
              <span className="font-mono text-[6px] text-[var(--neon-red)]/60">
                {shutdownPhase ? shutdownPhase.toUpperCase() : "STANDBY"}
              </span>
            </div>
        ...
        <div className="mt-1 flex items-center justify-between">
          ...
          <div className="flex flex-1 flex-col items-end">
            <div className="flex items-center gap-1">
              <span
                className={cn(
                  "max-w-[40px] overflow-hidden font-mono text-[5px] text-ellipsis whitespace-nowrap transition-colors",
                  deviceState === "testing"
                    ? "text-[var(--neon-cyan)]"
                    : deviceState === "rebooting" || deviceState === "booting"
                      ? "text-[var(--neon-amber)]"
...
export function QuantumCompass({ className }: QuantumCompassProps) {
    ...
    >
      ...
      >
        <button
          ...
          className="group absolute top-1 right-1 z-10"
          title="Fold"
        >
          <div
            className="flex h-3 w-3 items-center justify-center rounded-full border border-white/20 transition-all hover:border-[var(--neon-amber)]/40"
        ...
        {/* Header with logo */}
        <div className="mb-1 flex items-center justify-between">
          <div className="font-mono text-[8px] text-[var(--neon-amber)]">QUANTUM COMPASS</div>
          <div className="flex items-center gap-1">
            ...
            {/* QNTM logo */}
            <div
              className="px-0.5 font-mono text-[3px] leading-none text-[#ffaa00]"
              style={{
        ...
        <div
          className={cn(
            "relative flex h-12 items-center justify-center rounded bg-black/40",
            deviceState === "testing" && "ring-1 ring-[var(--neon-amber)]/30",
          )}
        >
          {/* Compass ring */}
          <div className="relative h-10 w-10 rounded-full border border-[var(--neon-amber)]/40">
            {/* Tick marks */}
          ...
          {/* Distance indicator */}
          <div className="absolute top-1 right-1.5 font-mono text-[8px] text-[var(--neon-amber)]">
          ...
          {/* Standby overlay */}
          {(deviceState === "standby" || deviceState === "shutdown") && (
            <div className="absolute inset-0 z-30 flex items-center justify-center rounded bg-black/80">
        ...
        <div className="mt-1 flex items-center justify-between font-mono text-[6px]">
          ...
          >
            <div
              className="h-3 w-3 rounded-full transition-all group-active:scale-95"
              style={{
                background:
                  deviceState === "testing"
    ...
    >
      ...
      >
        <div className="px-2 py-1.5">
          ...
          <div className="flex items-center gap-1.5">
            ...
            />
            {/* Device ID */}
            <span className="font-mono text-[8px] font-bold tracking-wide text-[var(--neon-amber)]">
              PWB-001
            </span>
            {/* Status label */}
            ...
            {isPowered && (
              <>
                ...
                >
                  <div
                    className={cn("border transition-all", getButtonClass(), getButtonSize())}
                    style={{
                      background: "radial-gradient(circle at 30% 30%, #2a2a3a 0%, #0a0a0f 70%)",
                      borderColor: deviceState === "testing" ? "var(--neon-purple)" : "#3a3a4a",
                      boxShadow:
            ...
            {isPowered ? (
              <button
                onClick={toggleFoldedInfo}
                className="flex-shrink-0 font-mono text-[7px] text-white/40 transition-colors hover:text-white/70"
                title={showFoldedInfo ? "Hide info" : "Show info"}
              >
                {showFoldedInfo ? "▲" : "▼"}
              </button>
              ...
              <button
                ...
                className="flex-shrink-0 font-mono text-[7px] text-white/30 transition-colors hover:text-white/50"
                title="Unfold"
              >
                ▼
              </button>
          ...
          >
            <div className="mt-1 grid grid-cols-3 gap-x-3 gap-y-0.5 font-mono text-[6px]">
              <span className="text-white/30">
                Slots: <span className="text-[var(--neon-amber)]">3</span>
              </span>
              <span className="text-white/30">
                Queue: <span className="text-[var(--neon-amber)]">{queuedItems}</span>
              </span>
              <span className="text-white/30">
                Prog: <span className="text-[var(--neon-amber)]">{craftingProgress}%</span>
              </span>
              <span className="text-white/30">
                Active:{" "}
                <span className="text-[var(--neon-cyan)]">
                  {activeSlot !== null ? `S${activeSlot}` : "--"}
                </span>
              </span>
              <span className="text-white/30">
                Draw: <span className="text-[var(--neon-amber)]">{currentDraw.toFixed(1)}</span>
              </span>
              <span className="text-white/30">
                Tier: <span className="text-white/50">T1</span>
              </span>
            </div>
          </div>
        </div>
      ...
      >
        ...
        <button
          ...
          className="absolute top-0.5 right-1 z-20 font-mono text-[7px] text-white/30 transition-colors hover:text-white/60"
          title="Fold"
        >
          ▴
        </button>
        ...
        <div className="p-2">
          ...
          <div className="absolute top-1/2 left-1 z-10 flex -translate-y-1/2 flex-col gap-1.5">
            {/* Test button */}
            <button
              onClick={runTest}
              disabled={deviceState !== "online"}
            ...
            >
              ...
              />
              <div
                className={cn("pointer-events-none absolute inset-0", getButtonClass())}
                style={{
                  border: "1px solid",
                  borderColor: deviceState === "online" ? "var(--neon-cyan)" : "transparent",
                  opacity: deviceState === "online" ? 0.5 : 0,
            ...
            >
              ...
              <div
                ...
                style={{
                  ...
                  animation:
                    deviceState === "rebooting" ? "pulse 0.3s ease-in-out infinite" : "none",
                }}
              />
            </button>
            {/* Power micro-toggle */}
          ...
          <div
            className="pointer-events-none absolute z-0 font-mono text-[5px] tracking-wider text-white/15"
            style={logoPosition}
          >
            FABX
          </div>
          ...
          <div className="mb-1 ml-5 flex items-center justify-between">
            <div className="flex items-center gap-1">
              <div
                className="h-1.5 w-1.5 rounded-full"
                style={{
          ...
          <div className="relative mb-1 ml-5 flex gap-1">
                  ...
                  <div className="flex gap-0.5">
                    {["post", "firmware", "calibration", "tools", "ready"].map((phase, i) => (
                      <div
                        ...
                        style={{
                          backgroundColor:
                            ["post", "firmware", "calibration", "tools", "ready"].indexOf(
                              bootPhase,
                            ) >= i
                              ? "var(--neon-cyan)"
                              : "#333",
                          ...
                          backgroundColor:
                            testPhase === phase
                              ...
                              : ["motors", "clamps", "sensors", "calibrate"].indexOf(phase) <
                                  ["motors", "clamps", "sensors", "calibrate"].indexOf(
                                    testPhase || "motors",
                                  )
                                ? "var(--neon-green)"
                                : "#333",
            ...
            {[0, 1, 2].map((slot) => {
              ...
              return (
                <button
                  ...
                  disabled={deviceState !== "online"}
                  className="h-4 flex-1 rounded border bg-black/40 transition-all"
                  style={{
          ...
          <div className="ml-5 h-1.5 overflow-hidden rounded bg-black/40">
            <div
              className="h-full transition-all duration-300"
              style={{
                ...
                boxShadow:
                  isCrafting && deviceState === "online" ? "0 0 4px var(--neon-amber)" : "none",
              }}
            />
          </div>

          <div className="mt-1 ml-5 flex justify-between font-mono text-[7px]">
            <span className="text-white/40">QUEUE: {queuedItems}</span>
            <span className="text-[var(--neon-amber)]">
              {deviceState === "online" ? `${craftingProgress}%` : "--"}
            </span>
          </div>
        </div>
      </div>
...
export function MicrofusionReactor({
  ...
  const coreActive =
    ...
    (deviceState === "online" ||
      deviceState === "testing" ||
      (deviceState === "booting" && bootPhase && bootPhase !== "ignition"));
    ...
    >
      <div
        className={cn(
          ...
          `border bg-[#0a0a0f] ${borderColor}`,
          "flex items-center justify-center",
          `group-hover:${hoverColor}`,
        )}
        style={{ clipPath: "polygon(50% 0%, 100% 25%, 100% 75%, 50% 100%, 0% 75%, 0% 25%)" }}
      >
        <div className={cn("h-1 w-1 rounded-full transition-all", ledColor)} />
      </div>
    </button>
  );
  ...
  const chevronBtn = (onClick: () => void, label: string, title: string, enabled = true) => (
    <button
      ...
      className={cn(
        "flex h-3 w-3 items-center justify-center rounded-sm border border-[#3a3a4a] bg-gradient-to-b from-[#2a2a3a] to-[#1a1a2a] transition-all",
        enabled ? "cursor-pointer hover:border-[var(--neon-cyan)]/50" : "cursor-default opacity-30",
      )}
      title={title}
    >
      <span className="text-[6px] text-white/50">{label}</span>
    ...
    >
      ...
      >
        <div className="p-2">
          <div className="flex items-center gap-1.5">
            <LED on={isLedOn} color={getLedColor()} size="sm" />
            <span className="font-mono text-[8px] font-bold text-[var(--neon-cyan)]">MFR-001</span>
            <span
              className={cn(
                "ml-1 font-mono text-[7px]",
                isPowered ? "text-[var(--neon-green)]" : "text-white/30",
              ...
              <>
                ...
                {hexBtn(
                  ...
                  deviceState === "booting" ||
                    ...
                    deviceState === "shutdown",
                  "Reboot",
                  "border-[var(--neon-amber)]/30",
                  "border-[var(--neon-amber)]/60",
                  rebootLedColor,
                )}
              </>
            )}
        ...
        <div className="p-2">
          ...
          <div className="mb-1 flex items-center justify-between">
            ...
            <div className="mr-4 flex items-center gap-1">
              ...
              <div className="ml-0.5 font-mono text-[6px] text-white/30">T2</div>
            </div>
          </div>
          ...
          <div
            className={cn(
              "relative flex h-12 items-center justify-center overflow-hidden rounded bg-black/40",
              deviceState === "testing" &&
                testPhase === "containment" &&
                "ring-1 ring-[var(--neon-cyan)]/50",
            )}
          >
            {[1, 2, 3].map((ring) => (
          ...
          <div className="mt-1 flex items-center border-t border-white/5 pt-1 font-mono text-[7px]">
            <span
              className={cn(
            ...
            <span
              className={cn(
                "flex-1 overflow-hidden px-0.5 text-center text-[5px] text-ellipsis whitespace-nowrap transition-colors",
                deviceState === "testing"
                  ? "text-[var(--neon-cyan)]"
                  : deviceState === "rebooting" || deviceState === "booting"
                    ? "text-[var(--neon-amber)]"
...
export function AIAssistant({ className }: AIAssistantProps) {
  ...
  const isLedOn =
    deviceState !== "standby" &&
    deviceState !== "shutdown" &&
    !(deviceState === "rebooting" && !bootPhase);
  const nodesActive =
    deviceState === "online" ||
    deviceState === "testing" ||
    (deviceState === "booting" &&
      bootPhase &&
      ["nodes", "training", "calibrate", "ready"].includes(bootPhase));
  const isTransitioning =
    deviceState === "booting" || deviceState === "shutdown" || deviceState === "rebooting";
  ...
  const NeuralPowerButton = () => (
    ...
    >
      ...
      >
        <svg viewBox="0 0 16 16" className="absolute inset-0 h-full w-full">
          {[
            { cx: 5.5, cy: 8, rx: 2.5, ry: 3 },
            { cx: 10.5, cy: 8, rx: 2.5, ry: 3 },
          ].map((lobe, i) => (
            <ellipse
              key={i}
              cx={lobe.cx}
          ...
          {[
            ...
            <circle
              ...
              style={{
                ...
                filter: isPowered
                  ? `drop-shadow(0 0 ${1 + (nodeActivity[i % 5] || 0) * 2}px var(--neon-green))`
                  : "none",
                animation:
                  isPowered && isLearning
                    ? `pulse ${0.6 + i * 0.15}s ease-in-out infinite`
                    : "none",
              }}
            />
          ))}
  ...
  const rubberBtn = (
    ...
    >
      <div
        className={cn(
          "flex h-2 w-2.5 items-center justify-center rounded-sm transition-all",
  ...
  const testLedColor =
    deviceState === "testing" || testResult === "pass"
      ? "bg-[var(--neon-green)] shadow-[0_0_3px_var(--neon-green)]"
      : testResult === "fail"
        ? "bg-[var(--neon-red)] shadow-[0_0_3px_var(--neon-red)]"
        : "bg-[#2a3a2a]";
  ...
  const chevronBtn = (onClick: () => void, label: string, title: string, enabled = true) => (
    <button
      onClick={enabled ? onClick : undefined}
      className={cn(
        "flex h-3 w-3 items-center justify-center rounded-sm border border-[#2a3a2a]/60 bg-gradient-to-b from-[#1a1f1a] to-[#0d120d] transition-all",
        enabled
          ? "cursor-pointer hover:border-[var(--neon-green)]/50"
          : "cursor-default opacity-30",
      )}
      title={title}
    >
      ...
      >
        <div className="p-2">
          <div className="flex items-center gap-1.5">
            <NeuralPowerButton />
            <LED on={isLedOn} color={getLedColor()} size="sm" />
            <span className="font-mono text-[8px] font-bold text-[var(--neon-green)]">AIC-001</span>
            <span
              className={cn(
                "ml-1 font-mono text-[7px]",
                isPowered ? "text-[var(--neon-green)]" : "text-white/30",
        ...
        <div className="p-2">
          ...
          <div className="mb-1 flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              <NeuralPowerButton />
              <LED on={isLedOn} color={getLedColor()} size="sm" />
              <div className="font-mono text-[9px] text-[var(--neon-green)]">AI ASSISTANT CORE</div>
            </div>
          ...
          <div
            className={cn(
              "relative h-10 overflow-hidden rounded bg-black/40",
              deviceState === "testing" &&
                testPhase === "neural" &&
                "ring-1 ring-[var(--neon-green)]/50",
            )}
          >
                ...
                <div key={node} className="flex flex-col items-center gap-1">
                  <div
                    className={cn(
                      ...
                      deviceState === "testing" &&
                        testPhase === "neural" &&
                        "ring-1 ring-[var(--neon-cyan)]",
                    )}
                    style={{
                      opacity: nodesActive ? 0.4 + nodeActivity[node] * 0.6 : 0.2,
                      animation:
                        nodesActive && isLearning
                          ? `pulse ${0.8 + node * 0.1}s ease-in-out infinite`
                          : "none",
                      boxShadow: nodesActive
                        ? `0 0 ${4 + nodeActivity[node] * 4}px var(--neon-green)`
                        : "none",
                    }}
                  />
                  <div
                    className={cn(
                      "h-3 w-0.5 transition-all duration-300",
            ...
            <div
              className={cn(
                "absolute right-1 bottom-1 left-1 h-0.5 overflow-hidden rounded bg-black/50",
                deviceState === "testing" &&
                  testPhase === "memory" &&
                  "ring-1 ring-[var(--neon-cyan)]/50",
              )}
            >
              <div
                ...
                style={{
                  ...
                  animation:
                    nodesActive && isLearning ? "loading 1.5s ease-in-out infinite" : "none",
                }}
              />
            </div>
            {deviceState === "testing" &&
              (testPhase === "learning" || testPhase === "optimization") && (
                <div className="absolute inset-0 animate-pulse bg-[var(--neon-green)]/10" />
              )}
          </div>

          {/* Status bar */}
          <div className="mt-1 flex items-center font-mono text-[7px]">
            <span
              className={cn(
                "w-12 shrink-0 transition-colors",
                deviceState === "booting" && bootPhase && ["neural", "memory"].includes(bootPhase)
                  ? "text-white/30"
                  : "text-white/40",
              )}
            >
...
export function ExplorerDrone({
  ...
  const stateLabel = exdStandby
    ? "STANDBY"
    : isTransitioning
      ? deviceState.toUpperCase()
      : exdManager?.isDeployed
        ? "DEPLOYED"
        : "DOCKED";
  const stateLedColor = exdStandby
    ? "#555"
    : isTransitioning
      ? "var(--neon-amber)"
      : "var(--neon-lime, #bfff00)";
      ...
      >
        <div className="flex items-center gap-1 px-1.5 py-1">
          ...
          <span
            ...
            style={{ color: "var(--neon-lime, #bfff00)" }}
          >
            EXD-001
          </span>
          <span className="shrink-0 font-mono text-[7px]" style={{ color: stateLedColor }}>
            {stateLabel}
          </span>
          <div className="flex-1" />
          {!exdStandby && (
      ...
      >
        ...
        <div className={cn("p-2 transition-opacity", exdStandby && "opacity-50")}>
          ...
          <div className="mb-1 flex items-center justify-between">
            ...
            <div className="flex items-center gap-0.5">
              <div
                className="mr-0.5 rounded-[1px] px-0.5 font-mono text-[3px] text-[#8a9a6a]"
                style={{
              ...
              >
                <div
                  className={cn(
                    "h-[7px] w-2.5 rounded-[1px] transition-all",
                    "flex items-center justify-center",
                  ...
                  style={{
                    background:
                      "linear-gradient(180deg, #4a5a3a 0%, #3a4a2a 30%, #2a3a1a 70%, #1a2a0a 100%)",
                    boxShadow:
                      "inset 0 0.5px 0 rgba(255,255,255,0.15), inset 0 -0.5px 0 rgba(0,0,0,0.4), 0 1px 1px rgba(0,0,0,0.3)",
                    border: "0.5px solid #3a4a2a",
                  }}
                >
                  <div
                    className="absolute inset-0 rounded-[1px] opacity-20"
                    style={{
                      backgroundImage:
                  ...
                  <div
                    className={cn(
                      "z-10 h-[3px] w-[3px] rounded-full transition-all",
                      deviceState === "testing"
              ...
              >
                <div
                  ...
                  style={{
                    background:
                      "linear-gradient(180deg, #5a4a3a 0%, #4a3a2a 30%, #3a2a1a 70%, #2a1a0a 100%)",
                    boxShadow:
                      "inset 0 0.5px 0 rgba(255,255,255,0.15), inset 0 -0.5px 0 rgba(0,0,0,0.4), 0 1px 1px rgba(0,0,0,0.3)",
          ...
          <div
            className={cn(
              ...
              deviceState === "testing" &&
                testPhase === "gps" &&
                "ring-1 ring-[var(--neon-lime,#bfff00)]/50",
            )}
          >
            {exdStandby && (
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div
                className={cn(
                  "h-8 w-8 rounded-full border transition-colors duration-300",
                  radarActive ? "border-white/10" : "border-white/5",
                )}
              />
              <div
                className={cn(
                  "absolute h-4 w-4 rounded-full border transition-colors duration-300",
                  radarActive ? "border-white/10" : "border-white/5",
                )}
              />
            </div>
            {radarActive && (
              <div
                className={cn(
                  "absolute h-1.5 w-1.5 rounded-full bg-[var(--neon-lime,#bfff00)]",
                  deviceState === "testing" &&
                    testPhase === "camera" &&
                    "ring-2 ring-[var(--neon-cyan)]/50",
                )}
            ...
            {deviceState === "testing" && testPhase === "motors" && (
              <div className="absolute inset-0 animate-pulse bg-[var(--neon-lime,#bfff00)]/10" />
            )}
          </div>
    ...
    >
      ...
      >
        <div className="flex items-center gap-1.5 px-2 py-1.5">
          ...
          />
          <span className="font-mono text-[8px] font-bold text-[var(--neon-magenta,#e91e8c)]">
            AND-001
          </span>
          <span
            className={cn(
              "font-mono text-[7px]",
              isPowered ? "text-[var(--neon-magenta,#e91e8c)]/70" : "text-white/30",
            )}
          >
            {stateLabel}
          </span>
          ...
          {isPowered ? (
            <button
              onClick={toggleFoldedInfo}
              className="flex h-3 w-3 cursor-pointer items-center justify-center rounded-full border border-white/20 transition-colors hover:border-[var(--neon-magenta,#e91e8c)]/40"
              style={{ background: "radial-gradient(circle at 30% 30%, #2a2a3a 0%, #0a0a0f 70%)" }}
              title={showFoldedInfo ? "Hide info" : "Show info"}
            >
              <span className="font-mono text-[6px] text-[var(--neon-magenta,#e91e8c)]/60">
                {showFoldedInfo ? "\u25B2" : "\u25BC"}
              </span>
            </button>
          ) : (
        ...
        >
          <div className="flex flex-wrap gap-3 px-2 pb-1.5 font-mono text-[7px] text-[var(--neon-magenta,#e91e8c)]/60">
      ...
      >
        ...
        >
          <div
            className="flex h-3 w-3 items-center justify-center rounded-full border border-white/20 transition-all hover:border-[var(--neon-magenta,#e91e8c)]/40"
            style={{ background: "radial-gradient(circle at 30% 30%, #2a2a3a 0%, #0a0a0f 70%)" }}
          >
            <span className="font-mono text-[6px] text-[var(--neon-magenta,#e91e8c)]/60">
              {"\u25B4"}
        ...
        {(deviceState === "standby" || deviceState === "shutdown") && (
          <div className="absolute inset-0 z-30 flex items-center justify-center rounded bg-black/80">
            <span className="font-mono text-[7px] tracking-widest text-white/40">STANDBY</span>
          </div>
        )}
        ...
        <div className="mb-1 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            ...
            <div
              className="px-0.5 font-mono text-[3px] leading-none text-[#ff66cc]"
              style={{
        ...
        >
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            {Array.from({ length: 24 }).map((_, i) => {
              ...
              return (
                <div
                  key={i}
                  className="mx-px flex-1 rounded-sm transition-all duration-100"
                  style={{
          ...
          {(deviceState === "booting" || deviceState === "rebooting") && (
            <div className="absolute inset-0 flex items-center justify-center bg-black/50">
              <span className="animate-pulse font-mono text-[6px] text-[var(--neon-magenta,#e91e8c)]">
                {statusMessage}
              </span>
            </div>
          )}
        ...
        <div className="mt-1 flex items-center justify-between font-mono text-[6px]">
          ...
          <div className="flex items-center gap-1">
            ...
            >
              <div
                className="h-2.5 w-2.5 rounded-full p-[1px] transition-all group-active:scale-95"
                style={{
                  background:
                    "linear-gradient(180deg, #3a2a3a 0%, #2a1a2a 30%, #1a0a1a 70%, #0a000a 100%)",
                  boxShadow:
                    "inset 0 -1px 2px rgba(255,100,200,0.1), inset 0 1px 1px rgba(0,0,0,0.5), 0 1px 3px rgba(0,0,0,0.6)",
            ...
            >
              <div
                ...
                style={{
                  background:
                    "linear-gradient(180deg, #3a2a2a 0%, #2a1a1a 30%, #1a0a0a 70%, #0a0000 100%)",
                  boxShadow:
                    "inset 0 -1px 2px rgba(255,100,100,0.1), inset 0 1px 1px rgba(0,0,0,0.5), 0 1px 3px rgba(0,0,0,0.6)",
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center gap-1.5 px-2 py-1.5">
          ...
          <span
            className={cn(
              "font-mono text-[7px]",
              isPowered ? "text-[var(--neon-blue)]/70" : "text-white/30",
            )}
          >
            {stateLabel}
          </span>
          ...
          {isPowered ? (
            <button
              onClick={toggleFoldedInfo}
              className="flex h-3 w-3 cursor-pointer items-center justify-center rounded-full border border-white/20 transition-colors hover:border-[var(--neon-blue)]/40"
              style={{ background: "radial-gradient(circle at 30% 30%, #2a2a3a 0%, #0a0a0f 70%)" }}
              title={showFoldedInfo ? "Hide info" : "Show info"}
            >
              <span className="font-mono text-[6px] text-[var(--neon-blue)]/60">
                {showFoldedInfo ? "▲" : "▼"}
              </span>
            </button>
          ) : (
        ...
        >
          <div className="flex flex-wrap gap-3 px-2 pb-1.5 font-mono text-[7px] text-[var(--neon-blue)]/60">
      ...
      >
        ...
        >
          <div
            className="flex h-3 w-3 items-center justify-center rounded-full border border-white/20 transition-all hover:border-[var(--neon-blue)]/40"
            style={{ background: "radial-gradient(circle at 30% 30%, #2a2a3a 0%, #0a0a0f 70%)" }}
          >
        ...
        {/* Header with logo */}
        <div className="mb-1 flex items-center justify-between">
          <div className="font-mono text-[8px] text-[var(--neon-blue)]">TELEPORT PAD</div>
          <div className="flex items-center gap-1">
            {/* WARP logo */}
            <div
              className="px-0.5 font-mono text-[3px] leading-none text-[#66aaff]"
              style={{
        ...
        >
          ...
          <div className="absolute top-1 right-1.5 font-mono text-[6px] text-white/40">
            → {lastDestination}
          </div>
          ...
          {(deviceState === "booting" || deviceState === "rebooting") && (
            <div className="absolute inset-0 flex items-center justify-center rounded bg-black/50">
              <span className="animate-pulse font-mono text-[6px] text-[var(--neon-blue)]">
                {statusMessage}
              </span>
            </div>
          )}
          ...
          {deviceState === "testing" && (
            <div
              ...
              style={{
                background:
                  "radial-gradient(circle at center, var(--neon-blue) 0%, transparent 60%)",
        ...
        <div className="mt-1 flex items-center justify-between font-mono text-[6px]">
          ...
          >
            <div
              className="h-2.5 w-2.5 rounded-sm transition-all group-active:scale-95"
              style={{
                background:
                  deviceState === "testing"
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center gap-1.5 px-2 py-1.5">
          ...
          <span
            className={cn(
              "font-mono text-[7px]",
              isPowered ? "text-[var(--neon-red)]/70" : "text-white/30",
            )}
          >
            {stateLabel}
          </span>
          ...
          {isPowered ? (
            <button
              onClick={toggleFoldedInfo}
              className="flex h-3 w-3 cursor-pointer items-center justify-center rounded-full border border-white/20 transition-colors hover:border-[var(--neon-red)]/40"
              style={{ background: "radial-gradient(circle at 30% 30%, #2a2a3a 0%, #0a0a0f 70%)" }}
              title={showFoldedInfo ? "Hide info" : "Show info"}
            >
              <span className="font-mono text-[6px] text-[var(--neon-red)]/60">
        ...
        >
          <div className="flex flex-wrap gap-3 px-2 pb-1.5 font-mono text-[7px] text-[var(--neon-red)]/60">
      ...
      >
        ...
        >
          <div
            className="flex h-3 w-3 items-center justify-center rounded-full border border-white/20 transition-all hover:border-[var(--neon-red)]/40"
            style={{ background: "radial-gradient(circle at 30% 30%, #2a2a3a 0%, #0a0a0f 70%)" }}
          >
        ...
        <div className="mr-2 flex shrink-0 flex-col justify-center gap-1.5">
          ...
          <button
            ...
            disabled={deviceState !== "online"}
            className={cn(
              "h-3 w-3 rounded-[2px] border transition-all duration-200",
              "flex items-center justify-center",
              deviceState === "online"
                ? "cursor-pointer border-[var(--neon-red)]/50 bg-[var(--neon-red)]/10 hover:bg-[var(--neon-red)]/30"
          ...
          <button
            ...
            className={cn(
              ...
              "flex items-center justify-center",
              deviceState !== "booting" && deviceState !== "rebooting"
                ? "cursor-pointer border-amber-500/50 bg-amber-500/10 hover:bg-amber-500/30"
                : "cursor-not-allowed border-white/10 bg-black/30 opacity-50",
            )}
            style={{
              boxShadow:
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <div className="mb-1 flex items-center justify-between">
            <div className="font-mono text-[9px] text-[var(--neon-red)]">PRECISION LASER</div>
            <div className="flex items-center gap-1">
          ...
          <div className="relative min-h-[2rem] flex-1 overflow-hidden rounded bg-black/40">
            {/* Company logo */}
            <div
              className={cn("absolute z-10 font-mono text-[5px] font-bold", logoPositionClass)}
              style={{
            ...
            {(deviceState === "booting" || deviceState === "rebooting") && (
              <div className="absolute inset-0 flex items-center justify-center">
                <span
                  className="animate-pulse font-mono text-[7px]"
            ...
            {testPhase && testPhase !== "complete" && (
              <div className="absolute top-0.5 left-0.5 rounded bg-black/60 px-0.5 font-mono text-[6px] text-[var(--neon-red)]">
                {testPhase.toUpperCase()}
              </div>
            )}
            ...
            {/* Focus point */}
            <div
              className="absolute bottom-1.5 h-2 w-2 -translate-x-1/2 rounded-full transition-all duration-200"
              style={{
                left: `${laserPosition}%`,
            ...
            {isTesting && (
              <div
                ...
                style={{
                  background:
                    "linear-gradient(to bottom, transparent 0%, rgba(255,100,100,0.1) 50%, transparent 100%)",
...
export function Printer3D({ className }: Printer3DProps) {
  ...
  const knurlBtn = (
    ...
    <button
      ...
      disabled={disabled}
      className={cn(
        "h-3.5 w-3.5 rounded-[2px] border transition-all duration-200",
        "relative flex items-center justify-center overflow-hidden",
        !disabled
          ? "cursor-pointer border-white/30 hover:border-[var(--neon-amber)]/60"
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-1.5 px-2 py-1.5">
          ...
          {isPowered && (
            <>
              {knurlBtn(
                ...
                "TEST",
                <div
                  className="relative z-10 h-1.5 w-1.5 rounded-[1px]"
                  style={{
                    background: isTesting
                      ? "var(--neon-amber)"
          ...
          >
            <svg width="6" height="6" viewBox="0 0 6 6" className="relative z-10">
              <polygon
                points="1,0 5,0 3,5"
                fill={isPowered ? "var(--neon-amber)" : "#3a3a4a"}
                stroke={isPowered ? "var(--neon-amber)" : "#5a5a6a"}
                strokeWidth="0.5"
                style={{ filter: isPowered ? "drop-shadow(0 0 2px var(--neon-amber))" : "none" }}
              />
            </svg>
          </button>
          {/* Info / Fold toggle */}
          {isPowered ? (
            <button
              onClick={toggleFoldedInfo}
              className="flex h-3.5 w-3.5 cursor-pointer items-center justify-center rounded-[2px] border border-white/20 transition-colors hover:border-[var(--neon-amber)]/40"
              style={{ background: "linear-gradient(135deg, #3a3a4a 0%, #1a1a2a 100%)" }}
              title={showFoldedInfo ? "Hide info" : "Show info"}
            >
              <span className="font-mono text-[7px] text-[var(--neon-amber)]/60">
                {showFoldedInfo ? "▲" : "▼"}
              </span>
            </button>
          ) : (
        ...
        >
          <div className="grid grid-cols-3 gap-x-2 gap-y-0.5 px-2 pb-1.5 font-mono text-[7px] text-[var(--neon-amber)]/60">
      ...
      >
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <div className="mb-1 flex items-center justify-between">
            <div className="font-mono text-[9px] text-[var(--neon-amber)]">3D FABRICATOR</div>
            <div className="flex items-center gap-1">
              <div className="font-mono text-[7px] text-white/30">T2</div>
              <LED
                on={isActive}
          ...
          <div className="relative min-h-[2.5rem] flex-1 overflow-hidden rounded bg-black/40">
            {/* Company logo */}
            <div
              className={cn("absolute z-10 font-mono text-[5px] font-bold", logoPositionClass)}
              style={{
            ...
            {/* Test phase indicator */}
            {testPhase && testPhase !== "complete" && (
              <div className="absolute top-0.5 left-0.5 z-20 rounded bg-black/60 px-0.5 font-mono text-[6px] text-[var(--neon-amber)]">
                {testPhase.toUpperCase()}
              </div>
            )}
            ...
            {isActive && (
              <div
                className="absolute right-2 bottom-0 left-2 bg-[var(--neon-amber)]/30 transition-all duration-300"
                style={{ height: `${currentProgress}%` }}
              >
                {Array.from({ length: 5 }).map((_, i) => (
                  <div
                    key={i}
                    className="absolute right-0 left-0 h-px bg-[var(--neon-amber)]/50"
                    style={{ bottom: `${i * 20}%` }}
                  />
                ))}
              </div>
            ...
            {isActive && (
              <div
                className="absolute right-0.5 bottom-0.5 font-mono text-[5px]"
        ...
        {/* Right side buttons */}
        <div className="ml-1.5 flex shrink-0 flex-col justify-end pb-0">
...
export function ExoticMatterContainment({
      ...
      >
        <div
          ...
          style={{
            ...
            boxShadow: isStandby
              ? "none"
              : "0 0 4px var(--neon-pink), inset 0 0 2px rgba(255,0,255,0.3)",
  ...
  const ledBtn = (
    ...
    >
      <div
        className={cn(
          "h-2 w-2.5 p-[1px] transition-all group-active:scale-95",
    ...
    >
      ...
      >
        <div className="flex items-center gap-1 px-1 py-0.5">
          ...
          <span className="shrink-0 font-mono text-[6px] font-bold text-[var(--neon-pink)]">
            EMC-001
          </span>
          <span
            className={cn(
              "shrink-0 font-mono text-[5px]",
          ...
          {!isStandby ? (
            <button
              ...
              className="px-0.5 font-mono text-[6px] text-[var(--neon-pink)]/50 transition-colors hover:text-[var(--neon-pink)]"
              title={showFoldedInfo ? "Hide info" : "Show info"}
            >
              {showFoldedInfo ? "▲" : "▼"}
            </button>
        ...
        >
          <div className="flex flex-wrap gap-2 px-1.5 pb-1 font-mono text-[5px] text-[var(--neon-pink)]/60">
      ...
      >
        ...
        {emcManager && !isStandby && (
          <button
            ...
            className="absolute top-[2px] right-[2px] z-10 font-mono text-[5px] text-[var(--neon-pink)]/40 transition-colors hover:text-[var(--neon-pink)]"
            title="Fold"
          >
            ▴
          </button>
        ...
        <div className="p-1">
          ...
          <div className="mb-0.5 flex items-center justify-between">
            <div className="flex items-center gap-0.5">
              ...
              <div
                className="px-0.5 font-mono text-[3px] leading-none text-[#d060d0]"
                style={{
          ...
          <div
            className={cn(
              "relative h-14 overflow-hidden rounded bg-black/60",
          ...
          >
            ...
            <div
              ...
              style={{
                ...
                boxShadow: fieldActive
                  ? "0 0 6px var(--neon-pink), inset 0 0 12px rgba(255,0,255,0.15)"
                  : "none",
            ...
            {isStandby && (
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="font-mono text-[6px] tracking-widest text-white/15">STANDBY</div>
              </div>
            )}
            ...
            {deviceState === "testing" && (
              <>
                <div
                  ...
                  style={{
                    background:
                      "repeating-linear-gradient(90deg, transparent 0px, transparent 2px, var(--neon-cyan) 2px, var(--neon-cyan) 3px)",
                ...
                <div
                  ...
                  style={{
                    background:
                      "radial-gradient(ellipse at center, var(--neon-purple) 0%, transparent 50%)",
                    opacity: 0.25,
                ...
                <div
                  ...
                  style={{
                    background:
                      "repeating-linear-gradient(0deg, transparent 0px, transparent 4px, var(--neon-pink) 4px, var(--neon-pink) 5px)",
            ...
            {(deviceState === "booting" || deviceState === "rebooting") && (
              <>
                <div
                  ...
                  style={{
                    background:
                      "radial-gradient(circle at center, transparent 0%, var(--neon-amber) 30%, transparent 50%)",
                    opacity: 0.3,
          ...
          {/* Compact status bar */}
          <div className="mt-0.5 flex items-center justify-between font-mono text-[5px]">
            <span
              className={cn(
                "w-10 shrink-0 transition-colors",
            ...
            <span
              className={cn(
                "flex-1 overflow-hidden px-0.5 text-center text-[4px] text-ellipsis whitespace-nowrap transition-colors",
                deviceState === "testing"
                  ? "text-[var(--neon-pink)]"
                  : deviceState === "rebooting" || deviceState === "booting"
                    ? "text-[var(--neon-amber)]"
...
export function QuantumStateMonitor({
    ...
    qsmManager ? (
      <button
        ...
        style={{
          ...
          boxShadow: isStandby
            ? "none"
            : "0 0 4px var(--neon-cyan), inset 0 0 3px rgba(0,220,255,0.2)",
          opacity: isTransitioning ? 0.4 : 1,
          cursor: isTransitioning ? "not-allowed" : "pointer",
  ...
  const woodBtn = (
    ...
    >
      <div
        className={cn(
          "h-2.5 w-2.5 p-[1px] transition-all group-active:scale-95",
      ...
      >
        ...
        >
          <span
            className={cn(
              "font-mono text-[3px] font-bold",
              round ? "text-[#2a1a00]" : "text-[#1a0a00]",
            )}
          >
            {label}
          </span>
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center gap-1 px-1 py-0.5">
          <LED on={isLedOn} color={getLedColor()} size="sm" className="shrink-0 scale-75" />
          <span className="shrink-0 font-mono text-[6px] font-bold text-[var(--neon-cyan)]">
            QSM-001
          </span>
          <span
            className={cn(
          ...
          {!isStandby ? (
            <button
              onClick={() => setShowFoldedInfo((p) => !p)}
              className="px-0.5 font-mono text-[6px] text-[var(--neon-cyan)]/50 transition-colors hover:text-[var(--neon-cyan)]"
              title={showFoldedInfo ? "Hide info" : "Show info"}
            >
              {showFoldedInfo ? "▲" : "▼"}
            </button>
        ...
        >
          <div className="flex flex-wrap gap-2 px-1.5 pb-1 font-mono text-[5px] text-[var(--neon-cyan)]/60">
      ...
      >
        ...
        {qsmManager && !isStandby && (
          <button
            ...
            className="absolute top-[2px] right-[2px] z-10 font-mono text-[5px] text-[var(--neon-cyan)]/40 transition-colors hover:text-[var(--neon-cyan)]"
            title="Fold"
          >
            ▴
          </button>
        ...
        <div className="p-1">
          ...
          <div className="mb-0.5 flex items-center justify-between">
            <div className="flex items-center gap-0.5">
              ...
              {/* IBM style logo */}
              <div
                className="px-0.5 font-mono text-[3px] leading-none font-bold text-[#6090c0]"
                style={{
          ...
          <div
            className={cn(
              "relative h-10 overflow-hidden rounded bg-black/60",
          ...
          >
                ...
                <defs>
                  <linearGradient id="waveGrad" x1="0%" y1="0%" x2="100%" y2="0%">
                    ...
                    <stop
                      ...
                      stopOpacity={isActive ? "0.6" : isStandby ? "0.05" : "0.2"}
                    />
              ...
              <div
                className={cn(
                  ...
                  isActive
                    ? "text-[var(--neon-cyan)]"
                    : isStandby
                      ? "text-[var(--neon-cyan)]/10"
                      : "text-[var(--neon-cyan)]/30",
                )}
                style={{
                  textShadow: isActive ? "0 0 8px var(--neon-cyan)" : "none",
                  animation:
                    deviceState === "testing" ? "quantum-pulse 0.3s ease-in-out infinite" : "none",
                }}
              >
                |ψ⟩
              </div>
            ...
            {deviceState === "testing" && (
              <div
                ...
                style={{
                  background:
                    "repeating-linear-gradient(90deg, transparent 0px, transparent 5px, var(--neon-cyan) 5px, var(--neon-cyan) 6px)",
            ...
            {(deviceState === "booting" || deviceState === "rebooting") && (
              <div
                ...
                style={{
                  background:
                    "radial-gradient(circle at center, var(--neon-amber) 0%, transparent 60%)",
                  opacity: 0.2,
          ...
          <div className="mt-0.5 flex items-center justify-between font-mono text-[5px]">
            ...
            <span
              className={cn(
                "flex-1 overflow-hidden px-0.5 text-center text-[4px] text-ellipsis whitespace-nowrap",
                isStandby
                  ? "text-white/30"
                  : deviceState === "testing"
                    ? "text-[var(--neon-cyan)]"
...
export function NetworkMonitor({
  ...
  const getStatusLabel = () => {
    switch (deviceState) {
      case "online":
        return displayConnected ? "CONNECTED" : "ONLINE";
      case "booting":
        return "BOOT";
      case "testing":
        return "TEST";
  ...
  const powerSwitch = (
    ...
    >
      <div
        className="relative h-[4px] w-[7px] overflow-hidden rounded-full border transition-all"
        style={{
          background: "#0a0a0a",
          borderColor:
            isPowered && deviceState !== "standby"
              ...
              : deviceState === "standby"
                ...
                : "#2a2a2a",
          boxShadow:
            isPowered && deviceState !== "standby" ? "0 0 3px rgba(191,255,0,0.4)" : "none",
        }}
      >
        <div
          className="absolute top-[0.5px] h-[2px] w-[3px] rounded-full transition-all"
          style={{
            ...
            backgroundColor:
              isPowered && deviceState !== "standby"
                ...
                : deviceState === "standby"
                  ...
                  : "#333",
            boxShadow:
              isPowered && deviceState !== "standby" ? "0 0 2px var(--neon-lime,#bfff00)" : "none",
          }}
        />
      </div>
    </button>
      ...
      >
        <div
          className={cn(
            ...
            isActiveState ? activeClass : inactiveClass,
          )}
        />
      </div>
    </button>
    ...
    >
      ...
      >
        <div className="px-1 py-0.5">
          ...
          <div className="flex items-center gap-1">
            {/* Status LED */}
            <div
              className="h-1.5 w-1.5 flex-shrink-0 scale-75 rounded-full"
              style={{
                backgroundColor: getStatusColor(),
                boxShadow: `0 0 4px ${getStatusColor()}`,
                animation:
            ...
            />
            <span className="font-mono text-[6px] font-bold tracking-wide text-[var(--neon-lime,#bfff00)]">
              NET-001
            </span>
            ...
            {isPowered ? (
              <button
                onClick={toggleFoldedInfo}
                className="flex-shrink-0 font-mono text-[6px] text-white/40 transition-colors hover:text-white/70"
                title={showFoldedInfo ? "Hide info" : "Show info"}
              >
                {showFoldedInfo ? "▲" : "▼"}
              </button>
              ...
              <button
                ...
                className="flex-shrink-0 font-mono text-[6px] text-white/30 transition-colors hover:text-white/50"
                title="Unfold"
              >
                ▼
              </button>
          ...
          >
            <div className="mt-1 grid grid-cols-3 gap-x-2 gap-y-0.5 font-mono text-[5px]">
              ...
              <span className="text-white/30">
                Lat: <span className="text-[var(--neon-lime,#bfff00)]">{displayLatency}ms</span>
              </span>
              <span className="text-white/30">
                Link:{" "}
                <span className="text-[var(--neon-lime,#bfff00)]">
                  {displayConnected ? "UP" : "DOWN"}
                </span>
              </span>
              <span className="text-white/30">
                Loss:{" "}
                <span className="text-[var(--neon-lime,#bfff00)]">{manager?.packetLoss ?? 0}%</span>
              </span>
              <span className="text-white/30">
                Draw:{" "}
                <span className="text-[var(--neon-lime,#bfff00)]">{currentDraw.toFixed(1)}</span>
              </span>
              <span className="text-white/30">
                Tier: <span className="text-white/50">T1</span>
              </span>
      ...
      >
        ...
        <div className="relative p-1">
          ...
          <div className="absolute top-[3px] right-[10px] z-10">{powerSwitch}</div>
          ...
          <div className="mb-0.5 flex items-center justify-between">
            <div className="flex items-center gap-0.5">
              ...
              {/* Cisco style logo */}
              <div
                className="px-0.5 font-mono text-[3px] leading-none text-[#60c0a0]"
                style={{
            ...
            <div className="mr-5 font-mono text-[3px] text-white/20">NET-001</div>
          </div>

          {/* Bandwidth visualization - expanded */}
          <div
            className={cn(
              "relative h-12 overflow-hidden rounded bg-black/60 p-1",
          ...
          >
              ...
              {bars.map((h, i) => (
                <div
                  ...
                  style={{
                    ...
                    opacity:
                      deviceState === "testing" ? (testPhase === "bandwidth" ? 1 : 0.7) : 0.8,
                  }}
                />
              ))}
            </div>
            ...
            {deviceState === "testing" && (
              <div
                ...
                style={{
                  background:
                    "repeating-linear-gradient(0deg, transparent 0px, transparent 3px, var(--neon-lime,#bfff00) 3px, var(--neon-lime,#bfff00) 4px)",
            ...
            {(deviceState === "booting" || deviceState === "rebooting") && (
              ...
              >
                <div className="animate-pulse font-mono text-[6px] text-[var(--neon-amber)]">
                  {statusMessage}
                </div>
              </div>
            )}
          ...
          <div className="mt-0.5 flex items-center justify-between font-mono text-[5px]">
            ...
            >
              {deviceState === "online" || deviceState === "testing"
                ? `${displayLatency}ms`
                : statusMessage}
            </span>
          </div>
...
export function TemperatureMonitor({
  ...
  const getStatusLabel = () => {
    switch (deviceState) {
      case "online":
        return isActive ? `${currentTemp.toFixed(1)}°C` : "ONLINE";
      case "booting":
        return "BOOT";
      case "testing":
        return "TEST";
  ...
  const hexPowerBtn = (
    ...
    >
      <div
        className="h-[6px] w-[6px] transition-all"
        style={{
          clipPath: "polygon(50% 0%, 100% 25%, 100% 75%, 50% 100%, 0% 75%, 0% 25%)",
          background:
            isPowered && deviceState !== "standby"
    ...
    >
      ...
      >
        <div className="px-1 py-0.5">
          <div className="flex items-center gap-1">
            ...
            />
            <span className="font-mono text-[6px] font-bold tracking-wide text-[var(--neon-amber)]">
              TMP-001
            </span>
            <span className="font-mono text-[5px] text-white/50">{getStatusLabel()}</span>
            <div className="flex-1" />
              ...
              <>
                ...
                {ledBtn(
                  ...
                  deviceState === "booting" ||
                    ...
                    deviceState === "shutdown",
                  "Reboot",
                  "linear-gradient(180deg, #4a2a2a 0%, #3a1a1a 50%, #2a0a0a 100%)",
                  "#5a3a3a",
          ...
          >
            <div className="mt-1 grid grid-cols-3 gap-x-2 gap-y-0.5 font-mono text-[5px]">
              <span className="text-white/30">
                Temp: <span style={{ color: tempColor }}>{currentTemp.toFixed(1)}°C</span>
              </span>
              <span className="text-white/30">
                Range:{" "}
                <span className="text-[var(--neon-amber)]">
                  {displayMinTemp}-{displayMaxTemp}°
                </span>
              </span>
              <span className="text-white/30">
                Status: <span className="text-[var(--neon-green)]">{statusMessage}</span>
              </span>
              <span className="text-white/30">
                Draw: <span className="text-[var(--neon-amber)]">{currentDraw.toFixed(1)}</span>
              </span>
              ...
              <span className="text-white/30">
                Mode: <span className="text-[var(--neon-amber)]">AUTO</span>
              </span>
            </div>
          </div>
        </div>
      ...
      >
        ...
        <div className="relative p-1">
          ...
          <div className="absolute top-[3px] left-[3px] z-10">{hexPowerBtn}</div>

          {/* Header with company logo */}
          <div className="mb-0.5 flex items-center justify-between">
            <div className="flex items-center gap-0.5">
              <div className="ml-2 font-mono text-[5px] text-[var(--neon-amber)]">TEMP</div>
            </div>
            <div className="mr-3 flex items-center gap-0.5">
              {/* Intel style logo */}
              <div
                className="px-0.5 font-mono text-[3px] leading-none text-[#0088cc]"
                style={{
                  background: "linear-gradient(180deg, #1a2a4a 0%, #0a1a3a 100%)",
          ...
          >
            ...
            <div className="absolute inset-x-2 top-1/2 h-3 -translate-y-1/2 overflow-hidden rounded">
              <div
                className="absolute inset-0 rounded"
                style={{
                  background:
                    "linear-gradient(90deg, var(--neon-cyan) 0%, var(--neon-green) 35%, var(--neon-amber) 65%, var(--neon-red) 100%)",
                  opacity: isActive ? 0.8 : 0.3,
                }}
              />
              {isActive && (
            ...
            <div className="absolute inset-x-2 bottom-0.5 flex justify-between">
              <span className="font-mono text-[4px] text-[var(--neon-cyan)]/60">
                {displayMinTemp}°
              </span>
              ...
              <span className="font-mono text-[4px] text-[var(--neon-red)]/60">
                {displayMaxTemp}°
              </span>
            </div>
            ...
            {deviceState === "testing" && (
              <div
                ...
                style={{
                  background:
                    "repeating-linear-gradient(90deg, transparent 0px, transparent 4px, var(--neon-amber) 4px, var(--neon-amber) 5px)",
            ...
            {(deviceState === "booting" || deviceState === "rebooting") && (
              <div className="absolute inset-0 flex items-center justify-center bg-black/50">
                <span className="animate-pulse font-mono text-[5px] text-[var(--neon-amber)]">
                  {statusMessage}
                </span>
              </div>
            )}
...
export function SupercomputerArray({ className }: SupercomputerProps) {
  ...
  const isLedOn = !isStandby;
  const nodesActive =
    deviceState === "online" || deviceState === "testing" || deviceState === "booting";
        ...
        >
          <div className="grid grid-cols-3 gap-x-3 gap-y-0.5 px-2 pb-1.5">
            {[
              ...
              <div key={l} className="flex justify-between">
                <span className="font-mono text-[6px] text-white/30">{l}</span>
                <span className="font-mono text-[6px]" style={{ color: "var(--neon-cyan)" }}>
                  {v}
                </span>
              </div>
            ))}
      ...
      >
        ...
        <div className={cn("p-2", isStandby && "opacity-50")}>
          ...
          <div className="mb-1 flex items-center justify-between">
            <div className="flex items-center gap-1">
              <LED on={isLedOn} color={getLedColor()} size="sm" />
              <div className="font-mono text-[8px] text-[var(--neon-cyan)]">SUPERCOMPUTER</div>
            ...
            <div className="flex items-center gap-0.5">
              ...
              <div
                className="mr-0.5 rounded-[1px] border border-white/10 px-0.5 font-mono text-[3px] text-white/25"
                style={{
                  background:
              ...
              >
                <div
                  ...
                  style={{
                    background:
                      "linear-gradient(180deg, #5a6a7a 0%, #3a4a5a 20%, #2a3a4a 80%, #1a2a3a 100%)",
                    boxShadow:
                      "inset 0 0.5px 0 rgba(255,255,255,0.2), inset 0 -0.5px 0 rgba(0,0,0,0.3)",
                  }}
                >
                  <div
              ...
              >
                <div
                  ...
                  style={{
                    background:
                      "linear-gradient(180deg, #7a6a5a 0%, #5a4a3a 20%, #4a3a2a 80%, #3a2a1a 100%)",
                    boxShadow:
                      "inset 0 0.5px 0 rgba(255,255,255,0.2), inset 0 -0.5px 0 rgba(0,0,0,0.3)",
                  }}
                >
          ...
          <div
            className={cn(
              "relative h-8 overflow-hidden rounded bg-black/40 p-1",
              deviceState === "testing" &&
                testPhase === "nodes" &&
                "ring-1 ring-[var(--neon-cyan)]/50",
            )}
          >
            <div className="grid h-full grid-cols-8 grid-rows-2 gap-0.5">
          ...
          <div className="mt-1 flex items-center font-mono text-[7px]">
            ...
            <span className={cn("w-10 shrink-0 text-right transition-colors", "text-white/40")}>
...
export function DimensionMonitor({
  ...
  const entanglementStrength =
    displayStability > 90 ? "STRONG" : displayStability > 70 ? "MODERATE" : "WEAK";
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center gap-1 px-1 py-0.5">
          <div
            className="h-1.5 w-1.5 shrink-0 scale-75 rounded-full"
            style={{
              backgroundColor: getStatusColor(),
              boxShadow: `0 0 4px ${getStatusColor()}`,
              animation:
          ...
          />
          <span className="font-mono text-[6px] font-bold text-[var(--neon-purple,#9d00ff)]">
            DIM-001
          </span>
          <span
            className={cn(
              "font-mono text-[5px]",
              isPowered ? "text-[var(--neon-purple,#9d00ff)]/70" : "text-white/30",
            )}
          >
            {stateLabel}
          </span>
          ...
          {isPowered ? (
            <button
              onClick={toggleFoldedInfo}
              className="flex h-3 w-3 cursor-pointer items-center justify-center rounded-full border border-white/20 transition-colors hover:border-[var(--neon-purple,#9d00ff)]/40"
              style={{ background: "radial-gradient(circle at 30% 30%, #2a2a3a 0%, #0a0a0f 70%)" }}
              title={showFoldedInfo ? "Hide info" : "Show info"}
            >
              <span className="font-mono text-[6px] text-[var(--neon-purple,#9d00ff)]/60">
                {showFoldedInfo ? "▲" : "▼"}
              </span>
            </button>
          ) : (
        ...
        >
          <div className="flex flex-wrap gap-2 px-1.5 pb-1 font-mono text-[5px] text-[var(--neon-purple,#9d00ff)]/60">
      ...
      >
        ...
        >
          <div
            className="flex h-3 w-3 items-center justify-center rounded-full border border-white/20 transition-all hover:border-[var(--neon-purple,#9d00ff)]/40"
            style={{ background: "radial-gradient(circle at 30% 30%, #2a2a3a 0%, #0a0a0f 70%)" }}
          >
        ...
        {(dim?.deviceState === "standby" || dim?.deviceState === "shutdown") && (
          <div className="pointer-events-none absolute inset-0 z-30 flex items-center justify-center rounded bg-black/80">
            <span className="font-mono text-[7px] tracking-widest text-white/50">
        ...
        <div className="mb-0.5 flex items-center justify-between">
          <div className="flex items-center gap-0.5">
            ...
            <div className="ml-1 flex items-center gap-0.5">
              ...
              >
                <div
                  className="h-2 w-2 rounded-full p-[1px] transition-all group-active:scale-95"
                  style={{
          ...
          <div className="flex items-center gap-0.5">
            ...
            <div
              className="px-0.5 font-mono text-[3px] leading-none text-[#cc00ff]"
              style={{
        ...
        <div
          className={cn(
            "relative min-h-[3rem] flex-1 overflow-hidden rounded bg-black/60",
            deviceState === "testing" && "ring-1 ring-[var(--neon-purple,#9d00ff)]/50",
            (deviceState === "booting" || deviceState === "rebooting") &&
              "ring-1 ring-[var(--neon-pink)]/30",
          )}
        >
          ...
          {isActive && (
            <div
              ...
              style={{
                background:
                  "radial-gradient(ellipse at center, var(--neon-purple,#9d00ff) 0%, transparent 65%)",
                opacity: 0.08,
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="relative">
              ...
              <span
                className={cn(
                  "relative z-10 font-mono text-[14px] transition-all duration-200",
          ...
          <div className="absolute top-1 bottom-1 left-1 flex flex-col justify-between">
            <div>
              <div className="font-mono text-[4px] text-white/30">RIFT ACTIVITY</div>
              <div className="font-mono text-[5px] text-[var(--neon-pink)]">
                {(displayRiftActivity * 100).toFixed(1)}%
              </div>
            </div>
            <div>
              ...
              <div
                ...
                style={{ color: haloProximity > 20 ? "var(--neon-amber)" : "var(--neon-cyan)" }}
              >
          ...
          <div className="absolute top-1 right-1 bottom-1 flex flex-col items-end justify-between">
            <div className="text-right">
              <div className="font-mono text-[4px] text-white/30">STABILITY</div>
              <div className="font-mono text-[5px]" style={{ color: stabilityColor }}>
                {displayStability}%
              </div>
            </div>
            <div className="text-right">
              ...
              <div
                ...
                style={{
                  color:
                    entanglementStrength === "STRONG"
                      ? "var(--neon-green)"
                      : entanglementStrength === "MODERATE"
                        ? "var(--neon-amber)"
                        : "var(--neon-red)",
                }}
              >
          ...
          <div className="absolute inset-x-2 bottom-1.5 h-1.5 overflow-hidden rounded bg-black/40">
            <div
              className="h-full transition-all duration-300"
              style={{
          ...
          {deviceState === "testing" && (
            <div
              ...
              style={{
                ...
                animation:
                  testPhase === "rift-scan"
                    ? "dim-scan 0.8s ease-out infinite"
                    : "dim-pulse 0.8s ease-in-out infinite",
                opacity: 0.25,
              }}
            />
          )}
          ...
          {(deviceState === "booting" || deviceState === "rebooting") && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/50">
              ...
              <div
                ...
                style={{
                  background:
                    "linear-gradient(90deg, transparent 0%, var(--neon-purple,#9d00ff) 50%, transparent 100%)",
                  boxShadow: "0 0 6px var(--neon-purple,#9d00ff)",
                  animation: "dim-scanline 1.5s ease-in-out infinite",
                }}
              />
              <span className="relative z-10 animate-pulse font-mono text-[6px] text-[var(--neon-purple,#9d00ff)]">
                {statusMessage}
              </span>
            </div>
          )}
          ...
          {deviceState === "shutdown" && (
            <div className="absolute inset-0 flex items-center justify-center bg-black/60">
              <span className="animate-pulse font-mono text-[6px] text-[var(--neon-pink)]">
                {statusMessage}
              </span>
            </div>
          )}
...
export function CpuMonitor({ className }: { className?: string }) {
  ...
  const isActive = deviceState === "online" || deviceState === "testing";
  const avgLoad =
    coreLoads.length > 0 ? Math.round(coreLoads.reduce((a, b) => a + b, 0) / coreLoads.length) : 0;
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center gap-1.5 px-2 py-1.5">
          ...
          {isPowered ? (
            <button
              onClick={toggleFoldedInfo}
              className="flex h-3 w-3 cursor-pointer items-center justify-center rounded-full border border-white/20 transition-colors hover:border-[var(--neon-cyan)]/40"
              style={{ background: "radial-gradient(circle at 30% 30%, #2a2a3a 0%, #0a0a0f 70%)" }}
              title={showFoldedInfo ? "Hide info" : "Show info"}
            >
              <span className="font-mono text-[6px] text-[var(--neon-cyan)]/60">
                {showFoldedInfo ? "▲" : "▼"}
              </span>
            </button>
          ) : (
      ...
      >
        ...
        >
          <div
            className="flex h-3 w-3 items-center justify-center rounded-full border border-white/20 transition-all hover:border-[var(--neon-cyan)]/40"
            style={{ background: "radial-gradient(circle at 30% 30%, #2a2a3a 0%, #0a0a0f 70%)" }}
          >
        ...
        <div className="mb-0.5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            ...
            >
              <div
                className="h-3 w-3 rounded-full p-[2px] transition-all group-active:scale-95"
                style={{
                  background:
                    "linear-gradient(180deg, #3a4a4a 0%, #2a3a3a 30%, #1a2a2a 70%, #0a1a1a 100%)",
                  boxShadow:
                    "inset 0 -1px 2px rgba(0,255,255,0.1), inset 0 1px 1px rgba(0,0,0,0.5), 0 1px 3px rgba(0,0,0,0.6)",
            ...
            >
              <div
                ...
                style={{
                  background:
                    "linear-gradient(180deg, #4a3a2a 0%, #3a2a1a 30%, #2a1a0a 70%, #1a0a00 100%)",
                  boxShadow:
                    "inset 0 -1px 2px rgba(255,150,0,0.1), inset 0 1px 1px rgba(0,0,0,0.5), 0 1px 3px rgba(0,0,0,0.6)",
          ...
          <div className="flex items-center gap-0.5">
            <div
              className="px-0.5 font-mono text-[3px] leading-none text-[#00cc99]"
              style={{
                background: "linear-gradient(180deg, #1a3a2a 0%, #0a2a1a 100%)",
        ...
        <div
          className={cn(
            "relative min-h-[2.5rem] flex-1 overflow-hidden rounded bg-black/60",
            deviceState === "testing" && "ring-1 ring-[var(--neon-cyan)]/30",
          )}
        >
          {/* Core bars */}
              ...
              <div key={i} className="flex h-full flex-1 flex-col justify-end">
                <div
                  ...
                  style={{
                    ...
                    boxShadow: isActive
                      ? `0 0 4px ${load > 80 ? "var(--neon-red)" : "var(--neon-cyan)"}`
                      : "none",
          ...
          <div
            ...
            style={{
              backgroundImage:
                "linear-gradient(0deg, transparent 0%, transparent 24%, rgba(255,255,255,0.1) 25%, transparent 26%)",
          ...
          {(deviceState === "booting" || deviceState === "rebooting") && (
            <div className="absolute inset-0 flex items-center justify-center bg-black/50">
              <span className="animate-pulse font-mono text-[5px] text-[var(--neon-cyan)]">
                {statusMessage}
              </span>
            </div>
          )}
          ...
          {deviceState === "standby" && (
            <div className="absolute inset-0 flex items-center justify-center bg-black/80">
              <span className="font-mono text-[6px] tracking-wide text-white/30">STANDBY</span>
            </div>
          )}
        </div>

        {/* Status bar with power button */}
        <div className="relative mt-0.5 flex items-center justify-between font-mono text-[5px]">
          <span
            className={cn(
              "transition-colors",
              isActive
...
export function LabClock({ className }: { className?: string }) {
  ...
  const getCountdownStyle = (seconds: number) => {
    if (seconds <= 5)
      return { numColor: "#ff1a1a", colonColor: "#ff1a1a", weight: 700, flash: true };
    if (seconds <= 10)
      return { numColor: "#ef4444", colonColor: "#ff1a1a", weight: 700, flash: false };
    if (seconds <= 30)
      return { numColor: "#ef4444", colonColor: "#eab308", weight: 700, flash: false };
    if (seconds <= 120)
      return { numColor: "#eab308", colonColor: "#22c55e", weight: 400, flash: false };
  ...
  const renderCountdownDisplay = (seconds: number) => {
    ...
    return (
      <span
        className="font-mono text-[16px] tracking-wider tabular-nums"
        style={{
          fontWeight: style.weight,
          animation: style.flash ? "blink 0.5s steps(1) infinite" : undefined,
        }}
      >
        <span style={{ color: style.numColor, textShadow: `0 0 8px ${style.numColor}66` }}>
          {nums[0]}
        </span>
  ...
  const getDisplayContent = () => {
    ...
    switch (displayMode) {
      ...
      case "date":
        return {
          main: currentTime.toLocaleDateString("en-US", {
            month: "2-digit",
            day: "2-digit",
            year: "2-digit",
          }),
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center gap-1.5 px-2 py-1.5">
          ...
          <span
            className={cn(
              "font-mono text-[7px]",
              isPowered ? "text-[var(--neon-green)]/70" : "text-white/30",
            )}
          >
            {stateLabel}
          </span>
          ...
          >
            <div
              className="flex h-1 w-2 items-center rounded-full px-0.5 transition-all"
              style={{
            ...
            >
              <div
                className="h-0.5 w-0.5 rounded-full transition-all duration-200"
                style={{
                  background: isPowered
          ...
          {isPowered ? (
            <button
              onClick={toggleFoldedInfo}
              className="flex h-3 w-3 cursor-pointer items-center justify-center rounded-full border border-white/20 transition-colors hover:border-[var(--neon-green)]/40"
              style={{ background: "radial-gradient(circle at 30% 30%, #2a2a3a 0%, #0a0a0f 70%)" }}
              title={showFoldedInfo ? "Hide info" : "Show info"}
            >
              <span className="font-mono text-[6px] text-[var(--neon-green)]/60">
                {showFoldedInfo ? "\u25B2" : "\u25BC"}
              </span>
            </button>
          ) : (
        ...
        >
          <div className="flex flex-wrap gap-3 px-2 pb-1.5 font-mono text-[7px] text-[var(--neon-green)]/60">
            <span>Mode: {displayMode.toUpperCase()}</span>
      ...
      >
        ...
        >
          <div
            className="flex h-3 w-3 items-center justify-center rounded-full border border-white/20 transition-all hover:border-[var(--neon-green)]/40"
            style={{ background: "radial-gradient(circle at 30% 30%, #2a2a3a 0%, #0a0a0f 70%)" }}
          >
        ...
        <div className="mb-0.5 flex items-center justify-between">
          <div className="flex items-center gap-1">
            ...
            >
              <div
                className="rounded px-1 py-0.5 transition-all group-active:scale-95"
                style={{
          ...
          <div className="flex items-center gap-0.5">
            <div
              className="px-0.5 font-mono text-[3px] leading-none font-bold text-[#ffcc00]"
              style={{
        ...
        >
          ...
          {/* Sub label */}
          <div
            className="absolute bottom-1 left-1 font-mono text-[4px]"
            style={{ color: "var(--neon-green)", opacity: 0.6 }}
          >
            {display.sub}
          </div>
          ...
          {displayMode === "stopwatch" && isActive && (
            <div className="absolute right-1 bottom-1 flex gap-1">
              <button
                ...
                className="rounded px-1 font-mono text-[4px]"
                style={{
            ...
            <div className="absolute top-1 right-1 flex gap-0.5">
              {[0, 1, 2, 3].map((i) => (
                <div
                  ...
                  style={{
                    background:
                      currentTime.getSeconds() % 4 === i
                        ? "var(--neon-green)"
                        : "var(--neon-green)",
                    opacity: currentTime.getSeconds() % 4 === i ? 1 : 0.2,
                    boxShadow:
                      currentTime.getSeconds() % 4 === i ? "0 0 4px var(--neon-green)" : "none",
                  }}
                />
              ))}
            </div>
          ...
          {(deviceState === "booting" || deviceState === "rebooting") && (
            <div className="absolute inset-0 flex items-center justify-center bg-black/50">
              <span className="animate-pulse font-mono text-[6px] text-[var(--neon-green)]">
                {statusMessage}
              </span>
            </div>
          )}
          ...
          {deviceState === "testing" && (
            <div
              ...
              style={{
                background:
                  "repeating-linear-gradient(90deg, transparent 0px, transparent 3px, var(--neon-green) 3px, var(--neon-green) 4px)",
          ...
          <div className="flex items-center gap-1">
            ...
            >
              <div
                ...
                style={{
                  background:
                    "linear-gradient(180deg, #2a3a2a 0%, #1a2a1a 30%, #0a1a0a 70%, #001a00 100%)",
                  boxShadow:
                    "inset 0 -1px 2px rgba(0,255,0,0.1), inset 0 1px 1px rgba(0,0,0,0.5), 0 1px 3px rgba(0,0,0,0.6)",
            ...
            >
              <div
                ...
                style={{
                  background:
                    "linear-gradient(180deg, #3a3a2a 0%, #2a2a1a 30%, #1a1a0a 70%, #1a0a00 100%)",
                  boxShadow:
                    "inset 0 -1px 2px rgba(255,180,0,0.1), inset 0 1px 1px rgba(0,0,0,0.5), 0 1px 3px rgba(0,0,0,0.6)",
...
export function MemoryMonitor({ className }: { className?: string }) {
  ...
  const toggleFoldedInfo = () => {
    setShowFoldedInfo((prev) => {
      ...
      if (next) {
        foldedInfoTimerMem.current = setTimeout(() => setShowFoldedInfo(false), 5 * 60 * 1000);
      }
      return next;
    });
  };
    ...
    >
      ...
      >
        ...
        >
          <div className="flex flex-wrap gap-3 px-2 pb-1.5 font-mono text-[7px] text-[var(--neon-amber)]/60">
            <span>
              Used: {usedMemory.toFixed(1)}G / {totalMemory}G
            </span>
            <span>Mode: {displayMode.toUpperCase()}</span>
      ...
      >
        ...
        <div className="mb-0.5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-0.5">
            <div
              className="px-0.5 font-mono text-[3px] leading-none font-bold text-[#ffaa00]"
              style={{
        ...
        >
          ...
          {displayMode === "usage" && (
            <>
              {/* Main usage bar */}
              <div className="absolute inset-x-2 top-1/2 h-3 -translate-y-1/2 overflow-hidden rounded bg-black/40">
                <div
              ...
              {/* Usage text */}
              <div
                className="absolute bottom-1 left-2 font-mono text-[5px]"
                style={{ color: usageColor }}
              >
                {isActive ? `${usedMemory.toFixed(1)}G` : "--"}
              </div>
              <div className="absolute right-2 bottom-1 font-mono text-[5px] text-white/40">
                / {totalMemory}G
              </div>
            </>
          )}
          ...
          {displayMode === "heap" && (
            <div className="absolute inset-2 flex flex-col justify-center gap-1">
              <div className="flex items-center gap-1">
                <span className="w-6 font-mono text-[4px] text-white/40">HEAP</span>
                <div className="h-2 flex-1 overflow-hidden rounded bg-black/40">
                  <div
                    className="h-full bg-[var(--neon-cyan)] transition-all"
                    style={{
                      width: isActive ? `${(memData.heap.used / memData.heap.total) * 100}%` : "0%",
                    }}
                  />
                </div>
              ...
              <div className="flex items-center gap-1">
                <span className="w-6 font-mono text-[4px] text-white/40">STACK</span>
                <div className="h-2 flex-1 overflow-hidden rounded bg-black/40">
                  <div
                ...
                <span className="font-mono text-[4px] text-[var(--neon-green)]">
                  {isActive ? "1.4G" : "--"}
                </span>
              </div>
            </div>
          ...
          {displayMode === "cache" && (
            <div className="absolute inset-2 flex flex-col justify-center gap-1">
              <div className="flex items-center gap-1">
                <span className="w-6 font-mono text-[4px] text-white/40">L1</span>
                <div className="h-1.5 flex-1 overflow-hidden rounded bg-black/40">
                  <div
                    className="h-full bg-[var(--neon-pink)] transition-all"
                    style={{ width: isActive ? "78%" : "0%" }}
                  />
                </div>
              </div>
              <div className="flex items-center gap-1">
                <span className="w-6 font-mono text-[4px] text-white/40">L2</span>
                <div className="h-1.5 flex-1 overflow-hidden rounded bg-black/40">
                  <div
                    className="h-full bg-[var(--neon-purple,#9d00ff)] transition-all"
                    style={{ width: isActive ? "65%" : "0%" }}
                  />
                </div>
              </div>
              <div className="flex items-center gap-1">
                <span className="w-6 font-mono text-[4px] text-white/40">L3</span>
                <div className="h-1.5 flex-1 overflow-hidden rounded bg-black/40">
                  <div
                    className="h-full bg-[var(--neon-cyan)] transition-all"
                    style={{ width: isActive ? "52%" : "0%" }}
                  />
                </div>
              </div>
            </div>
          ...
          {displayMode === "swap" && (
            <div className="absolute inset-2 flex flex-col justify-center">
              <div className="mb-1 font-mono text-[4px] text-white/40">SWAP USAGE</div>
              <div className="h-2.5 overflow-hidden rounded bg-black/40">
                <div
                  className="h-full bg-[var(--neon-red)] transition-all"
                  style={{
                    width: isActive ? `${(memData.swap.used / memData.swap.total) * 100}%` : "0%",
                  }}
                />
              </div>
            ...
            <div className="absolute inset-1 flex flex-col justify-center gap-0.5 overflow-hidden">
              {memData.processes.slice(0, 5).map((proc, i) => (
                <div key={i} className="flex items-center gap-1">
                  <span className="w-8 truncate font-mono text-[3px] text-white/40">
                    {proc.name}
                  </span>
                  ...
                  <span className="font-mono text-[3px] text-white/40">
                    {isActive ? `${proc.mem.toFixed(1)}G` : "--"}
                  </span>
                </div>
              ))}
            </div>
          ...
          {displayMode === "allocation" && (
            <div className="absolute inset-2 flex items-center justify-center">
              ...
              <div className="flex h-4 w-full overflow-hidden rounded bg-black/40">
                <div
                  className="h-full bg-[var(--neon-red)] transition-all"
                  style={{
                    width: isActive ? `${(memData.allocation.kernel / totalMemory) * 100}%` : "0%",
                  }}
                />
                <div
                  className="h-full bg-[var(--neon-amber)] transition-all"
                  style={{
                    width: isActive ? `${(memData.allocation.user / totalMemory) * 100}%` : "0%",
                  }}
                />
                <div
                  className="h-full bg-[var(--neon-cyan)] transition-all"
                  style={{
                    width: isActive ? `${(memData.allocation.buffers / totalMemory) * 100}%` : "0%",
                  }}
                />
              </div>
            </div>
          ...
          {displayMode === "allocation" && isActive && (
            <div className="absolute inset-x-2 bottom-1 flex justify-between font-mono text-[3px]">
          ...
          {deviceState === "testing" && (
            <div
              ...
              style={{
                background:
                  "repeating-linear-gradient(0deg, transparent 0px, transparent 2px, var(--neon-amber) 2px, var(--neon-amber) 3px)",
          ...
          <div className="flex items-center gap-1">
            ...
            >
              <div
                ...
                style={{
                  background:
                    "linear-gradient(180deg, #3a3a2a 0%, #2a2a1a 30%, #1a1a0a 70%, #0a0a00 100%)",
                  boxShadow:
                    "inset 0 -1px 2px rgba(255,180,0,0.1), inset 0 1px 1px rgba(0,0,0,0.5), 0 1px 3px rgba(0,0,0,0.6)",
                  border: "1px solid #4a4a3a",
                }}
...
export function VoltMeter({
  ...
  const modeConfig: Record<
    VoltMeterMode,
    { label: string; getValue: () => string; unit?: string }
  > = {
    VOLT: {
      label: "VOLT",
  ...
  return (
    <div
      className={cn(
        "relative inline-flex items-center gap-0.5 overflow-hidden rounded-sm",
        className,
      )}
      style={{
    ...
    >
      ...
      {/* Label */}
      <span
        className="min-w-[24px] font-mono text-[8px] tracking-wider select-none"
        style={{
          color: "#ffaa00",
      ...
      <div
        className="ml-1 min-w-[40px] text-right font-mono text-[11px] font-bold tracking-tight select-none"
        style={{
          fontFamily: "'DSEG7 Classic', 'Courier New', monospace",
          color: status === "critical" ? "#ff4444" : status === "warning" ? "#ffaa00" : "#ffbb22",
          textShadow:
            status === "critical"
      ...
      <div
        ...
        style={{
          background:
            status === "critical" ? "#ff4444" : status === "warning" ? "#ffaa00" : "#44ff44",
          boxShadow:
            status === "critical"
              ? "0 0 4px #ff4444"
              : status === "warning"
...
export function PowerDisplay({
  ...
  return (
    <PanelFrame variant="default" className={cn("p-2", className)}>
      {/* Header */}
      <div className="mb-1.5 flex items-center justify-between">
        <div className="font-mono text-[8px] tracking-wider text-[var(--neon-amber)] uppercase">
          PWR-001
        </div>
      ...
      <div className="space-y-1 font-mono text-[7px]">
        ...
        <div className="mt-1 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            <div
              className="h-2 w-6 overflow-hidden rounded-sm"
              style={{
                background: "#1a1a1a",
                border: "1px solid #333",
              }}
            >
              <div
                ...
                style={{
                  ...
                  boxShadow: storagePercent > 20 ? "0 0 4px #44ff88" : "0 0 4px #ff4444",
                }}
              />
            </div>
      ...
      <div className="mt-1.5 border-t border-white/10 pt-1">
        <div className="flex items-center justify-between font-mono text-[5px]">
          <span
            className={cn(
