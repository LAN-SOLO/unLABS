...
function FanUnit({ speed, isOn }: { speed: number; isOn: boolean }) {
  ...
  return (
    <div className="relative flex aspect-square w-full items-center justify-center overflow-hidden rounded-sm border border-[#1a1a2a] bg-[#050508]">
      {/* Outer casing ring */}
      <div
...
export function VentilationFan({
  ...
  const vntTransitioning = vntManager
    ? ["booting", "shutdown", "testing", "rebooting"].includes(vntManager.deviceState)
    : false;
  ...
  useEffect(() => {
    ...
    if (localMode === "AUTO" && localIsOn) {
      const autoSpeed = Math.min(
        100,
        Math.max(20, systemLoad + 15 + (localTemp > targetTemp ? (localTemp - targetTemp) * 3 : 0)),
      );
  ...
  // Hard interlock: at least one fan must always run. We can stop this fan
  // only if the *other* fan is currently active. The thermal manager enforces
  // the same rule, this just keeps the local UI consistent (disabled button
  // instead of a silent no-op).
  const otherFanId = fanId === "cpu" ? "gpu" : "cpu";
  const otherFanOn = thermalManager?.state.fans[otherFanId].isOn ?? true;
  const isLastRunningFan = isOn && !otherFanOn;
  const canStop = isOn && !isLastRunningFan;

  const handlePowerToggle = useCallback(() => {
    if (thermalManager) {
      // toggleFan in the manager already refuses to stop the last fan, but
      // bail early so the UI never even tries.
      if (isOn && !otherFanOn) return;
      thermalManager.toggleFan(fanId, !isOn);
    } else {
      // Fallback path (no manager): never let a single isolated instance turn
      // itself off — we have no way to verify the sibling fan is still on.
      if (localIsOn) return;
      setLocalIsOn(true);
    }
  }, [thermalManager, fanId, isOn, otherFanOn, localIsOn]);

  // Continuous blue → red gradient over 20–90 °C. Mirrors
  // ThermalManager.getTemperatureColor so the panel and the terminal agree.
  const getTempColor = () => {
    if (thermalManager) return thermalManager.getTemperatureColor(temp);
    const t = Math.max(0, Math.min(1, (temp - 20) / 70));
  ...
  const stateLabel = vntStandby
    ? "STANDBY"
    : vntTransitioning
      ? (vntManager?.deviceState?.toUpperCase() ?? "...")
      : "ONLINE";
  const stateLedColor = vntStandby
    ? "#555"
    : vntTransitioning
      ? "var(--neon-amber)"
      : "var(--neon-cyan)";
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-1 px-1.5 py-1">
          ...
          <span className="shrink-0 font-mono text-[6px]" style={{ color: stateLedColor }}>
            {stateLabel}
          </span>
          ...
          {/* Unfold panel — must always be reachable from the folded bar */}
          {vntManager && (
            <button
      ...
      >
        ...
        <div className={cn("flex min-h-0 flex-1 flex-col", vntStandby && "opacity-60")}>
          {/* Header */}
          <div className="flex shrink-0 items-center justify-between border-b border-white/10 px-1.5 py-1">
            <div className="flex items-center gap-1">
              <LED
                on={isOn && !vntStandby}
                color={isOn && !vntStandby ? "green" : "red"}
                size="sm"
              />
              {/* Warning LED - flashes when overheating */}
              <LED on={showWarningLed} color={getWarningLedColor() || "red"} size="sm" />
              <span className="font-mono text-[7px] font-bold text-[var(--neon-cyan)]">
                {label}
              </span>
              {/* VNT Power button */}
              {vntManager && (
          ...
          <div className="flex min-h-0 flex-1 flex-col gap-1 p-1.5">
            ...
            <div className="flex shrink-0 justify-center">
              {vntTransitioning ? (
                <div
                  className="animate-pulse rounded border px-2 py-0.5 font-mono text-[8px]"
                  style={{
                ...
                <div
                  className={cn(
                    "relative rounded border px-2 py-0.5 font-mono text-[9px]",
                ...
                >
                  ...
                  {(overallStatus === "critical" || overallStatus === "warning") && (
                    <span
                      ...
                      style={{
                        backgroundColor:
                          overallStatus === "critical" ? "var(--neon-red)" : "var(--neon-amber)",
          ...
          <div className="shrink-0 px-1.5 pb-1.5">
            ...
            <div className="flex items-stretch gap-1.5">
              ...
              <div className="flex flex-1 flex-col justify-center gap-0.5">
                ...
                <div className="grid grid-cols-2 gap-0.5">
                  {(["AUTO", "LOW", "MED", "HIGH"] as const).map((m) => (
                    <button
                      ...
                      disabled={!isOn || vntStandby}
                      className={cn(
                        "rounded border py-0.5 font-mono text-[5px] transition-all",
                        mode === m
                          ? "border-[var(--neon-cyan)] bg-[var(--neon-cyan)] font-bold text-black shadow-[0_0_4px_var(--neon-cyan)]"
                          : "border-[#1a1a2a] bg-[#0a0a0f] text-white/40 hover:border-white/20 hover:text-white/60 disabled:opacity-50",
                      )}
                    >
                      {m}
                    </button>
                ...
                {/* Power button — locked when this is the last running fan */}
                <button
                  onClick={handlePowerToggle}
                  disabled={isLastRunningFan}
                  title={isLastRunningFan ? "At least one fan must remain running" : undefined}
                  className={cn(
                    "w-full rounded border py-0.5 font-mono text-[5px] transition-all",
                    isLastRunningFan
                      ? "cursor-not-allowed border-[var(--neon-amber)]/30 bg-[var(--neon-amber)]/10 text-[var(--neon-amber)]/70"
                      : isOn
                ...
                >
                  {isLastRunningFan ? "LOCKED" : isOn ? "STOP" : "START"}
                </button>
              </div>
            </div>
          </div>
