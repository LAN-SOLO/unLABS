...
export function NarrowSpeaker({ className }: NarrowSpeakerProps) {
  ...
  return (
    <div
      className={cn(
        "flex w-12 shrink-0 flex-col gap-1 rounded border border-[#2a2a3a] bg-gradient-to-b from-[#1a1a1a] to-[#0a0a0a] p-1",
        className,
      )}
    >
      ...
      <div className="flex items-center justify-between px-0.5">
        ...
        <button
          ...
          disabled={isBusy}
          className={cn(
            "relative h-[8px] w-[14px] rounded-[2px] border transition-all duration-200",
            "focus:outline-none",
            isPowered
              ? "border-[var(--neon-green)]/40 bg-gradient-to-b from-[#2a3a2a] to-[#1a2a1a]"
              : "border-white/20 bg-gradient-to-b from-[#3a2a2a] to-[#2a1a1a]",
        ...
        >
          ...
          <div
            className={cn(
              "absolute top-[1px] h-[4px] w-[5px] rounded-[1px] transition-all duration-300",
              isPowered
          ...
          <div
            className={cn(
              "absolute -bottom-[3px] left-1/2 h-[3px] w-[3px] -translate-x-1/2 rounded-full transition-all duration-300",
              isPowered
      ...
      {isBusy && (
        <div className="px-0.5">
          <div
            className="truncate text-center font-mono text-[4px]"
            style={{
              color:
          ...
          {/* Tiny progress bar */}
          <div className="mt-0.5 h-[2px] overflow-hidden rounded bg-[#0a0a0a]">
            <div
              className="h-full transition-all duration-200"
              style={{
                width: isBooting
                  ? `${((["driver", "amplifier", "dac", "calibrate", "output", "ready"].indexOf(bootPhase ?? "ready") + 1) / 6) * 100}%`
                  : isShuttingDown
                    ? `${((["mute", "drain", "driver-off"].indexOf(shutdownPhase ?? "driver-off") + 1) / 3) * 100}%`
                    : "50%",
                background:
                  isBooting || isRebooting
                    ? "var(--neon-cyan)"
      ...
      {testResult && (
        <div
          className={cn(
            "rounded py-0.5 text-center font-mono text-[5px]",
            testResult === "pass"
      ...
      <div
        className={cn(
          "relative min-h-[60px] flex-1 overflow-hidden rounded border bg-[#0a0a0a] transition-all duration-500",
      ...
      >
        ...
        <div className="absolute inset-1 flex justify-center gap-px">
          {audioLevel.map((level, i) => (
            <div
              key={i}
              className="flex h-full w-1 flex-col justify-end overflow-hidden rounded-sm bg-[#1a1a1a]"
            >
              <div
                ...
                style={{
                  ...
                  boxShadow:
                    level > 0
                      ? `0 0 3px ${level > 80 ? "var(--neon-red)" : level > 50 ? "var(--neon-amber)" : "var(--neon-green)"}`
                      : "none",
                }}
              />
            </div>
      ...
      <div className="flex flex-col items-center gap-0.5">
        <div className="flex w-full items-center gap-1">
          ...
          <div className="relative h-1.5 flex-1 rounded border border-[#1a1a2a] bg-[#0a0a0a]">
            <div
      ...
      <button
        ...
        disabled={!isOn}
        className={cn(
          "w-full rounded py-0.5 font-mono text-[5px] transition-all",
          isMuted
            ? "border border-[var(--neon-red)]/50 bg-[var(--neon-red)]/30 text-[var(--neon-red)]"
            : "border border-[#2a2a3a] bg-[#1a1a1a] text-white/40 hover:text-white/60",
      ...
      <div className="flex gap-0.5">
        {[
          ...
          <button
            ...
            disabled={!isOn}
            className={cn(
              "flex-1 rounded py-0.5 font-mono text-[5px] transition-all",
              filters[filter.key]
                ? "font-bold"
                : "border border-[#2a2a3a] bg-[#1a1a1a] text-white/30",
              !isOn && "pointer-events-none opacity-40",
            )}
            style={{
