...
interface WaveformProps {
  ...
  is3D?: boolean;
  // Extended controls
  speed?: number; // Animation speed (default 0.05)
  zDepth?: number; // 3D layer depth spread (default 1)
  noise?: number; // Random noise amount 0-1 (default 0)
}
...
export function Waveform({
    ...
    (animPhase: number): number[] => {
      ...
      for (let i = 0; i < samples; i++) {
        ...
        switch (type) {
          ...
          case "bell":
            y =
              Math.sin(x) * Math.exp(-((i / samples) * 3)) * amplitude +
          ...
          case "pluck":
            y =
              Math.sin(x) *
              Math.exp(-((i / samples) * 4)) *
              amplitude *
              (1 + 0.3 * Math.sin(x * 8) * Math.exp(-((i / samples) * 8)));
            break;
          case "pad":
            y =
              (Math.sin(x) + Math.sin(x * 1.005) + Math.sin(x * 0.995)) *
              amplitude *
              0.33 *
          ...
          case "bass":
            y =
              (Math.sin(x) * 0.7 + Math.sin(x * 2) * 0.2 + (Math.sin(x) > 0 ? 0.1 : -0.1)) *
              amplitude;
            break;
          case "lead":
          ...
          case "vocal":
            y =
              (Math.sin(x) * (1 + 0.3 * Math.sin(x * 5)) + Math.sin(x * 1.5) * 0.3) *
              amplitude *
              0.65;
            break;
          case "breath":
            y =
              (Math.random() * 0.3 + Math.sin(x) * 0.7) *
              (0.7 + 0.3 * Math.sin(animPhase * 0.2)) *
              amplitude;
            break;
          case "wind":
            y =
              ((Math.random() * 2 - 1) * 0.4 + Math.sin(x * 0.5) * 0.6) *
              (0.5 + 0.5 * Math.sin(animPhase * 0.1 + i * 0.01)) *
              amplitude;
            break;
          case "string":
            y =
          ...
          case "brass": {
            ...
            y =
              (Math.sin(x) + Math.sin(x * 2) * 0.6 + Math.sin(x * 3) * 0.4) *
              amplitude *
              0.5 *
              attack;
            break;
          ...
          case "keys":
            y =
              (Math.sin(x) + Math.sin(x * 2) * 0.4) *
              Math.exp(-((i / samples) * 2)) *
              amplitude *
              0.71;
        ...
        // Apply noise
        if (noise > 0) {
          y += (Math.random() - 0.5) * 2 * noise * amplitude;
        }
  ...
  const draw = useCallback(() => {
    ...
    if (is3D) {
      ...
      for (let layer = layers - 1; layer >= 0; layer--) {
        const layerOffset = layer * 3 * zDepth;
  ...
  }, [width, height, color, showGrid, gridColor, generateWaveform, data, is3D, zDepth]);
