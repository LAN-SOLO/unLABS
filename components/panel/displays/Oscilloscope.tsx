...
export function Oscilloscope({
  ...
  const presetConfigs: Record<string, PresetConfig> = {
    ...
    random: {
      ...
      is3D: true,
      intensity: 75,
      colors: { R: 50, G: 50, B: 50, C: 50, M: 50, Y: 50 },
      xGain: 50,
      yGain: 50,
      phaseKnob: 50,
      amplKnob: 50,
  ...
  // Info display mode
  const [infoMode, setInfoMode] = useState<"WAVE" | "SIGNAL" | "COLOR" | "SYS" | "PRESET" | "TIME">(
    "WAVE",
  );
  const infoModes: ("WAVE" | "SIGNAL" | "COLOR" | "SYS" | "PRESET" | "TIME")[] = [
    "WAVE",
    "SIGNAL",
    "COLOR",
    "SYS",
  ...
  const startBootSequence = () => {
    ...
    const runStage = () => {
      ...
      setComponentTests((prev) =>
        prev.map((t, i) => (i === stage.testIndex ? { ...t, status: "testing" } : t)),
      );
      ...
      bootTimeoutRef.current = setTimeout(() => {
        ...
        setComponentTests((prev) =>
          prev.map((t, i) =>
            i === stage.testIndex
              ? { ...t, status: passed ? "pass" : "fail", value: stage.value }
              : t,
          ),
        );
  ...
  // X-O: horizontal phase offset
  const horizontalOffset = useMemo(() => {
    return (xOff / 100) * Math.PI;
  }, [xOff]);

  // Z-O: additional vertical offset
  const zVerticalOffset = useMemo(() => {
    return (zOff / 100) * 0.3;
  }, [zOff]);

  // Z-G: 3D layer depth spread
  const zDepthValue = useMemo(() => {
    return 1 + (zGain / 100) * 2;
  }, [zGain]);

  // NOS: noise amount (0-1)
  const noiseAmount = useMemo(() => {
    return noiseKnob / 100;
  }, [noiseKnob]);

  // CHS: chaos frequency modulation
  const chaosModulation = useMemo(() => {
    return 1 + (chaosKnob / 100) * 0.5;
  }, [chaosKnob]);

  // SPD: animation speed
  const animationSpeed = useMemo(() => {
    return 0.02 + (speedKnob / 100) * 0.08;
  }, [speedKnob]);

  // FOC: CRT focus/blur
  const focusBlur = useMemo(() => {
    return (100 - focus) * 0.02;
  }, [focus]);
  ...
  const KnobWithInput = ({
    ...
    <div className="flex flex-col items-center">
      ...
      <input
        ...
        className="w-8 rounded border border-[#0a1a0a] bg-[#1a2a1a] px-0.5 py-0 text-center font-mono text-[7px] text-white"
      />
    </div>
  );
  ...
  const isDeviceOff =
    powerState === "off" || powerState === "booting" || powerState === "shutting_down";
    ...
    <div
      className={cn(
        ...
        isDeviceOff
          ...
          : "border-[#3a4a3a] bg-gradient-to-b from-[#5a6a5a] to-[#4a5a4a]",
        className,
      )}
    >
      ...
      <div className="flex flex-shrink-0 items-center justify-between">
        <div className="flex items-center gap-1">
          <span
            className={cn("font-mono text-[8px]", isDeviceOff ? "text-white/40" : "text-white/80")}
          >
            INT
          </span>
          <Knob
            ...
            size="sm"
            accentColor={isDeviceOff ? "#3a3a3a" : "#ffffff"}
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            className={cn(
              "rounded border px-2 py-1 font-mono text-[8px] font-bold transition-opacity",
              maikActive && powerState === "on"
                ? "bg-[var(--neon-green)] text-black"
                : "border-[#1a2a1a] bg-[#2a3a2a] text-[var(--neon-green)]",
              isDeviceOff && "cursor-not-allowed opacity-30",
            )}
          ...
          <button
            className={cn(
              "rounded border px-2 py-1 font-mono text-[8px] font-bold transition-opacity",
              wallet.connected && powerState === "on"
                ? "bg-[#bfff00] text-black"
                : "border-[var(--neon-amber)] bg-[#2a2a1a] text-[var(--neon-amber)]",
              isDeviceOff && "cursor-not-allowed opacity-30",
            )}
            onClick={() => {
      ...
      {isDeviceOff ? (
        <>
          {/* Main boot display */}
          <div className="relative flex-1 overflow-hidden rounded border border-[#2a2a2a] bg-[#0a0a0a]">
            {powerState === "off" ? (
              // Off state - dark screen
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center">
                  <div className="mb-2 font-mono text-[32px] font-bold text-[#1a1a1a]">OZSC</div>
              ...
              <div className="absolute inset-0 flex flex-col p-2">
                    ...
                    <div
                      ...
                      className={cn(
                        ...
                        test.status === "pending"
                          ...
                          : test.status === "testing"
                            ? "animate-pulse border-[var(--neon-amber)] bg-[#1a1a0a]"
                            : test.status === "pass"
                              ? "border-[var(--neon-green)] bg-[#0a1a0a]"
                              : test.status === "fail"
                                ? "border-[var(--neon-red)] bg-[#1a0a0a]"
                                : "border-[#2a2a2a] bg-[#0a0a0a]",
                      )}
                    >
                      ...
                      <div
                        className={cn(
                          ...
                          test.status === "pending"
                            ? "bg-[#2a2a2a]"
                            : test.status === "testing"
                              ? "animate-pulse bg-[var(--neon-amber)]"
                              : test.status === "pass"
                                ? "bg-[var(--neon-green)]"
                                : test.status === "fail"
                                  ? "bg-[var(--neon-red)]"
                      ...
                      />
                      <span className="mt-0.5 max-w-full truncate font-mono text-[5px] text-white/40">
                        {test.value || "---"}
                      </span>
                    </div>
                  ))}
                ...
                {/* Boot log */}
                <div className="flex-1 overflow-hidden rounded border border-[#1a1a1a] bg-[#050505] p-1.5 font-mono text-[7px]">
                  <div className="h-full overflow-y-auto">
                    ...
                    {powerState === "booting" && bootStage !== "running" && (
                      <span className="inline-block h-3 w-2 animate-pulse bg-[var(--crt-green)]" />
                    )}
                  </div>
                </div>
                ...
                <div className="mt-2">
                  ...
                  <div className="h-2 overflow-hidden rounded border border-[#2a2a2a] bg-[#0a0a0a]">
                    <div
                      className={cn(
          ...
          {/* Status bar when off */}
          <div className="flex items-center justify-between rounded border border-[#2a2a2a] bg-[#1a1a1a] px-2 py-1">
            <div className="flex items-center gap-2">
              <LED
                on={powerState !== "off"}
                color={powerState === "booting" ? "amber" : "red"}
                size="sm"
              />
              <span className="font-mono text-[7px] text-white/50">
                {powerState === "off"
                  ? "STANDBY"
                  : powerState === "booting"
                    ? `BOOT: ${bootStage.toUpperCase().replace("_", " ")}`
        ...
        <>
          ...
          <CRTScreen className="min-h-[120px] w-full flex-1">
            ...
            >
              <Waveform
                type={waveType}
                frequency={frequency * horGainValue * chaosModulation}
                amplitude={finalAmplitude}
                color={waveColor}
                width={480}
                height={310}
                animated={isAnimated}
                offset={verticalOffset + zVerticalOffset}
                phase={phaseShift + horizontalOffset}
              ...
              />
              <div className="absolute top-1 right-1 text-right font-mono text-[7px] leading-tight text-[var(--crt-green)]">
                <div>
                  F1:{frequency.toFixed(1)}Hz F2:{secondFrequency.toFixed(1)}Hz
                </div>
                <div
                  className={cn(
                    "font-bold",
                    isAnimated ? "text-[var(--neon-amber)]" : "text-white/40",
                  )}
                >
          ...
          <div className="flex flex-shrink-0 gap-1">
            <div className="flex flex-1 items-center justify-between rounded border border-[#4a4a9a] bg-gradient-to-r from-[#2a2a6a] to-[#3a3a8a] px-2 py-1.5">
              <div className="flex flex-col">
                <span className="font-mono text-[7px] text-[var(--neon-cyan)]/60">
                  {wallet.connected ? walletDisplayAddress : "WALLET"}
                </span>
                <span className="font-mono text-[10px] leading-tight font-bold text-[var(--neon-cyan)]">
                  {wallet.connected ? "CONNECTED" : "NOT CONNECTED"}
                </span>
              </div>
              <div className="flex flex-col items-end">
                <span className="font-mono text-[6px] text-[var(--neon-cyan)]/50">_unSC</span>
                <span className="font-mono text-base leading-tight font-bold text-[var(--neon-cyan)]">
                  {formattedBalance}
                </span>
              </div>
            </div>
            <select
              value={waveType}
              onChange={(e) => setWaveType(e.target.value as typeof waveType)}
              className="max-h-[200px] rounded border border-[#0a1a0a] bg-[#1a2a1a] px-1 py-0.5 font-mono text-[8px] text-white"
            >
            ...
            <select
              ...
              className="rounded border border-[#0a1a0a] bg-[#1a2a1a] px-1 py-0.5 font-mono text-[8px] text-white"
            >
              {presets.map((p) => (
          ...
          {/* Sliders row */}
          <div className="flex flex-shrink-0 items-center gap-1 rounded border border-[#1a2a1a] bg-[#3a4a3a] p-1">
            <div className="flex min-w-0 flex-1 items-center gap-0.5 overflow-hidden">
              <span className="shrink-0 font-mono text-[7px] text-[var(--neon-blue)]">F1</span>
              <Slider
                value={freq1 * 10}
                min={5}
                max={100}
                onChange={(v) => setFreq1(v / 10)}
              ...
              />
              <span className="shrink-0 font-mono text-[7px] text-[var(--neon-blue)]">
                {freq1.toFixed(1)}
              </span>
            </div>
            <div className="flex min-w-0 flex-1 items-center gap-0.5 overflow-hidden">
              <span className="shrink-0 font-mono text-[7px] text-[var(--neon-blue)]">F2</span>
              <Slider
                value={freq2 * 10}
                min={5}
                max={100}
                onChange={(v) => setFreq2(v / 10)}
                orientation="horizontal"
                height={35}
                width={6}
                accentColor="var(--neon-blue)"
            ...
            <div className="flex min-w-0 flex-1 items-center gap-0.5 overflow-hidden">
              <span className="shrink-0 font-mono text-[7px] text-[var(--neon-amber)]">Int</span>
              <Slider
                value={interference}
                min={0}
                max={100}
                ...
                disabled={!interferenceEnabled}
                className="min-w-0 shrink"
              />
              <span className="shrink-0 font-mono text-[7px] text-[var(--neon-amber)]">
                {interference}%
              </span>
            </div>
            <div className="flex min-w-0 flex-1 items-center gap-0.5 overflow-hidden">
              <span className="shrink-0 font-mono text-[7px] text-[var(--neon-pink)]">Wv</span>
              <Slider
                value={wavelength * 10}
                min={10}
                max={200}
                ...
                className="min-w-0 shrink"
              />
              <span className="shrink-0 font-mono text-[7px] text-[var(--neon-pink)]">
            ...
            <label className="flex shrink-0 items-center gap-0.5">
              <input
                type="checkbox"
                checked={interferenceEnabled}
                onChange={(e) => setInterferenceEnabled(e.target.checked)}
          ...
          <div className="flex-shrink-0 rounded border border-[#1a2a1a] bg-[#3a4a3a] p-1">
          ...
          <div className="flex flex-shrink-0 items-center gap-1 rounded border border-[#1a2a1a] bg-[#3a4a3a] px-1.5 py-0.5">
            <span className="font-mono text-[7px] font-bold text-[var(--neon-amber)]">AUD</span>
            <button
              onClick={() => setAudioEnabled(!audioEnabled)}
              className={cn(
                "relative h-3 w-6 rounded-full",
                audioEnabled ? "bg-[var(--neon-green)]" : "bg-[var(--neon-red)]",
              )}
            >
              <div
                className={cn(
                  "absolute top-0.5 h-2 w-2 rounded-full bg-white",
                  audioEnabled ? "left-3" : "left-0.5",
                )}
              />
            </button>
            ...
            <div className="ml-auto flex items-center gap-1">
              <div className="min-w-[120px] rounded border border-[#1a2a1a] bg-[#0a1a0a] px-1.5 py-0.5">
                <div className="mb-0.5 flex items-center justify-between">
                ...
                <div className="font-mono text-[7px] leading-tight text-[var(--crt-green)]">
                  {infoMode === "WAVE" && (
                    <>
                      <div className="flex justify-between">
                        <span>TYPE:</span>
                      ...
                      <div className="flex justify-between">
                        <span>F1/F2:</span>
                        <span>
                          {freq1.toFixed(1)}/{freq2.toFixed(1)}Hz
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span>3D:</span>
                        <span
                          className={is3DEnabled ? "text-[var(--neon-green)]" : "text-white/30"}
                        >
                          {is3DEnabled ? "ON" : "OFF"}
                        </span>
                      </div>
                  ...
                  {infoMode === "SIGNAL" && (
                    <>
                      ...
                      <div className="flex justify-between">
                        ...
                        <span
                          className={
                            interferenceEnabled ? "text-[var(--neon-amber)]" : "text-white/30"
                          }
                        >
                          {interferenceEnabled ? `${interference}%` : "OFF"}
                        </span>
                      </div>
                    </>
                  )}
                  ...
                  {infoMode === "SYS" && (
                    <>
                      <div className="flex justify-between">
                        ...
                        <span
                          className={
                            audioEnabled ? "text-[var(--neon-amber)]" : "text-[var(--neon-green)]"
                          }
                        >
                          {audioEnabled ? "23%" : "4%"}
                        </span>
                    ...
                    <>
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-[var(--neon-amber)]">
                          {xGain + yGain + freqKnob > 0 ? "CUSTOM" : "STD"}
                        </span>
                      </div>
                    </>
                  )}
              ...
              <button
                ...
                className="flex h-10 w-5 items-center justify-center rounded border border-[#1a2a1a] bg-[#2a3a2a] transition-colors hover:bg-[#3a4a3a] active:bg-[#1a2a1a]"
                title="Cycle info mode"
              >
                <span
                  className="writing-mode-vertical font-mono text-[6px] text-[var(--neon-cyan)]"
          ...
          <div className="flex flex-shrink-0 gap-1 rounded border border-[#1a2a1a] bg-[#2a3a2a] p-1">
            {/* Color sliders */}
            <div className="flex flex-1 gap-0.5">
              {WAVEFORM_COLORS.map((color) => {
                ...
                return (
                  <div key={color.label} className="flex flex-1 flex-col items-center">
                    <span className="font-mono text-[5px] font-bold text-white/70">
                      {color.label}
                    </span>
                    <div
                      className="relative h-10 w-3 cursor-pointer rounded-sm"
                      style={{
                      ...
                      onClick={(e) => {
                        ...
                        const percent = Math.max(
                          0,
                          Math.min(100, Math.round((1 - y / rect.height) * 100)),
                        );
                        setSelectedColors((prev) => ({ ...prev, [color.label]: percent }));
                      }}
                    >
                      <div
                        className="absolute right-0 bottom-0 left-0 rounded-sm transition-all duration-75"
                        style={{
                          height: `${intensity}%`,
            ...
            {/* Inline color info */}
            <div className="flex w-20 flex-col justify-between border-l border-[#1a2a1a] pl-1">
              <div
