...
export function ScrewButton({
  ...
  const handleLongPressComplete = useCallback(async () => {
    ...
    const success = wasActive ? await onDeactivate() : await onActivate();
  ...
  const ledGlowColor =
    ledColor === "green"
      ? "var(--screw-led-green-glow)"
      : ledColor === "red"
        ? "var(--screw-led-red-glow)"
        : ledColor === "amber"
          ? "var(--screw-led-amber-glow)"
          : "transparent";

  const animationClass =
    pressState === "pressing"
      ? "animate-screw-led-charge"
      : pressState === "activating"
        ? "animate-screw-led-transition"
        : pressState === "active"
          ? "animate-screw-led-active"
          : "";
  ...
  return (
    ...
    >
      {/* LED Ring */}
      <div
        className={cn("pointer-events-none absolute inset-[-3px] rounded-full", animationClass)}
        style={{
