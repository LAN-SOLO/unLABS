 ...
 */
export function CRTShutdownEffect({
  active,
  onComplete,
  holdBlack = false,
  scope = "system",
}: CRTShutdownEffectProps) {
  const [phase, setPhase] = useState<
    "idle" | "surge" | "collapse-h" | "collapse-v" | "phosphor" | "black" | "hold"
  >("idle");
  ...
  return (
    ...
    >
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div
            style={{
              ...
              background: "white",
              boxShadow:
                "0 0 8px 4px rgba(255, 255, 255, 0.6), 0 0 20px 8px rgba(0, 255, 100, 0.4), 0 0 40px 16px rgba(0, 255, 100, 0.2)",
      ...
      {/* Final black / hold */}
      {(phase === "black" || phase === "hold") && <div className="absolute inset-0 bg-black" />}
    </div>
  );
}
