...
export function QSMManagerProvider({ children, initialState }: QSMManagerProviderProps) {
  ...
  const [deviceState, setDeviceState] = useState<QSMDeviceState>(
  ...
  const [currentDraw, setCurrentDraw] = useState(
    startPowered ? QSM_POWER_SPECS.full : QSM_POWER_SPECS.standby,
  );
  ...
  const runTest = useCallback(async () => {
    ...
    const phases: NonNullable<QSMTestPhase>[] = [
      "coherence",
      "entanglement",
      "decoherence",
      "error",
  ...
  return <QSMManagerContext.Provider value={value}>{children}</QSMManagerContext.Provider>;
}
