...
export function CPUManagerProvider({ children, initialState }: CPUManagerProviderProps) {
  ...
  const [deviceState, setDeviceState] = useState<CPUDeviceState>(
  ...
  const runTest = useCallback(async () => {
    ...
    const phases: NonNullable<CPUTestPhase>[] = [
      "cores",
      "cache",
      "frequency",
      "thermal",
    ...
    const interval = setInterval(() => {
      if (deviceState === "online") {
        setCoreLoads(
          Array(cores)
            .fill(0)
            .map(() => utilization + (Math.random() - 0.5) * 50),
        );
      } else if (deviceState === "testing") {
        setCoreLoads(
          Array(cores)
            .fill(0)
            .map(() => 85 + (Math.random() - 0.5) * 30),
        );
  ...
  return <CPUManagerContext.Provider value={value}>{children}</CPUManagerContext.Provider>;
}
