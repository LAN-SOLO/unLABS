...
export function QUAManagerProvider({ children, initialState }: QUAManagerProviderProps) {
  ...
  const [deviceState, setDeviceState] = useState<QUADeviceState>(
  ...
  const [currentDraw, setCurrentDraw] = useState(
    startPowered ? QUA_POWER_SPECS.full : QUA_POWER_SPECS.standby,
  );
  ...
  const runTest = useCallback(async () => {
    ...
    const phases: NonNullable<QUATestPhase>[] = [
      "quantum-core",
      "sensors",
      "neural-net",
      "calibration",
  ...
  return <QUAManagerContext.Provider value={value}>{children}</QUAManagerContext.Provider>;
}
