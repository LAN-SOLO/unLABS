...
export function TLPManagerProvider({ children, initialState }: TLPManagerProviderProps) {
  const isUnlocked = useDeviceUnlocked("TLP-001");
  ...
  const [deviceState, setDeviceState] = useState<TLPDeviceState>(
  ...
  const [lastDestination, setLastDestinationState] = useState(
    initialState?.lastDestination ?? "LAB-Ω",
  );
  ...
  return <TLPManagerContext.Provider value={value}>{children}</TLPManagerContext.Provider>;
}
