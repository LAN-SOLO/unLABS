...
export function CDCManagerProvider({ children, initialState }: CDCManagerProviderProps) {
  ...
  const [deviceState, setDeviceState] = useState<CDCDeviceState>(
  ...
  const runTest = useCallback(async () => {
    ...
    const phases: NonNullable<CDCTestPhase>[] = [
      "memory",
      "bus",
      "cache",
      "power",
  ...
  const updateData = useCallback(
    (crystals: number, slices: number, power: number) => {
  ...
  return <CDCManagerContext.Provider value={value}>{children}</CDCManagerContext.Provider>;
}
