...
export function EXDManagerProvider({ children, initialState }: EXDManagerProviderProps) {
  ...
  const [deviceState, setDeviceState] = useState<EXDDeviceState>(
  ...
  const [currentDraw, setCurrentDraw] = useState(
    startPowered ? EXD_POWER_SPECS.full : EXD_POWER_SPECS.standby,
  );
  ...
  useEffect(() => {
    if (deviceState === "online" && isPowered && isDeployed) {
      const interval = setInterval(() => {
        ...
        setCurrentDraw(
          EXD_POWER_SPECS.idle + (EXD_POWER_SPECS.full - EXD_POWER_SPECS.idle) * (speed / 80),
        );
      }, 2000);
  ...
  const runTest = useCallback(async () => {
    ...
    const phases: NonNullable<EXDTestPhase>[] = [
      "motors",
      "gps",
      "camera",
      "radio",
  ...
  return <EXDManagerContext.Provider value={value}>{children}</EXDManagerContext.Provider>;
}
