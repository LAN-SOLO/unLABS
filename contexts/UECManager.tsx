...
export function UECManagerProvider({ children, initialState }: UECManagerProviderProps) {
  ...
  const runTest = useCallback(async () => {
    ...
    const phases: NonNullable<UECTestPhase>[] = [
      "voltage",
      "frequency",
      "stability",
      "output",
  ...
  const updateVolatility = useCallback(
    (tier: number, newTps: number) => {
  ...
  return <UECManagerContext.Provider value={value}>{children}</UECManagerContext.Provider>;
}
