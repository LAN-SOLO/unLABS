...
export function MEMManagerProvider({ children, initialState }: MEMManagerProviderProps) {
  ...
  const [deviceState, setDeviceState] = useState<MEMDeviceState>(
  ...
  const [displayMode, setDisplayMode] = useState<MEMMode>(
    (initialState?.displayMode as MEMMode) ?? "usage",
  );
  ...
  const runTest = useCallback(async () => {
    ...
    const phases: NonNullable<MEMTestPhase>[] = [
      "modules",
      "timing",
      "integrity",
      "bandwidth",
  ...
  const setTotalMemory = useCallback(
    (value: number) => {
      if (deviceState !== "online") return;
  ...
  const setUsedMemory = useCallback(
    (value: number) => {
      if (deviceState !== "online") return;
  ...
  return <MEMManagerContext.Provider value={value}>{children}</MEMManagerContext.Provider>;
}
