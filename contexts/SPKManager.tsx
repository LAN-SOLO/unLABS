...
export function SPKManagerProvider({ children, initialState }: SPKManagerProviderProps) {
  ...
  const [filters, setFilters] = useState(
    initialState?.filters ?? { bass: false, mid: true, high: false },
  );
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const levels = Array.from({ length: 8 }, () => Math.random() * (volume / 100) * 100);
  ...
  const runTest = useCallback(async () => {
    ...
    const phases: NonNullable<SPKTestPhase>[] = [
      "driver",
      "amplifier",
      "dac",
      "filter",
  ...
  const setFilter = useCallback(
    (filter: "bass" | "mid" | "high", enabled: boolean) => {
      if (deviceState !== "online") return;
      ...
      if (startPowered) {
        runBootSequence().then(() => {
          if (savedStateRef.current) {
            if (savedStateRef.current.volume !== undefined)
              setVolumeState(savedStateRef.current.volume);
            if (savedStateRef.current.isMuted !== undefined)
              setIsMutedState(savedStateRef.current.isMuted);
  ...
  return <SPKManagerContext.Provider value={value}>{children}</SPKManagerContext.Provider>;
}
