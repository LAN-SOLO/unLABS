...
export function EMCManagerProvider({ children, initialState }: EMCManagerProviderProps) {
  ...
  const [deviceState, setDeviceState] = useState<EMCDeviceState>(
  ...
  const [currentDraw, setCurrentDraw] = useState(
    startPowered ? EMC_POWER_SPECS.full : EMC_POWER_SPECS.standby,
  );
  ...
  const runTest = useCallback(async () => {
    ...
    const phases: NonNullable<EMCTestPhase>[] = [
      "containment",
      "field",
      "stability",
      "particles",
  ...
  return <EMCManagerContext.Provider value={value}>{children}</EMCManagerContext.Provider>;
}
