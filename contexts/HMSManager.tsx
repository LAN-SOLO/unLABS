...
export function HMSManagerProvider({ children, initialState }: HMSManagerProviderProps) {
  ...
  const [waveformType, setWaveformType] = useState<"sine" | "square" | "saw" | "triangle">(
    (initialState?.waveformType as "sine" | "square" | "saw" | "triangle") ?? "sine",
  );
  ...
  const runTest = useCallback(async () => {
    ...
    const phases: NonNullable<HMSTestPhase>[] = [
      "oscillator",
      "waveform",
      "filter",
      "output",
  ...
  const setKnobValue = useCallback(
    (knob: "pulse" | "tempo" | "freq", value: number) => {
      if (deviceState !== "online") return;
      switch (knob) {
        case "pulse":
          setPulseValue(value);
  ...
  return <HMSManagerContext.Provider value={value}>{children}</HMSManagerContext.Provider>;
}
