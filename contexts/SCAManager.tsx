...
export function SCAManagerProvider({ children, initialState }: SCAManagerProviderProps) {
  ...
  const [deviceState, setDeviceState] = useState<SCADeviceState>(
  ...
  const [currentDraw, setCurrentDraw] = useState(
    startPowered ? SCA_POWER_SPECS.full : SCA_POWER_SPECS.standby,
  );
  ...
  useEffect(() => {
    if (deviceState === "online" && isPowered) {
      const interval = setInterval(() => {
        ...
        setInterconnectBandwidth((prev) =>
          Math.max(80, Math.min(200, prev + (Math.random() - 0.5) * 20)),
        );
        ...
        setCurrentDraw(
          SCA_POWER_SPECS.idle +
            (SCA_POWER_SPECS.full - SCA_POWER_SPECS.idle) * (utilization / 100),
        );
        // Increment uptime
  ...
  const runTest = useCallback(async () => {
    ...
    const phases: NonNullable<SCATestPhase>[] = [
      "nodes",
      "interconnect",
      "memory",
      "cache",
  ...
  return <SCAManagerContext.Provider value={value}>{children}</SCAManagerContext.Provider>;
}
