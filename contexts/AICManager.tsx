...
export function AICManagerProvider({ children, initialState }: AICManagerProviderProps) {
  ...
  useEffect(() => {
    if (deviceState === "online" && isPowered) {
      const interval = setInterval(() => {
        ...
        setEfficiency((prev) =>
          Math.max(120, Math.min(180, prev + Math.floor(Math.random() * 10) - 5)),
        );
  ...
  const runTest = useCallback(async () => {
    ...
    const phases: NonNullable<AICTestPhase>[] = [
      "neural",
      "memory",
      "logic",
      "learning",
  ...
  return <AICManagerContext.Provider value={value}>{children}</AICManagerContext.Provider>;
}
