...
export function SystemPowerManagerProvider({
  children,
  onShutdownComplete,
  onRebootComplete,
  saveDeviceState,
  ...
  const startCountdown = useCallback(
    (seconds: number, action: PowerAction) => {
      clearTimer();
  ...
  const scheduleShutdown = useCallback(
    (seconds: number, scope: PowerScope = "system") => {
...
export function useSystemPowerInternal() {
  const ctx = useContext(SystemPowerContext);
  if (!ctx)
    throw new Error("useSystemPowerInternal must be used within SystemPowerManagerProvider");
