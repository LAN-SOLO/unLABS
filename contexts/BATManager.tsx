...
export function BATManagerProvider({ children, initialState }: BATManagerProviderProps) {
  ...
  const powerOff = useCallback(async () => {
    if (deviceState !== "online" && deviceState !== "charging" && deviceState !== "discharging")
      return;
    setIsPowered(false);
  ...
  const runTest = useCallback(async () => {
    ...
    const phases: NonNullable<BATTestPhase>[] = [
      "cells",
      "voltage",
      "capacity",
      "discharge",
  ...
  // Set auto-regen
  const setAutoRegen = useCallback(
    (enabled: boolean) => {
  ...
  return <BATManagerContext.Provider value={value}>{children}</BATManagerContext.Provider>;
}
