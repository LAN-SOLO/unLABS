...
export function VNTManagerProvider({ children, initialState }: VNTManagerProviderProps) {
  ...
  const [deviceState, setDeviceState] = useState<VNTDeviceState>(
  ...
  const [statusMessage, setStatusMessage] = useState(
    startPowered ? "Initializing..." : "Standby mode",
  );
  ...
  const [localCpuFan, setLocalCpuFan] = useState<VNTFanState>({
    speed: savedCpuSpeed,
    rpm: Math.round((savedCpuSpeed / 100) * 4000 + 800),
    mode: savedMode,
    isOn: true,
  });
  const [localGpuFan, setLocalGpuFan] = useState<VNTFanState>({
    speed: savedGpuSpeed,
    rpm: Math.round((savedGpuSpeed / 100) * 4000 + 800),
    mode: savedMode,
    isOn: true,
  });
  ...
  const cpuFan: VNTFanState = cpuFanState
    ? {
        speed: cpuFanState.speed,
        rpm: cpuFanState.rpm,
        mode: cpuFanState.mode,
        isOn: cpuFanState.isOn,
      }
    : localCpuFan;
  const gpuFan: VNTFanState = gpuFanState
    ? {
        speed: gpuFanState.speed,
        rpm: gpuFanState.rpm,
        mode: gpuFanState.mode,
        isOn: gpuFanState.isOn,
      }
    : localGpuFan;
  ...
  const runTest = useCallback(async () => {
    ...
    const phases: NonNullable<VNTTestPhase>[] = [
      "motor",
      "airflow",
      "filter",
      "damper",
  ...
  const setFanMode = useCallback(
    (fanId: "cpu" | "gpu", mode: "AUTO" | "LOW" | "MED" | "HIGH") => {
      if (thermalManager) {
  ...
  return <VNTManagerContext.Provider value={value}>{children}</VNTManagerContext.Provider>;
}
