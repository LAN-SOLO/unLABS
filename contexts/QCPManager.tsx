...
export function QCPManagerProvider({ children, initialState }: QCPManagerProviderProps) {
  ...
  const [deviceState, setDeviceState] = useState<QCPDeviceState>(
  ...
  const [anomalyDirection, setAnomalyDirectionState] = useState(
    initialState?.anomalyDirection ?? 127,
  );
  ...
  return <QCPManagerContext.Provider value={value}>{children}</QCPManagerContext.Provider>;
}
