...
export function MFRManagerProvider({ children, initialState }: MFRManagerProviderProps) {
  ...
  useEffect(() => {
    if (deviceState === "online" && isPowered) {
      const interval = setInterval(() => {
        ...
        setPlasmaTemp((prev) =>
          Math.max(14000, Math.min(16000, prev + (Math.random() - 0.5) * 200)),
        );
  ...
  const runTest = useCallback(async () => {
    ...
    const phases: NonNullable<MFRTestPhase>[] = [
      "plasma",
      "containment",
      "coolant",
      "output",
  ...
  return <MFRManagerContext.Provider value={value}>{children}</MFRManagerContext.Provider>;
}
