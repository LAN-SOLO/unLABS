...
export async function fetchCrystals(): Promise<Crystal[]> {
  ...
  const { data, error } = await supabase
    .from("crystals")
    .select(
      "id, name, color, volatility, rotation, state, era, is_genesis, total_power, slice_count",
    )
    .eq("owner_id", user.id)
...
export async function logCommand(
  ...
  await (
    supabase.from("command_history") as unknown as {
...
export async function fetchVolatility(): Promise<{
  tps: number;
  tier: string;
  ...
  const { data, error } = await (
    supabase.from("volatility_snapshots") as unknown as {
      select: (cols: string) => {
        order: (
          col: string,
          ...
          limit: (n: number) => {
            single: () => Promise<{
              data: { tps: number; calculated_tier: string; block_time_ms: number } | null;
              error: unknown;
            }>;
          };
        };
