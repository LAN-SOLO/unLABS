...
export async function syncResourceState(saveData: ResourceSaveData): Promise<void> {
  ...
  for (const [id, entry] of Object.entries(saveData)) {
    ...
    rows.push({
      ...
      player_id: user.id,
      resource_type:
        RESOURCE_TYPE_MAP[def.resourceType] ??
        def.resourceType.toLowerCase().replace(/[.\s]/g, "_"),
  ...
  await fromAny(supabase, "resource_containers").upsert(rows, {
...
export async function advanceBootstrapPhase(newPhase: number): Promise<boolean> {
  ...
  const { error } = await fromAny(supabase, "bootstrap_state").upsert(
...
export async function fetchProductionRecipes(): Promise<ProductionRecipe[]> {
  ...
  const { data, error } = await fromAny(supabase, "production_recipes").select("*").order("tier");
...
export async function startProduction(
  ...
  const outputDef = RESOURCE_CONTAINERS.find(
    (d) =>
      (RESOURCE_TYPE_MAP[d.resourceType] ?? d.resourceType.toLowerCase().replace(/[.\s]/g, "_")) ===
      r.output_resource,
  );
  ...
  const { error } = await fromAny(supabase, "production_queue").insert({
    player_id: user.id,
    device_id: deviceId,
    recipe_id: recipeId,
    status: "active",
...
export async function fetchProductionQueue(): Promise<ProductionJob[]> {
  ...
  const { data, error } = await fromAny(supabase, "production_queue")
    .select(
      "queue_id, device_id, recipe_id, status, started_at, completes_at, output_resource, output_amount, energy_per_second",
    )
    .eq("player_id", user.id)
